...
本次版本升级涉及基础组件更新，若您使用 Zadig 内置的 **MongoDB，MySQL 或 MinIO（对象存储）**组件，请务必按照升级指南做好数据的备份/恢复工作
:::
...
MinIO 数据备份操作：
- 安装 MinIO 客户端 mc
参考[MinIO 官方文档](https://min.io/docs/minio/linux/reference/minio-mc.html#install-mc)，安装 mc
- 访问 MinIO 服务
```bash
...
```
- 对象存储设置：
登录 Zadig 系统，依次点击 `系统设置` -> `对象存储` 进入对象存储管理页面，编辑提供商为 `zadig-minio.${NAMESPACE}.svc.cluster.local:9000` 的记录，将接入地址改为 `zadig-minio.${NAMESPACE}.svc.cluster.local:9000`，点击保存。