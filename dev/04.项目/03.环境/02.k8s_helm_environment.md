...
在项目中，点击`创建环境`按钮，所需创建的环境后进入创建环境页面。
...
选择生产环境 <Badge text="企业版" /> 界面如下：

![新建 Helm Chart 环境](../_images/create_helm_chart_env_3.png)
...
- `环境配置`：为环境配置 Ingress、ConfigMap、Secret、PVC 资源，细节描述请阅读[环境配置管理](/dev/project/env/k8s/#环境配置管理)
...
- 可手动输入 values 内容
- 从代码库导入 values 文件
- 使用变量组，变量组管理参考[变量组配置](/dev/project/config/#变量组) <Badge text="企业版"/>
...
可对环境中的 Ingress、ConfigMap、Secret、PVC 配置进行操作，参考[环境配置管理](/dev/project/env/k8s/#环境配置管理)。
