...
> 对服务的构建产物进行分发部署操作，目前支持镜像仓库分发和存储空间分发。适用于使用构建部署更新环境并测试通过后，将服务构建产物上传到对象存储中，或部署到其他环境中。
::: tip 前提
...
- 部署到指定环境：将服务部署更新到指定的环境，需要确保指定环境可正常获取到指定镜像仓库中的镜像资源
