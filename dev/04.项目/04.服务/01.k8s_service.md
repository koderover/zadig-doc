...
permalink: /dev/project/service/k8s/
---
...
::: tip
1. 需要在系统中集成代码源，可参考：[代码源集成](/dev/settings/codehost/overview/)
2. 目前支持从 [GitLab](/dev/settings/codehost/gitlab/)/[GitHub](/dev/settings/codehost/github/)/[Gerrit](/dev/settings/codehost/gerrit/)/[Gitee](/dev/settings/codehost/gitee/) 代码源同步服务
:::
...
可参考[服务组件](/dev/quick-start/concepts/#服务组件)了解更多信息，此处相关字段说明如下：
...
::: tip 注意
  服务级别的变量高于全局变量 <br>
  比如：全局变量配置变量 k1=global，服务A 变量配置 k1=s1，服务B 没有配置 k1 变量，则最终服务A k1=s1，服务B k1=global；
:::
...
- 交付物命名规则设置：自定义该项目中工作流构建产物的命名规则，可通过以下[内置变量](#内置变量)和常量组合的方式设置，对当前项目的所有服务都生效。其中 `JENKINS 生成镜像规则`适用于使用 [Jenkins 构建](/dev/project/build/#jenkins-构建) 的服务，不可使用下述[内置变量](#内置变量)中的 `REPO_*` 变量。
