...
平台提供的构建系统中都安装了以下工具/命令，可以在[应用安装脚本](/dev/settings/app/)和构建脚本中直接使用。
...
> 支持使用自定义镜像作为构建系统，详情请参阅[构建镜像管理](/dev/settings/custom-image/#步骤-1-生成构建镜像)

#### 资源规格
平台提供 高/中/低/最低 四种配置供选择，可以根据实际需要自定义规格。

#### 集群名称
用于选择构建任务运行时所使用的集群，其中`本地集群`指 Zadig 系统所在的集群。关于集群的集成可参考[集群管理](/dev/pages/cluster_manage/)。
