...
> 如果没有集成 Jenkins，新建构建时默认创建的是 Zadig 构建；如果需要使用 Jenkins 构建，请先[集成 Jenkins](/dev/settings/jenkins/)。
...
1. 支持使用自定义镜像作为构建系统，详情请参阅[构建镜像管理](/dev/settings/custom-image/#步骤-1-生成构建镜像)。
2. 默认提供的 Ubuntu 系列构建系统中已安装了以下工具/命令，可以在[应用安装脚本](/dev/settings/app/)和构建脚本中直接使用。
...
- 平台已内置一些常见的应用，如果有其他应用或者版本需求，系统管理员可以在[应用管理](/dev/settings/app)中配置应用的安装脚本。
:::

- `高级配置`：
    - `集群名称`：选择构建任务运行时所使用的集群，其中`本地集群`指 Zadig 系统所在的集群，关于集群的集成可参考[集群管理](/dev/pages/cluster_manage/)。
...
::: tip 小贴士
- `自定义镜像`对应[构建脚本](/dev/project/build/#构建脚本)中的 `IMAGE` 变量
- `自定义 TAR 包`对应[构建脚本](/dev/project/build/#构建脚本)中的 `PKG_FILE` 变量
:::
...
::: tip
使用 Jenkins 构建前必须先集成 Jenkins 服务器，详见 [Jenkins 集成](/dev/settings/jenkins)
:::
...
- `构建参数`：`IMAGE` 参数，支持在 Zadig 中手动输入，传递给 Jenkins Job 作为构建镜像产物名称。

::: tip
- 所选的 Jenkins Job 中需要配置 Build Parameters，且包含 `IMAGE` 变量，作为构建出的镜像产物名称。
![jenkins build](./_images/jenkins_image_parameter.png)
![jenkins build](./_images/jenkins_image_build.png)
- Jenkins Job 成功构建出 `IMAGE` 并推送至镜像仓库后，Zadig 会使用 `IMAGE` 部署更新服务，请确保使用的镜像仓库集成无误，镜像仓库集成可参考：[镜像仓库管理](/dev/settings/image-registry/)。
:::
