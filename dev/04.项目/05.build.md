...
|`<REPO>_PR`|获取构建过程中指定代码仓库使用的 Pull Request 信息，若 REPO 中包含中划线 "-"，将其替换成 "_"，下图例中使用 $gss_PR、$zadig_infra_PR 即可|
|`<REPO>_BRANCH`|获取构建过程中指定代码仓库使用的分支信息，REPO 中包含中划线时需对其进行转换|
|`<REPO>_TAG`|获取构建过程中指定代码仓库使用的 Tag 信息，REPO 中包含中划线时需对其进行转换|
|`<REPO>_COMMIT_ID`|获取构建过程中指定代码仓库使用的 Commit ID 信息，REPO 中包含中划线时需对其进行转换|
