...
Zadig 中的构建包括 `Zadig 构建`和 `Jenkins 构建`，关于二者的区别如下：
...
- 提供模版化构建能力，通过配置通用构建脚本实现构建过程，简单易用。
- 整个构建模块包括通用配置、构建环境、代码信息、构建变量、通用构建脚本、高级配置以及自定义更多步骤，更多细节请阅读[定义构建](#定义构建)。
- 支持多个服务使用一份构建配置，更多细节请阅读[共享构建](#共享构建)。
- 支持修改构建交付物的生成规则，更多细节请阅读[构建交付产物生成规则](/dev/project/service/#策略配置)。
...
- `资源配置`：选择构建任务运行时所使用的集群资源及命名空间，其中本地集群指 Zadig 系统所在的集群，关于集群的集成可参考[集群管理](/dev/pages/cluster_manage/)。
- `操作系统规格`：配置资源规格用于执行构建任务。平台提供 高/中/低/最低 四种配置供选择，可以根据实际需要自定义规格。
