...
支持创建自定义变量，可在工作流任务中使用自定义变量。变量的值来源包括 2 种类型：
...
- `服务组件与构建选择`：选择服务组件与构建名称，支持配置构建变量及代码库默认分支，变量的配置参考：[变量配置](#变量来源配置)
...
### 变量来源配置
支持在内置构建任务、部署任务、通用任务中使用变量并指定变量值的来源，包括以下三种来源：
- `运行时输入`：运行自定义工作流时再指定该变量的值
- `固定值`：指定为固定值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量`：包括内置变量和[自定义变量](/dev/project/common-workflow/#变量配置)。工作流执行时不可修改内置变量的值（页面中该变量不可见），内置变量描述如下表：
...
<!-- TODO：从场景三开始，后续的图需要更换，目前有问题被 block -->
