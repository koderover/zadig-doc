...
- `运行时输入`：执行工作流时再指定该变量的值
- `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量/其他任务输出`：使用全局变量或前置任务输出的变量来为此处的变量赋值，全局变量包括 [项目变量](/dev/project/common-workflow/#项目) 和 [工作流变量](/dev/project/common-workflow/#工作流)
...
参数说明：
- `工作流执行变量`：希望被 Webhook 触发执行的任务，以及任务中的具体配置
- 其他参数：请参考 [触发器基本配置](/dev/project/workflow/#gui-方式) 中的描述在第三方系统中完成 Webhook 配置。
...
点击`调试`启动工作流进入调试模式，可在工作流执行时增加断点操作，便于调试工作流配置。

![debug_workflow](../_images/debug_mode.png)

点击`调试控制台`即可登录到当前任务的容器中，待当前任务执行完毕后，点击`继续`，工作流才会继续执行。
