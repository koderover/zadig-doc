...
## Helm项目工作流部署失败，如何解决
当工作流部署Helm服务遇到`cannot re-use a name that is still in use`或者`another operation (install/upgrade/rollback) is in progress`报错时，Helm release可能因为一些异常情形处于不正确的状态。可以按照以下情况解决：
* 如果release存在状态为deplyed的版本，手动将release rollback到最近的deplyed版本。
* 如果release当前状态处于superseded状态，并且release不存在状态为deployed的版本，手动执行helm upgrade命令，将release升级到新版本。
* 如果release长时间处于`pending-install/pending-upgrade/pending-rollback`状态，并且release不存在状态为deployed的版本，手动将release当前版本对应的secret从所在集群Namespace中删除，secret名称：sh.helm.release.v1.{ReleaseName}.v{ReleaseRevison}
