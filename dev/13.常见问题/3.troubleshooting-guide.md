...
Helm release 可能因为一些异常情形处于不正确的状态，通过`helm list -n <NameSpace> -a -f <ReleaseName>`以及`helm hisroty <ReleaseName> -n <NameSpace>`命令分别查看 release 状态以及版本记录: 
...
* 如果release 长时间处于`pending-install/pending-upgrade/pending-rollback`状态，并且 release 不存在状态为deployed 的版本，手动将 release 当前版本对应的 secret 从所在集群 Namespace 中删除，secret 名称：sh.helm.release.v1.{ReleaseName}.v{ReleaseRevison}。
