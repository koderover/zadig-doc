...
::: tip
1. 在项目中基于模板创建服务，以及创建环境时，均可修改自定义变量的默认值。
2. 自定义变量的 key 中不支持中划线。
3. Go template 的使用请参考 [官方文档](https://pkg.go.dev/text/template#hdr-Examples)。
:::


除了支持使用常量值，还支持使用 [服务配置](/dev/project/service/k8s/#变量配置) 中的变量来为自定义变量赋值，比如下图示例中，使用 `$EnvName$`、`$T-Service$` 给 `cmd` 赋值，使用 <span v-pre>`{{.REPLICAS_NUM}}`</span> 给 `replicas_num` 赋值。
