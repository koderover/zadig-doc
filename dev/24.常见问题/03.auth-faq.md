...
## 设置不可用的 OAuth 系统作为默认账号系统后登录 Zadig 会导致死循环，无法成功登录，如何解决

**现象描述：**

1. 集成 OAuth 账号系统，在其实际不可用的情况下将其设置为默认账号系统。

![set_oauth_as_default_login](./_images/set_oauth_as_default_login.png)

2. 在登录 Zadig 时，因 OAuth 系统不可用导致登录页如下图所示，无法成功登入 Zadig，导致无法修改默认账号系统。

![oauth_login_fail](./_images/oauth_login_fail.png)

**解决办法：**

访问 `{Zadig 安装地址}/login/password`，并使用在 Zadig 系统中创建的管理员账号和密码即可成功登入。

::: tip
若安装 Zadig 系统时未修改，则默认的管理员账号/密码为 `admin`/`zadig`。
:::

![public_login_page](./_images/public_login_page.png)
...
在系统设置 -> 集成管理 -> 系统集成 -> 账号系统集成中可查看集成账号系统的配置，参考 [LDAP](/dev/settings/account/ldap/)/[OAuth](/dev/settings/account/oauth2/)/[GitHub](/dev/settings/account/github/)/[自定义](/dev/settings/account/custom/)账号系统对检查有误的信息进行修改。
...
通常原因为 OAuth 配置错误，可以查阅 [OAuth 配置](/dev/settings/account/oauth2/) 进行检查。以下图中的错误配置为例示范：
