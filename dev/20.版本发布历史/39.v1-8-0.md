...
::: tip 注意事项
- 执行升级脚本之前需重新 export 参数，<font color=#FF000 >**如果使用 IP + PORT 的方式安装，推荐在升级时更换 PORT 参数指定不同的端口**</font>，规避可能会遇到因集群中端口释放延迟导致的 `provided port is already allocated` 问题。
- <font color=#FF000 >**尤其注意配置 ENCRYPTION_KEY，否则会导致升级后无法访问之前的数据**</font>。获取 ENCRYPTION_KEY 的两种方式如下：
...
2. 如果使用[`基于 Helm 命令安装`](/v1.7.0/install/helm-deploy/) 安装，执行以下步骤升级至 v1.8.0：
    ...
    ::: tip 注意事项
    <font color=#FF000 >**推荐使用和安装 v1.7.0/v1.7.1 时不同的端口来升级 v1.8.0**</font>，规避可能会遇到因集群中端口释放延迟导致的 `provided port is already allocated` 问题。
    :::
