...
helm upgrade --install zadig --namespace ${NAMESPACE} koderover-chart/zadig --version=1-nightly --set endpoint.FQDN=${DOMAIN} \
...
helm upgrade --install zadig --namespace ${NAMESPACE} koderover-chart/zadig --version=1-nightly --set endpoint.type=IP \
    --set endpoint.IP=${IP} \
...
> 后续可在访问系统 `系统设置` -> `对象存储` 配置，配置过程详见[对象存储](/dev/settings/object-storage/)
