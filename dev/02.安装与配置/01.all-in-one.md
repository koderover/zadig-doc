...
|正式版|官方下载源：[https://download.koderover.com/install?type=all-in-one](https://download.koderover.com/install?type=all-in-one) <br> GitHub 源：[https://github.com/koderover/zadig/releases/download/dev/all_in_one_install_quickstart.sh](https://github.com/koderover/zadig/releases/download/dev/all_in_one_install_quickstart.sh) | 适用于快速体验，不推荐生产使用 |

下载安装脚本并添加可执行权限。
```bash
# 下载地址和脚本名称根据实际情况进行替换
curl -LO https://github.com/koderover/zadig/releases/download/dev/all_in_one_install_quickstart.sh
...
curl -SsL https://github.com/koderover/zadig/releases/download/dev/uninstall.sh |bash
```
