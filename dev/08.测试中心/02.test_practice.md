...
下面介绍主流的测试框架/工具在 Zadig 中的使用实践，如何利用自动化测试能力最大程度的为软件交付保驾护航，以供参考。
...
根据实际测试业务和功能，编写自动化测试代码并组织在代码仓库中。本实践中将对 KodeRover 文档站的可用性进行自动化检测，自动化测试源码可点击[链接](https://github.com/koderover/zadig/tree/main/examples/test-demo/test)查看。
...
```
- 测试报告配置：按需配置 Junit xml 格式和 Junit 格式的测试报告。
