...
permalink: /ZadigX v1.4.0/install/helm-deploy/
---
...
Zadig Chart 除了 Zadig 系统外，还内置了一些必要的组件 MySQL、MongoDB、Minio，可以根据您的安装环境实际情况选择是否安装这些组件
...
::: tip
如果有自己的 S3 对象存储，可以选择不安装 Minio。在 Zadig 安装完后成访问 `系统设置` -> `对象存储` 来配置即可，配置过程详见[对象存储](/ZadigX%20v1.4.0/settings/object-storage/)。
:::
