...
包括[产品工作流](/ZadigX%20v1.4.0/project/workflow/)、[自定义工作流](/ZadigX%20v1.4.0/project/common-workflow/)和[发布工作流](/ZadigX%20v1.4.0/project/release-workflow/)。
...
产品工作流就是 ZadigX 平台对这样一个开发流程的实现，通过工作流来更新环境中的服务或者配置。简化示意图如下所示：
...
### 自定义工作流
可以自由编排工作流任务，自定义执行步骤，目前支持的任务类型包括：构建、部署、测试、项目管理、配置变更、数据变更、自定义任务。

### 发布工作流
可编排多种发布策略，包括蓝绿、金丝雀、分批次灰度、Istio 等。

## 环境

ZadigX 环境是一组服务集合及其配置、运行环境的总称，与 Kubernetes 的 NameSpace 是一对一的关系，使用一套服务模板可以创建多套环境。
...
ZadigX 代码扫描属于项目的资源，支持在自定义工作流中编排使用。
