...
permalink: /ZadigX v1.4.0/project/env/helm/chart/
---
...
- `资源选择`：
	- `K8s 集群`：选择创建环境时所使用的集群资源，其中`本地集群`指 ZadigX 系统所在的集群。关于集群的集成可参考[集群管理](/ZadigX%20v1.4.0/pages/cluster_manage/)。
...
- 环境创建成功后，此处新建的配置即会在环境/集群中生效，也可以在环境创建完成后再管理环境配置，参考[环境配置管理](/ZadigX%20v1.4.0/project/env/k8s/#环境配置管理)
...
- 从指定 GitHub/GitLab 代码库同步 values。
	- 在导入时打开自动同步开关，ZadigX 会定时从指定的代码库获取最新的 values 文件并将其自动更新到环境中。<Badge text="企业版" />
...
- `环境配置`：为环境配置 Ingress、ConfigMap、Secret、PVC 资源，细节描述请阅读[环境配置管理](/ZadigX%20v1.4.0/project/env/k8s/#环境配置管理)
...
- 可手动输入 values 内容
- 从代码库导入 values 文件
- 使用变量组，变量组管理参考[变量组配置](/ZadigX%20v1.4.0/project/config/#变量组) <Badge text="企业版"/>
...
可对环境中的 Ingress、ConfigMap、Secret、PVC 配置进行操作，参考[环境配置管理](/ZadigX%20v1.4.0/project/env/k8s/#环境配置管理)。

## 自测模式

参考[自测模式](/ZadigX%20v1.4.0/project/env/k8s/#自测模式)。
