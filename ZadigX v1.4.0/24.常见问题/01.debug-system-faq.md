...
permalink: /ZadigX v1.4.0/faq/debug-system/
---
...
ZadigX 系统对 Kubernetes 集群版本有要求。遇到此种错误请检查您的 Kubernetes 集群版本，需要集群版本在 1.16 及以上，才能正常安装使用。
...
1. 通过 [All in one](/ZadigX%20v1.4.0/install/all-in-one/) 或者 [Kubernetes 脚本](/ZadigX%20v1.4.0/install/install-on-k8s/)安装时，可以显式配置 ENCRYPTION_KEY 环境变量，或记录脚本自动生成的 ENCRYPTION_KEY，如下图所示：

![查看 ENCRYPTION_KEY](./_images/show_encryption_key.png)

2. 安装 ZadigX 成功后，可通过如下命令获取 ENCRYPTION_KEY 信息：

```bash
kubectl -n <ZadigX 安装的 Namespace> get secret zadig-aes-key -o=jsonpath='{.data.aesKey}' | base64 -d
```
