...
permalink: /ZadigX dev/faq/product-usage/
---
...
::: tip
ZadigX 提供两种方式，用户可自行选择，对于高级用户，推荐使用 kubectl 工具
:::
### 页面查询
...
因为 ZadigX 创建的 Ingress 的时候会自动添加 kubernetes.io/ingress.class 描述使用的 Ingress class，所以手动 kubectl apply 使用的 YAML 也需要加上这项。
...
2. 通过配置工作流 Webhook 可以实现代码 Push 到代码库以及提交 Pull Request(Merge Request) 时触发工作流的运行，实现从提交到构建自动化的流程，具体可以参考[代码变更触发工作流](/ZadigX%20dev/workflow/trigger/#代码变更触发)。
