...
[项目](/ZadigX%20dev/quick-start/concepts/#项目)是 ZadigX 中的一个核心概念，项目搭建完毕后，团队便可以在项目中进行代码部署、集成测试验证、服务上下线、版本发布等操作。
...
1. 在系统中集成多个镜像仓库，两个环境分别使用不同的镜像仓库，参考文档：[集成镜像仓库](/ZadigX%20dev/settings/image-registry/)、[环境的基本信息](/ZadigX%20dev/project/env/k8s/#基本信息-2)。
2. 对于产品工作流，在服务策略中修改镜像名称规则，使用 `ENV_NAME` 变量来区分不同环境的镜像信息，参考文档：[交付物命名规则设置](/ZadigX%20dev/project/service/k8s/#策略配置)。
3. 对于自定义工作流：
    - 在构建配置中增加自定义变量 `ENV_NAME`，在构建镜像之前修改 `IMAGE` 变量的值，让它和 `ENV_NAME` 联系起来，比如：`export IMAGE=$IMAGE-$ENV_NAME`，参考文档：[自定义变量](/ZadigX%20dev/project/build/#构建变量)。
    ![进阶使用](./_images/project_admin_demo_1.png)
    - 在工作流中增加全局变量，值设置为运行时输入，参考文档：[工作流变量](/ZadigX%20dev/project/common-workflow/#工作流)。
![进阶使用](./_images/project_admin_demo_2.png)
    - 修改构建任务的构建变量配置，使用上一步中的全局变量来为 `ENV_NAME` 赋值，参考文档：[构建任务变量](/ZadigX%20dev/project/common-workflow/#构建任务)。
![进阶使用](./_images/project_admin_demo_3.png)
...
2. [推荐]在一条自定义工作流里构建部署多个服务，在部署之前增加审批操作，通过后才会开始执行部署任务。审批配置可参考文档：[人工审批](/ZadigX%20dev/project/common-workflow/#人工审批)。