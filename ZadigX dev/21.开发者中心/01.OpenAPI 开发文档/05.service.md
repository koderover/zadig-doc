...
```
PUT /openapi/service/yaml/:serviceName/variable?projectKey=<project_name>
```
...
| `value`   | 变量的值，若有嵌套值，使用 json 格式               | any      | 是   |
| `type`    | 变量值的类型，有四种类型：bool、string、enum、yaml | string   | 是   |
| `options` | 变量可选值列表，当 `type` 为 `enum` 时该字段有意义 | []string | 是   |
| `desc`    | 变量描述信息                                       | String   | 是   |
...
```
PUT /openapi/service/yaml/production/:serviceName/variable?projectKey=<project_name>
```
