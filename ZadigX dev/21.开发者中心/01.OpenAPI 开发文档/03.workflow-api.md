...
permalink: /ZadigX dev/api/workflow/
---
...
```
GET /openapi/workflows/product/:workflowName/task/:taskID?projectName=<projectName>
```
...
```json
{
    ...
    "display_name": "patrick-yaml-01-ops-workflow",
    "project_name": "patrick-yaml-01",
    "task_id": 7,
...
|`project_name`|string|项目标识|无|must|
...
|`repo_info`|[[]RepositoryInfo](#repo_info)|代码库信息|无|optional|
...
|参数名|类型|描述|默认值|是否必须|
|---|---|---|---|---|
|`codehost_name`|string|代码源标识|无|must|
|`repo_namespace`|string|代码库所属命名空间（组织/个人）|无|must|
...
|参数名|类型|描述|默认值|是否必须|
|---|---|---|---|---|
|`key`|string|自定义变量的键|无|must|
|`value`|string|自定义变量的值|无|must|
...
```
DELETE /openapi/workflows/product?workflowName=<workflow_name>&projectName=<project_name>
```

#### query 参数说明
|参数名|类型|描述|默认值|是否必须|
|---|---|---|---|---|
|`workflowName`|string|工作流标识|无|是|
|`projectName`|string|项目标识|无|是|
...
```
GET /openapi/workflows/product/:workflowName/tasks?projectName=<projectName>&pageNum=<pageNum>&pageSize=<pageSize>
```
...
[
    {
        "workflow_name": "cosmos-build-1",
        "project_name": "cosmos-1",
        "task_id": 21,
...
```
GET /openapi/workflows?projectName=<projectName>&viewName=<viewName>
```
    ...
    "workflows": [
        {
            ...
            "display_name": "just-deploy",
            "update_by": "admin",
            "update_time": 1686217885,
            "type": "custom"
        },
        {
            "workflow_name": "xiaxianfuwu",
            "display_name": "下线服务",
            "update_by": "admin",
...
```
GET /openapi/workflows/custom/:workflowName/tasks?projectName=<projectName>
```
...
#### 请求
```
DELETE /openapi/workflows/custom/task?task_id=<task_id>&workflow_name=<workflow_name>
```
...
```
POST /openapi/workflows/custom/:workflowName/task/:taskID?projectName=<projectName>
```
...
{
    "task_id": 2,
    "workflow_name": "infra-dev-workflow",
    "stage_name": "deploy",
    "approve": true,
    "comment": "LGTM"
}
...
<!-- |`approve_id`|审批 ID|是|  TODO：待确认 LarkApproval 结构体中这个字段的意思和必要性-->
...
|`workflow_type`|工作流类型，有以下两种类型："product": 产品工作流; "custom": 自定义工作流|string|是|
...
``` json
{
    ...
    "workflow_list": [
        ...
        {
            "workflow_name": "nacos-config-update",
            "workflow_type": "custom"
        }
    ]
}
...
```
PUT /openapi/workflows/view/:viewName?projectName=<projectName>
```
...
| 参数名          | 说明                                                         | 类型   | 必填 | 默认值 |
| --------------- | ------------------------------------------------------------ | ------ | ---- | ------ |
| `workflow_name` | 工作流标识                                                   | string | 是   | 无     |
| `workflow_type` | 工作流类型，有以下两种类型："product": 产品工作流"; custom": 自定义工作流 | string | 是   | 无     |
| `enabled`       | true: 该工作流在该视图中可见; false: 该工作流在该视图中不可见 | bool   | 是   | 无     |
...
```
GET /openapi/workflows/view?projectName=<projectName>
```
...
| 参数名          | 说明                                                         | 类型   |
| --------------- | ------------------------------------------------------------ | ------ |
| `workflow_name` | 工作流标识                                                   | string |
| `workflow_type` | 工作流类型，有以下两种类型："product": 产品工作流; "custom": 自定义工作流 | string |
...
```
DELETE /openapi/workflows/view/:viewName?projectName=<projectName>
```
...
```
DELETE /openapi/workflows/custom?workflowName=<workflow_name>&projectName=<project_name>
```
