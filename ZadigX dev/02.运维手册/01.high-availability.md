...
## K8s 资源高可用
基础资源的高可用，需要由 Kubernetes 集群的提供者(公有云/自建云)保障。
...
- 备份当前安装参数以及 Chart：
  ...
  - 通过 ```helm pull koderover-chart/zadig-ee --version=<version>``` 备份当前 Chart

## 数据高可用
- 保证数据库实例本身高可用性，以阿里云为例：
    - MySQL 建议使用 RDS MySQL 高可用版
    - MongoDB 选用独占型实例
- 使用高可用的制品仓库：
  - 使用云厂商提供的高可用镜像仓库 / Helm 仓库
  - 若有配置 Helm 项目，需要集成高可用的对象存储并设为默认使用，备份建议：一天一次
- 定期备份数据库数据
    ...
    },
    {
      ...
      "options": {
        ...
        "sortBy": [
          {
            "desc": true,
            "displayName": "{handler=\"/api/cluster/bundle-resources\", method=\"GET\"}"
          }
        ]
      },
      "pluginVersion": "9.4.7",
      "targets": [
        {
          "editorMode": "code",
          "exemplar": false,
          "expr": "topk(10, sum by(method, handler) (api_response_time_bucket{status=\"200\",le=\"+Inf\"}-api_response_time_bucket{status=\"200\",le=\"1.4\"}))",
          "format": "time_series",
          "legendFormat": "__auto",
          "range": true,
          "refId": "A"
...
## 多副本策略
Zadig 中部分组件支持多副本策略，若有必要可以按需调整实际副本数。支持多副本配置的组件如下：
