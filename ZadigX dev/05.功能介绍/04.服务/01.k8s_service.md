...
permalink: /ZadigX dev/project/service/k8s/
---
...
2. 目前支持从 [GitLab](/ZadigX%20dev/settings/codehost/gitlab/)/[GitHub](/ZadigX%20dev/settings/codehost/github/)/[Gerrit](/ZadigX%20dev/settings/codehost/gerrit/)/[Gitee](/ZadigX%20dev/settings/codehost/gitee/) 代码源同步服务
:::
...
可参考[服务组件](/ZadigX%20dev/quick-start/concepts/#服务组件)了解更多信息，此处相关字段说明如下：
...
- `构建信息/操作`：可为服务组件配置构建操作，定义其构建过程，参考：[构建配置](/ZadigX%20dev/project/build)
...
- 交付物命名规则设置：自定义该项目中工作流构建产物的命名规则，可通过以下[内置变量](#内置变量)和常量组合的方式设置，对当前项目的所有服务都生效。其中 `JENKINS 生成镜像规则`适用于使用 [Jenkins 构建](/ZadigX%20dev/project/build/#jenkins-构建) 的服务，不可使用下述[内置变量](#内置变量)中的 `REPO_*` 变量。
