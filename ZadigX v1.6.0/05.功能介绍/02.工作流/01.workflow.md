...
permalink: /ZadigX v1.6.0/project/workflow/
---
...
关于镜像版本回退，更多信息可阅读：[工作流的镜像版本回退](/ZadigX%20v1.6.0/workflow/image/rollback/)。
...
> 服务构建配置细节可参阅[构建配置](/ZadigX%20v1.6.0/project/build/)

![workflow](../_images/workflow_build.png)

字段说明：

- `服务`：内容的格式为`服务名/服务组件名`，点击服务可跳转查看该服务的配置详情。关于服务组件的概念可阅读[服务组件](/ZadigX%20v1.6.0/project/service/module/)。
...
通过修改工作流的测试配置，来实现测试步骤，关于测试配置的创建和使用，请参阅[测试管理](/ZadigX%20v1.6.0/project/test/)。
...
- 分发方式：包括镜像分发和对象存储分发
- 对象存储：构建的文件产物（一般为压缩包）被分发的目标对象存储，可在系统设置的 [对象存储](/ZadigX%20v1.6.0/settings/object-storage) 中查阅
- 镜像仓库：构建的镜像产物被分发的目标镜像仓库，可在系统设置的 [镜像仓库](/ZadigX%20v1.6.0/settings/image-registry/) 中查阅
...
参数说明：
- URL：包括两部分，集成的[外部系统](/ZadigX%20v1.6.0/settings/others/)，以及自定义的 Path 部分。
...
> 支持的代码源请参考文档：[代码源信息](/ZadigX%20v1.6.0/settings/codehost/overview/#功能兼容列表)。

支持以下三种方式配置触发器：

- 自动创建：在 ZadigX 中配置触发器参数后即可，会自动在对应代码库中创建 Webhook。使用前提：系统集成代码源中的账号对代码库有创建 Webhook 权限。
- 手动创建：先在代码库中创建 Webhook，再在 ZadigX 中配置触发器参数。
- YAML 方式：Webhook 触发工作流运行相关参数组织在代码库的 YAML 文件中，在 ZadigX 中配置 YAML 文件路径即可。
...
第一步：编辑工作流，新增触发器模块，打开 Webhook 开关，添加触发器，切换到手动创建页面。

<img src="../_images/product_workflow_webhook_1.png" width="400">
<img src="../_images/product_workflow_webhook_2.png" width="400">

第二步：配置代码库中的 Webhook，填写 Webhook URL、Secret Token、触发事件后保存。以 GitLab 示例如下：
...
第三步：参考[自动创建](#自动创建)中的参数说明，填写 ZadigX 中的相关配置后保存即可。
...
支持将工作流执行情况通知到钉钉、企业微信、飞书，具体如何使用可参考文档：[工作流通知](/ZadigX%20v1.6.0/project/workflow-im/)。

## 运行工作流

工作流支持多种构建方式，用户可以根据需求灵活配置。工作流模块支持通过自动构建（定时器/ Webhook）的方式实现对代码进行持续集成和持续部署，进行快速验证和快速反馈，保证代码的质量。
具体操作可以参阅[工作流的触发](/ZadigX%20v1.6.0/workflow/trigger/)部分获取更多的信息。
