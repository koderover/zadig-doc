...
|任务名称|描述|具体配置|
|---|---|----|
|构建部署|拉取代码，执行代码构建和镜像推送，部署到指定环境|[查看](/Zadig%20v2.2.0/project/workflow/#构建部署)|
|交付物部署|将已有的镜像/二进制包部署到指定环境|[查看](/Zadig%20v2.2.0/project/workflow/#交付物部署)|
|测试|执行自动化测试，对部署结果进行验证|[查看](/Zadig%20v2.2.0/project/workflow/#测试)|
|分发部署|将构建产物分发到对象存储/镜像仓库，并将对应镜像部署到指定环境|[查看](/Zadig%20v2.2.0/project/workflow/#分发部署)|
|扩展|触发第三方系统，并等待回调|[查看](/Zadig%20v2.2.0/project/workflow/#扩展)|
...
- 任务：一个独立的、完整的操作，比如构建、部署、测试、自定义任务...多个任务可串行执行或并发执行。任务分为两种类型：
    - 自定义任务：通过开发自定义任务实现和第三方系统的交互，具体开发方式参考：[自定义工作流任务](/Zadig%20v2.2.0/settings/custom-task/)
...
|  | 触发 Zadig 工作流 | 可触发其他 Zadig 自定义工作流 | [查看](/Zadig%20v2.2.0/project/workflow-jobs/#触发-zadig-工作流) |
