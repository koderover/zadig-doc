...
- 步骤 1：集成代码源。具体过程参考 [GitLab 代码源集成](/Zadig%20v2.2.0/settings/codehost/gitlab/)
...
2. 在 Zadig 中镜像命名规则支持统一配置和管理，具体可参考[文档](/Zadig%20v2.2.0/project/service/k8s/#策略配置)，因此无需在脚本中定义 IMAGE 变量的生成规则。
...
除了上述两种触发器，Zadig 还支持多种其他触发器，包括「JIRA 触发器」、「飞书项目触发器」和「通用触发器」等，使用详情参考[文档](/Zadig%20v2.2.0/project/workflow-trigger/)。
...
除以上能力外，Jenkins 通过插件来扩展更多的能力，而 Zadig 可以通过开发「自定义任务」，和企业自建系统打通，来满足企业复杂流程，具体开发过程参考[自定义工作流任务](/Zadig%20v2.2.0/settings/custom-task/)。
