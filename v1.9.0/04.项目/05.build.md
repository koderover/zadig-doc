...
permalink: /v1.9.0/project/build/
---
...
> 如果没有集成 Jenkins，新建构建时默认创建的是 Zadig 构建；如果需要使用 Jenkins 构建，请先[集成 Jenkins](/v1.9.0/settings/jenkins/)。
...
- 整个构建模块包括通用配置、运行时环境、代码库信息、环境变量、构建脚本、镜像构建、二进制包归档和 Shell 脚本，更多细节请阅读[定义构建](#定义构建)。
...
1. 支持使用自定义镜像作为构建系统，详情请参阅[构建镜像管理](/v1.9.0/settings/custom-image/#步骤-1-生成构建镜像)。
2. 默认提供的 Ubuntu 系列构建系统中已安装了以下工具/命令，可以在[应用安装脚本](/v1.9.0/settings/app/)和构建脚本中直接使用。
...
- 平台已内置一些常见的应用，如果有其他应用或者版本需求，系统管理员可以在[应用管理](/v1.9.0/settings/app)中配置应用的安装脚本。
:::

- `高级配置`：
    - `集群名称`：选择构建任务运行时所使用的集群，其中`本地集群`指 Zadig 系统所在的集群，关于集群的集成可参考[集群管理](/v1.9.0/pages/cluster_manage/)。
...
- `$WORKSPACE` 是 Zadig 系统构建模块内置的变量，关于其更详细的描述可阅：[系统内置环境变量](/v1.9.0/project/build/#构建脚本)
::::
...
![运行工作流时修改环境变量](./_images/update_env_when_run_pipeline.png '运行工作流时修改环境变量')
...
2. 系统有内置的规则生成 `IMAGE` 和 `PKG_FILE`，关于规则的查看和修改，请参考：[构建交付物生成规则](/v1.9.0/project/build/#构建交付物生成规则)。
:::
...
- `Dockerfile 来源`: 包括`代码库`和`模板库`，模板库功能可参考 [Dockerfile 模板管理](/v1.9.0/template/dockerfile/)。
...
::: tip 小贴士
- `自定义镜像`对应[构建脚本](/v1.9.0/project/build/#构建脚本)中的 `IMAGE` 变量
- `自定义 TAR 包`对应[构建脚本](/v1.9.0/project/build/#构建脚本)中的 `PKG_FILE` 变量
:::
...
### 前提
1. 需要先在 Zadig 系统中集成 Jenkins 服务器，详见 [Jenkins 集成](/v1.9.0/settings/jenkins)。
2. 需要先在 Zadig 系统中集成镜像仓库，详见[镜像仓库管理](/v1.9.0/settings/image-registry/)。
