...
permalink: /v1.7.1/install/all-in-one-nightly/
---
::: tip
- Nightly 版本基于最新代码每日定时构建，适合快速尝鲜体验 Zadig 产品最新能力，不推荐生产使用。
...
```bash
NOTE: 此处需要修改脚本
curl -LO https://github.com/koderover/zadig/releases/latest/download/all_in_one_install_nightly.sh
chmod +x ./all_in_one_install_nightly.sh
```

若网络状况欠佳，也可使用官方脚本安装: 
```bash
NOTE: 此处需要修改脚本
curl -L https://download.koderover.com/install?type=all-in-one -o all_in_one_install_nightly.sh
chmod +x ./all_in_one_install_nightly.sh
```
