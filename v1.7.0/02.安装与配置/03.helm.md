...
```bash
helm upgrade --install zadig --namespace <NAMESPACE> koderover-chart/zadig --version=1.7.0 --set endpoint.FQDN=<your domain> \
...
```bash
helm upgrade --install zadig --namespace <NAMESPACE> koderover-chart/zadig --version=1.7.0 --set endpoint.type=IP \
    --set endpoint.IP=<IP> \
...
```
helm list -n zadig # 获得 Zadig 的 relase name
