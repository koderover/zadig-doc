...
> 请勿在环境变量名称中使用 bash 关键字（大小写均不可使用），否则系统将无法解析变量的值，比如 `dockerfile_path`、`pwdrepo`、`CD_ZADIG` 均无法正确解析。
...
### 前提
1. 需要先在 Zadig 系统中集成 Jenkins 服务器，详见 [Jenkins 集成](/v1.7.0/settings/jenkins)。
2. 需要先在 Zadig 系统中集成镜像仓库，详见[镜像仓库管理](/v1.7.0/settings/image-registry/)。
