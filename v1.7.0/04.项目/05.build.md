...
这些工具可在[应用安装脚本](/v1.7.0/settings/app/)和构建脚本中直接使用。
...
平台已内置一些常见的应用，如果有其他应用或者版本需求，系统管理员可以在[应用管理](/v1.7.0/settings/app)中配置应用的安装脚本。
:::
...
- `$WORKSPACE` 是 Zadig 系统构建模块内置的变量，关于其更详细的描述可阅：[系统内置环境变量](/v1.7.0/project/build/#构建脚本)
::::
...
- Jenkins Job 成功构建出 `IMAGE` 并推送至镜像仓库后，Zadig 会使用 `IMAGE` 部署更新服务，请确保使用的镜像仓库集成无误，镜像仓库集成可参考：[镜像仓库管理](/v1.7.0/settings/image-registry/)。
:::