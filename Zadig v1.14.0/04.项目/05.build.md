...
permalink: /Zadig v1.14.0/project/build/
---

Zadig 构建是基于容器运行时环境提供的高效云原生构建引擎，内置了常见的构建环境及软件包、缓存机制，支持多个服务共享构建脚本、共享全局构建模板及 Jenkins 构建的调用，还可以通过自定义[构建镜像](/Zadig%20v1.14.0/settings/custom-image) 及 [安装软件包](/Zadig%20v1.14.0/settings/app/) 满足更为丰富的构建场景。
...
- 支持为一个服务配置多个构建，在工作流配置中按需选择。
    - 产品工作流：[构建部署配置](/Zadig%20v1.14.0/project/workflow/#构建部署)。
    - 自定义工作流：[构建任务配置](/Zadig%20v1.14.0/project/workflow-jobs/#构建任务)。
- 支持多个服务使用一份构建配置，更多细节请阅读[共享构建](#zadig-共享构建)。
- 支持修改构建交付物的生成规则，更多细节请阅读[构建交付物生成规则](/Zadig%20v1.14.0/project/service/k8s/#策略配置)。
...
1. 支持自定义添加构建环境作为构建系统，详情请参阅[构建镜像管理](/Zadig%20v1.14.0/settings/custom-image/#步骤-1-生成构建镜像)。
2. 默认提供的 Ubuntu 系列构建系统中已安装了以下工具/命令，可以在[软件包管理](/Zadig%20v1.14.0/settings/app/)和构建脚本中直接使用。
...
- 平台已内置一些常见的软件包，如果有其他软件包或者版本需求，系统管理员可以在[软件包管理](/Zadig%20v1.14.0/settings/app)中配置其安装脚本。
:::
...
- `$WORKSPACE` 是 Zadig 系统构建模块内置的变量，关于其更详细的描述可阅：[系统内置环境变量](/Zadig%20v1.14.0/project/build/#构建脚本)
::::
...
|`IMAGE`|系统按照内置规则生成的镜像名称，用于容器服务的部署更新，可以在服务的[策略配置](/Zadig%20v1.14.0/project/service/k8s/#策略配置)中修改其命名规则|
|`PKG_FILE`|系统按照内置规则生成的交付物文件名称，可修改其命名规则，可以在服务的[策略配置](/Zadig%20v1.14.0/project/service/k8s/#策略配置)中修改其命名规则|
|`SERVICE`|编译的服务名称|
|`ENV_NAME`|环境名称|
|`BUILD_URL`|构建任务的 URL|
...
- `资源配置`：选择构建任务运行时所使用的集群资源，其中本地集群指 Zadig 系统所在的集群，关于集群的集成可参考[集群管理](/Zadig%20v1.14.0/pages/cluster_manage/)。
...
- `Dockerfile 来源`: 包括`代码库`和`模板库`，模板库功能可参考 [Dockerfile 模板管理](/Zadig%20v1.14.0/template/dockerfile/)。
...
1. 创建构建模板，在模板中完成代码库信息之外的所有配置，可参考[新建构建模板](/Zadig%20v1.14.0/template/build/#新建模板)。
...
### 前提
1. 需要先在 Zadig 系统中集成 Jenkins 服务器，详见 [Jenkins 集成](/Zadig%20v1.14.0/settings/jenkins)。
2. 需要先在 Zadig 系统中集成镜像仓库，详见[镜像仓库管理](/Zadig%20v1.14.0/settings/image-registry/)。
...
- `构建参数`：会自动解析出对应 Jenkins Job 中的参数项，可在 Zadig 平台中做修改。说明如下：
    - `IMAGE` 参数：即为[前提](#前提)中的 `IMAGE` 变量，运行工作流时会传递给对应的 Jenkins Job 作为构建镜像产物名称。支持使用系统内置变量来为其赋值，参考[策略配置](/Zadig%20v1.14.0/project/service/k8s/#策略配置)。
