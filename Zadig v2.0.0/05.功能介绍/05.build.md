...
参数说明：
- `代码源`：代码库所在的平台，支持的代码源请参考 [代码源信息](/Zadig%20v2.0.0/settings/codehost/overview/#功能兼容列表)，支持使用系统和项目两个范围的资源
    - `项目资源`：项目内集成的代码源，如何集成可参考文档 [代码源集成](/Zadig%20v2.0.0/project/overview/)
...
- `$WORKSPACE` 是 ZadigX 系统构建模块内置的变量，关于其更详细的描述可阅：[系统内置环境变量](/Zadig%20v2.0.0/project/build/#构建脚本)
::::
...
- `集群选择`：选择构建任务运行时所使用的集群资源，其中本地集群指 ZadigX 系统所在的集群，关于集群的集成可参考[集群管理](/Zadig%20v2.0.0/pages/cluster_manage/)。
...
1. 创建构建模板，在模板中完成代码库信息之外的所有配置，可参考[新建构建模板](/Zadig%20v2.0.0/template/build/#新建模板)。
2. 创建 ZadigX 构建，选择`使用模板` -> 填写构建名称并选择构建模板 -> 选择服务并按需配置代码信息后保存即可。

> 代码信息中的相关参数说明可参考：[代码信息说明](/Zadig%20v2.0.0/project/build/#代码信息)。
...
- 确保代码源在 ZadigX 中已经成功集成，集成方式参考文档[代码源集成](/Zadig%20v2.0.0/settings/codehost/overview)
- 一行一条数据
...
### 前提
1. 需要先在 ZadigX 系统中集成 Jenkins 服务器，详见 [Jenkins 集成](/Zadig%20v2.0.0/settings/jenkins)。
2. 需要先在 ZadigX 系统中集成镜像仓库，详见[镜像仓库管理](/Zadig%20v2.0.0/settings/image-registry/)。
...
- `构建参数`：会自动解析出对应 Jenkins Job 中的参数项，可在 ZadigX 平台中做修改。说明如下：
    - `IMAGE` 参数：即为[前提](#前提)中的 `IMAGE` 变量，运行工作流时会传递给对应的 Jenkins Job 作为构建镜像产物名称。支持使用系统内置变量来为其赋值，参考[策略配置](/Zadig%20v2.0.0/project/service/k8s/#策略配置)。
