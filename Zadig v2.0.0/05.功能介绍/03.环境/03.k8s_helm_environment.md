...
- 其他介绍可参考 K8s YAML 测试环境中的描述：[查看环境](/Zadig%20v2.0.0/project/env/k8s/#查看环境)
...
- `资源选择`：
	- `K8s 集群`：选择创建环境时所使用的集群资源，其中`本地集群`指 ZadigX 系统所在的集群。关于集群的集成可参考[集群管理](/Zadig%20v2.0.0/pages/cluster_manage/)。
...
- 支持直接粘贴或从代码库导入配置，支持的代码源请参考文档：[代码源信息](/Zadig%20v2.0.0/settings/codehost/overview/#功能兼容列表)
- 环境创建成功后，此处新建的配置即会在环境/集群中生效，也可以在环境创建完成后再管理环境配置，参考[环境配置管理](/Zadig%20v2.0.0/project/env/k8s/#环境配置管理)
...
- 从指定代码库导入 values，支持的代码源请参考文档：[代码源信息](/Zadig%20v2.0.0/settings/codehost/overview/#功能兼容列表)。
...
参考 K8s YAML 测试环境中的描述：[复制环境](/Zadig%20v2.0.0/project/env/k8s/#复制环境)。

## 更新全局变量
点击基本操作中的`更新全局变量`，可修改全局服务变量，修改后对当前环境下的所有服务都生效，支持以下几种方式：

- 手动输入 values 内容
- 从代码库导入 values 文件，支持的代码库请参考：[代码源信息](/Zadig%20v2.0.0/settings/codehost/overview/#功能兼容列表)
- 使用变量组，变量组管理参考[变量组配置](/Zadig%20v2.0.0/project/permission/#变量组) 
...
为环境配置 Ingress、ConfigMap、Secret、PVC 资源，细节描述请阅读[环境配置管理](/Zadig%20v2.0.0/project/env/k8s/#环境配置管理)。

## 自测模式

参考文档：[自测模式](/Zadig%20v2.0.0/project/env/k8s/#自测模式)。
