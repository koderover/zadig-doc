...
本文档主要介绍企业如何试用 Zadig，其中包括资源准备及详细的安装步骤。
...
使用[官方安装小助手](https://www.koderover.com/trial)一键快速安装。详细参数说明参考 [Chart 参数列表](#zadig-chart-参数列表)
...
试用版本不建议升级, 若需要升级试用版 Zadig，请参考以下操作：
...
helm repo update
helm upgrade {releaseName} -n {Namespace} --version={Version} koderover-chart/zadig -f zadig.yaml
```
...
::: tip
如果有自己的 S3 对象存储，可以选择不安装 MinIO。在 Zadig 安装完后成访问 `系统设置` -> `对象存储` 来配置即可，配置过程详见[对象存储](/Zadig%20v2.0.0/settings/object-storage/)。
:::
