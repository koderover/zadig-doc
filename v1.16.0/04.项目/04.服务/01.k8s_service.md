...
permalink: /v1.16.0/project/service/k8s/
---
...
::: tip
1. 需要在系统中集成代码源，可参考：[代码源集成](/v1.16.0/settings/codehost/overview/)
2. 目前支持从 [GitLab](/v1.16.0/settings/codehost/gitlab/)/[GitHub](/v1.16.0/settings/codehost/github/)/[Gerrit](/v1.16.0/settings/codehost/gerrit/)/[Gitee](/v1.16.0/settings/codehost/gitee/) 代码源同步服务
:::
...
::: tip 前提
需要先在系统模板库里创建 K8s YAML 模板，请参考 [K8s YAML 模板管理](/v1.16.0/template/k8s_yaml/)。
:::
...
可参考[服务组件](/v1.16.0/quick-start/concepts/#服务组件)了解更多信息，此处相关字段说明如下：
...
- `构建信息/操作`：可为服务组件配置构建操作，定义其构建过程，参考：[构建配置](/v1.16.0/project/build)
...
::: tip 注意
  ...
  `例如`：全局变量配置变量 k1=global，服务A 变量配置 k1=s1，服务B 没有配置 k1 变量，则最终服务A k1=s1，服务B k1=global；
:::
...
- 交付物命名规则设置：自定义该项目中工作流构建产物的命名规则，可通过以下[内置变量](#内置变量)和常量组合的方式设置，对当前项目的所有服务都生效。其中 `JENKINS 生成镜像规则`适用于使用 [Jenkins 构建](/v1.16.0/project/build/#jenkins-构建) 的服务，不可使用下述[内置变量](#内置变量)中的 `REPO_*` 变量。
