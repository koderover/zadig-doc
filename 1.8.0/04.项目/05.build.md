...
平台提供的操作系统中都安装了以下工具/命令，可以在[应用安装脚本](/v1.8.0/settings/app/)和构建脚本中直接使用。
...
> Zadig 构建环境支持自定义镜像，详情请参阅[构建镜像管理](/v1.8.0/settings/custom-image/#步骤-1-生成构建镜像)
...
:::: tip 小贴士
`$WORKSPACE` 是 Zadig 系统构建模块内置的环境变量，关于其更详细的描述可阅：[系统内置环境变量](/v1.8.0/project/build/#构建脚本)
::::
