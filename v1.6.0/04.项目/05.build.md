...
这些工具可在[应用安装脚本](/v1.6.0/settings/app/)和构建脚本中直接使用。
...
- Jenkins Job 成功构建出 `IMAGE` 并推送至镜像仓库后，Zadig 会使用 `IMAGE` 部署更新服务，请确保使用的镜像仓库集成无误，镜像仓库集成可参考：[镜像仓库管理](/v1.6.0/settings/image-registry/)。
:::
