...
permalink: /v1.6.0/install/helm-deploy/
---
...
```bash
helm upgrade --install zadig --namespace zadig  koderover-chart/zadig --verison=1.6.0 --set endpoint.FQDN=<your domain>
```
### IP + PORT 访问
...
```bash
helm upgrade --install zadig --namespace zadig  koderover-chart/zadig --verison=1.6.0 --set endpoint.type=IP \
    --set endpoint.IP=<IP> \
