...
permalink: /v1.6.0/v1.6.0/release-notes/v1.3.1/
---
...
1. 如果 v1.3.0 使用[`All in One 一键安装模式`](/install/all-in-one/) 和 [`基于现有 Kubernetes 安装`](/install/install-on-k8s/) 安装，均使用 [`基于现有 Kubernetes 安装`](/install/install-on-k8s/) 中的脚本进行升级，执行升级脚本之前需重新 export 参数。

2. 如果 v1.3.0 使用[`基于 Helm 命令安装`](/install/helm-deploy/) 安装，执行以下步骤升级 v1.3.1：
...
1. v1.2.0 -> v1.3.0，具体升级过程查看[文档](/release-notes/v1.3.0/)
