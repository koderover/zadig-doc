...
> 可参阅 [主机服务](/Zadig%20v1.16.0/project/service/vm/)，完成相关配置项。
...
包括服务名基本信息及服务构建配置，构建配置更细节的信息可参考[构建配置](/Zadig%20v1.16.0/project/build/)。
...
为服务选择将部署的主机资源（项目初始化向导结束后，系统将会自动创建包含该服务的 `dev` 和 `qa` 环境），并配置部署方式，填写部署脚本，参考：[部署配置](/Zadig%20v1.16.0/project/service/vm/#部署配置)。

::: tip
需要先集成主机资源，可参考：[主机管理](/Zadig%20v1.16.0/settings/vm-management/)。
:::
...
### 探活配置
为 `backend` 服务设置探活检测。
> 更详细的信息可参考 [探活配置](/Zadig%20v1.16.0/project/service/vm/#探活配置)。
