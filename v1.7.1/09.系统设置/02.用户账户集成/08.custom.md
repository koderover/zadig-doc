...
|接口名|使用说明|目前可参考已有实现|
|-----|-----|-----|
|PasswordConnector|1. 简单使用用户名密码方式登录授权获取用户信息的。<br>2. Dex 和账号系统的交互是同步的|keystone、atlassiancrowed、ldap、mock/passwordConnector、passwordDB(位于dex/server/server.go)|
|CallbackConnector|1. 通过重定向获取用户信息的<br>2. Dex 和账号系统的交互是异步的|mock/Callback、bitbucketcloud、authproxy、gitea、github、gitlab、google、linkedin、microsoft、oauth、oidc、openshift|
