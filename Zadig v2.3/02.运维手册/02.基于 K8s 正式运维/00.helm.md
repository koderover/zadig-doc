...
permalink: /Zadig v2.3/install/helm-deploy/
---
...
### 步骤 2：备份数据以及安装参数
- 参考文档：[备份数据](/Zadig%20v2.3/stable/backup-and-restore/#数据备份)进行数据备份。
...
::: tip
如果有自己的 S3 对象存储，可以选择不安装 MinIO。在 Zadig 安装完后成访问 `系统设置` -> `对象存储` 来配置即可，配置过程详见[对象存储](/Zadig%20v2.3/settings/object-storage/)。
:::
