...
::: tip 前提
需要先在系统模板库里创建 K8s YAML 模板，请参考 [K8s YAML 模板管理](/Zadig%20v2.3/template/k8s_yaml/)。
:::
...
参考文档：[服务组件](/Zadig%20v2.3/env/overview/#什么是服务组件)。
...
参考文档：[服务变量](/Zadig%20v2.3/project/service/variable/)。
...
- 交付物命名规则设置：自定义该项目中工作流构建产物的命名规则，可通过以下[内置变量](#内置变量)和常量组合的方式设置，注意：
  ...
  <!-- - `JENKINS 生成镜像规则`适用于使用 [Jenkins 构建](/Zadig%20v2.3/project/build/#jenkins-构建) 的服务，不可使用下述[内置变量](#内置变量)中的 `REPO_*` 变量。 -->
