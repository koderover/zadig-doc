...
### 任务
点击阶段下方的 `+任务` 为阶段增加任务配置，系统目前支持`构建`、`部署`、`通用任务`、`Kubernetes 部署`、`测试`、`自定义任务` 等任务类型。点击[工作流任务](/Zadig%20v1.17.0/project/workflow-jobs/)可查看目前 自定义工作流 和 发布工作流 所支持的任务类型。
...
参数说明：
- `审批应用`：选择使用的飞书审批应用，审批应用集成方式参考 [审批系统集成](/Zadig%20v1.17.0/settings/approval/)
...
- `类型`：支持字符串/多行文本/枚举/代码库类型
  - 字符串类型的值为单行文本
  - 在系统设置中集成代码源后，才能配置代码库类型变量的值，代码源集成请参考[代码源集成](/Zadig%20v1.17.0/settings/codehost/overview/)
...
变量的键和类型在此处不可更改，变量定义请阅读 [构建配置](/Zadig%20v1.17.0/project/build/#构建变量) 中的自定义构建变量部分。支持以下三种方式设置变量的值：

- `运行时输入`：执行工作流时再指定该变量的值
- `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量`：使用全局变量为构建任务中的变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.17.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.17.0/project/common-workflow/#工作流)
...
- `全局变量`：使用全局变量为环境赋值，工作流执行时会自动对全局变量进行渲染替换。全局变量包括 [项目变量](/Zadig%20v1.17.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.17.0/project/common-workflow/#工作流)
...
- `全局变量`：使用全局变量为测试任务中的变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.17.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.17.0/project/common-workflow/#工作流)
...
内置变量请参考 [构建变量](/Zadig%20v1.17.0/project/build/#构建变量) 中关于内置构建变量的描述，自定义变量说明如下：
...
- `值`：自定义变量的值，支持以下三种配置方式：
  - `运行时输入`：执行工作流时再指定该变量的值
  - `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
  - `全局变量`：使用全局变量为通用任务中的自定义变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.17.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.17.0/project/common-workflow/#工作流)
...
- `运行时输入`：执行工作流时再指定该变量的值
- `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量/其他任务输出`：使用全局变量或前置任务输出的变量来为此处的变量赋值，全局变量包括 [项目变量](/Zadig%20v1.17.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.17.0/project/common-workflow/#工作流)
...
在自定义任务的 YAML 文件中定义，YAML 定义请参考 [工作流任务](/Zadig%20v1.17.0/settings/custom-task/)，变量值的配置支持以下三种方式：
- `运行时输入`：执行工作流时再指定该变量的值
- `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量`：使用全局变量为通用任务中的自定义变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.17.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.17.0/project/common-workflow/#工作流)
...
- 其他参数：请参考 [触发器基本配置](/Zadig%20v1.17.0/project/workflow/#gui-方式) 中的描述
...
参数说明：
- `工作流执行变量`：希望被 Webhook 触发执行的任务，以及任务中的具体配置
- 其他参数：请参考 [触发器基本配置](/Zadig%20v1.17.0/project/workflow/#gui-方式) 中的描述在第三方系统中完成 Webhook 配置。
...
触发方式以及时间配置可参考 [定时器配置](/Zadig%20v1.17.0/project/workflow/#定时器)，在工作流执行变量中可设置定时运行的任务以及任务中的变量配置。
...
目前支持配置工作流通知到飞书/企业微信/钉钉中，详细的配置参数说明可参考：[IM 状态通知](/Zadig%20v1.17.0/project/workflow/#im-状态通知)。
