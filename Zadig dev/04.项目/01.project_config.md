...
permalink: /Zadig dev/project/config/
---
...
- `访问权限`：可选`公开`或`私有`，定义该项目的默认权限。
    - 公开项目：用户默认享有 `read-only` 角色中的权限，可查看项目中的资源。该角色的细节请阅[权限配置](/Zadig%20dev/project/config/#权限配置)。
...
- 角色：一系列权限的集合，在[角色管理](/Zadig%20dev/project/config/#角色管理)中可查看角色中的权限细节。
- 策略：工作流/环境资源粒度的权限集合，对私有项目适用。在[策略管理](/Zadig%20dev/project/config/#策略管理)中可查看该策略的权资源和限细节。
...
陈述各个权限项和功能的对应关系，各功能模块中完整的功能文档分别参见：[工作流](/Zadig%20dev/project/workflow/)/[环境](/Zadig%20dev/project/env/k8s)/[服务](/Zadig%20dev/project/service/k8s/)/[构建](/Zadig%20dev/project/build/)/[测试](/Zadig%20dev/project/test/)/[版本管理](/Zadig%20dev/project/version/)/[代码扫描](/Zadig%20dev/project/scan/)。
...
- 选择参与成员，若需要添加的用户不在成员列表中，可在[成员管理](/Zadig%20dev/project/config/#成员管理)中添加，并选择 `read-project-only` 角色。
...
主机项目中，项目管理员点击`配置` -> `主机管理`可在项目中对主机资源进行管理，具体配置可参考 [添加主机](/Zadig%20dev/settings/vm-management/#添加主机)。
...
创建后，Helm Chart 项目中环境的全局变量可以使用变量组，详情参考[更新全局变量](/Zadig%20dev/project/env/helm/chart/#更新全局变量)
