...
permalink: /Zadig dev/release-notes/v1.18.0/
---
...
1. 需要正确配置服务变量的可见性，环境中使用的变量需要显示声明出来，变量可见性配置可参考文档：[变量设置](/Zadig%20dev/project/service/k8s/#自定义变量)。
2. 如果有使用嵌套变量，请确保多个变量的可见性保持一致，比如下例中，`ports_config[].protocol` 和 `ports_config[].container_port` 的可见性需要一致。
...
> 对于已下线的软件包，如果有使用需要，请参考文档 [软件包管理](/Zadig%20dev/settings/app/) 进行配置。
...
## 版本升级过程
::: warning
如果当前系统版本 < v1.17.0，请先升级到 v1.17.0，具体升级过程参见 [v1.17.0 升级方式](/Zadig%20dev/release-notes/v1.17.0/#版本升级过程)，再按照以下方式升级到 v1.18.0
:::
