...
在系统设置 -> 集成管理 -> 系统集成 -> 账号系统集成中可查看集成账号系统的配置，参考 [LDAP](/Zadig%20dev/settings/account/ldap/)/[OAuth](/Zadig%20dev/settings/account/oauth2/)/[GitHub](/Zadig%20dev/settings/account/github/)/[自定义](/Zadig%20dev/settings/account/custom/)账号系统对检查有误的信息进行修改。
...
通常原因为 OAuth 配置错误，可以查阅 [OAuth 配置](/Zadig%20dev/settings/account/oauth2/) 进行检查。以下图中的错误配置为例示范：
