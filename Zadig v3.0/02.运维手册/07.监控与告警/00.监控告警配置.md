...
permalink: /Zadig v3.0/stable/monitoring-alter-config/
---
...
3. 或者可以添加ServiceMonitor到Prometheus的namespace中，如下所示：

> 要求Prometheus是通过Kubernetes安装，并且安装了相关CRD
...
spec:
  ...
  - port: svc
    path: /api/metrics
```
4. 在 Prometheus 的 query 界面，输入 request_total 进行查询，确认有数据，证明配置成功
