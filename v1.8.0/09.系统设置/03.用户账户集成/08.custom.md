...
permalink: /v1.8.0/settings/account/custom/
---

目前 Zadig 用户系统采用了开源项目 [Dex](https://github.com/dexidp/dex) 作为身份连接器，但 Dex 支持的协议列表（[Dex 支持的协议](https://dexidp.io/docs/connectors/)）有限，Zadig 则基于 Dex 官方库实现了一些标准的扩展，如 OAuth 协议、Portal 接入层等。如果用户自身的账号系统在 Zadig 官方支持之外，可以通过 Fork [koderover/dex](https://github.com/koderover/dex) 编写自定义 Connector 实现统一账号系统的集成。

## 自定义账号系统登录流程图
![account_custom](../_images/account_custom.jpeg)
...
2. 编写 Dex 自定义 Connector （基于最新 Branch release-1.8.0）
- 目前 Dex Connector 接口定义于 dex/connector/connector.go 中，官方抽象了`PasswordConnector`、`CallbackConnector`、`RefreshConnector`、`SAMLConnector` 四种接口，用户可以根据自己账号系统情况实现自己的 Connector，参考 dex/connector 的各种 connector 组合实现这四种接口。

以下是对各个接口的简要说明:
|接口名|使用说明|目前可参考已有实现|
|-----|-----|-----|
|PasswordConnector|简单使用用户名密码方式登录授权获取用户信息的|keystone、atlassiancrowed、ldap、mock/passwordConnector、passwordDB(位于dex/server/server.go)|
|CallbackConnector|通过重定向获取用户信息的|mock/Callback、bitbucketcloud、authproxy、gitea、github、gitlab、google、linkedin、microsoft、oauth、oidc、openshift|
...
此 connector 实现了 CallbackConnector interface，以下为 OAuth connector 代码
::: details
``` Go
package oauth
...
### 步骤 2：构建自定义的 Dex 镜像
1. 更改 Makefile 中的 `DOCKER_REPO` 变量为自己的公开镜像仓库
2. 运行 `make docker-image` 构建镜像后上传
3. 安装 Zadig 的时候用自定义的 Dex 镜像覆盖配置中 Zadig Dex 镜像
```bash
# 详细升级过程参考对应版本的升级文档
helm upgrade --install ... \ 
    --set dex.image.repository=<您的公开镜像仓库> \
    --set dex.image.tag=<上传的镜像 tag>
```

### 步骤 3：在 Zadig 平台中录入配置
登录 Zadig 平台后，在`系统设置` -> `用户账户集成`-> `自定义`账户类型录入配置，如下图所示。
