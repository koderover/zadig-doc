...
目前 Zadig 用户系统采用了开源项目 [dex](https://github.com/dexidp/dex) 作为身份连接器，但 dex 支持的协议列表（[dex 支持的协议](https://dexidp.io/docs/connectors/)）有限，Zadig 则基于 dex 官方库实现了一些标准的扩展，如 OAuth 协议、Portal 接入层等。如果用户自身的账号系统在 Zadig 官方支持之外，可以通过 Clone [koderover/dex](https://github.com/koderover/dex) 编写自定义 connector 实现统一账号系统的集成。
...
2. 编写 dex 自定义 connector （基于最新 tag ，比如：v1.7.0-zadig ）
