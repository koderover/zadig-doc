...
permalink: /Zadig v3.4/project/common-workflow/
---
...
### 任务
点击阶段下方的 `+任务` 为阶段增加任务配置，系统支持的任务类型及具体配置可参考文档 [工作流任务](/Zadig%20v3.4/project/workflow-jobs/)。
...
- `类型`：支持字符串、多行文本、单选、多选、代码库、镜像、[动态变量](/Zadig%20v3.4/workflow/dynamic-variable/)。
...
支持`全局变量/其他任务输出`配置方式的任务可参考文档：[不同任务中支持的配置方式](/Zadig%20v3.4/project/workflow-jobs/#不同任务中支持的配置方式)。<br>
...
为工作流配置触发器，当满足触发条件的事件发生时会自动触发工作流执行，支持 Git 触发器、定时器、JIRA 触发器、飞书项目触发器和通用触发器，具体如何使用可参考文档：[工作流触发器](/Zadig%20v3.4/project/workflow-trigger/)。
...
目前支持配置工作流通知到邮件/飞书/企业微信/钉钉/Webhook 中，详细的配置参数说明可参考：[IM 状态通知](/Zadig%20v3.4/workflow/im)。
...
### 集群资源配置
参考文档：[集群共享存储资源配置](/Zadig%20v3.4/pages/cluster_manage/#共享存储资源配置)。
...
> 支持高级配置的任务请参考文档：[高级配置](/Zadig%20v3.4/project/workflow-jobs/#高级配置)。
