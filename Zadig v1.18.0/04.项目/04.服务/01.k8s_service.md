...
permalink: /Zadig v1.18.0/project/service/k8s/
---
...
2. 支持的代码源请参考文档：[代码源信息](/Zadig%20v1.18.0/settings/codehost/overview/#功能兼容列表)
:::
...
::: tip 前提
需要先在系统模板库里创建 K8s YAML 模板，请参考 [K8s YAML 模板管理](/Zadig%20v1.18.0/template/k8s_yaml/)。
:::
...
参考文档：[服务组件](/Zadig%20v1.18.0/project/service/module/)。
...
在环境中，点击`更新服务`按钮，即可更新对应服务的变量值，支持手动修改或使用全局变量。同时可以通过勾选 `更新服务配置`，选择是否更新当前服务的 YAML 版本。
...
- 交付物命名规则设置：自定义该项目中工作流构建产物的命名规则，可通过以下[内置变量](#内置变量)和常量组合的方式设置，注意：
  ...
  - `JENKINS 生成镜像规则`适用于使用 [Jenkins 构建](/Zadig%20v1.18.0/project/build/#jenkins-构建) 的服务，不可使用下述[内置变量](#内置变量)中的 `REPO_*` 变量。
