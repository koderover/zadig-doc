...
permalink: /en/Zadig v4.2/project/common-workflow/
---
...
### Task
Click `+Task` below the stage to add task configuration. For the task types and specific configurations supported by the system, see [Workflow Tasks](/en/Zadig%20v4.2/project/workflow-jobs/).
...
- `Type`: Supports string, multi-line text, single-choice, multiple-choice, file, code repository, image, and [dynamic variables](/en/Zadig%20v4.2/workflow/dynamic-variable/).
...
Configure triggers for workflows. When an event that meets the trigger conditions occurs, the workflow execution will be automatically triggered. It supports Git triggers, timers, JIRA triggers, Feishu project triggers, and general triggers. For details, please refer to the document: [Workflow Triggers](/en/Zadig%20v4.2/project/workflow-trigger/).
...
Currently, it supports configuration workflow notifications to email / Feishu / Enterprise WeChat / DingTalk /Webhook in the detailed configuration parameter description can be found: [IM status notification](/en/Zadig%20v4.2/workflow/im) .
...
Reference document: [Cluster shared storage resource configuration](/en/Zadig%20v4.2/pages/cluster_manage/#shared-storage-resource-configuration) .
...
> For tasks that support advanced configuration, please refer to the documentation: [Advanced Configuration](/en/Zadig%20v4.2/project/workflow-jobs/#advanced-configuration) .
