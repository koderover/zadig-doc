...
Zadig provides the zadig-plugin-cli tool to help developers quickly create, develop, and build custom plugins.
...
- **Modern Browser**: Supporting ES6+ syntax
...
```bash
# Create navigation function page plugin
zadig-plugin create my-hello-plugin

# Create tab page plugin  
zadig-plugin create my-tab-plugin --type tab
```

**Command Options:**
- `-t, --type <type>` - Plugin type (page, tab), defaults to page
...
After the development server starts, you can view plugin effects and debug functionality in real-time in the browser.
...
After building is complete, a `plugin.js` file will be generated in the project directory, which can be directly uploaded to the Zadig system.

### Plugin Type Description

#### Page Plugin (Navigation Function Page)

- **Features**: Independent functional pages with complete routing and page layout
- **Display Location**: System sidebar menu
- **Usage**: Provide independent functional modules, such as GitLab MR viewer, code quality reports, etc.
- **Template Includes**: Route configuration, page components, style files

#### Tab Plugin (Tab Page)

- **Features**: Tab components embedded in existing pages, no independent routing
- **Display Location**: Tab area of business directories
- **Usage**: Extend existing page functionality, such as custom tabs in project detail pages
- **Template Includes**: Tab page components, integration configuration
...
**Issue**: Errors occur during the build process

**Solutions**:
1. Check if Node.js version meets requirements (>= 20.0.0)
   ```bash
   node --version
   ```
2. Reinstall dependencies
   ...
   npm install
   ```
3. Check code syntax and import paths

#### Development Server Start Failure

**Issue**: `zadig-plugin dev` command execution fails
...
#### Plugin Cannot Load in Zadig

**Issue**: Plugin does not display properly after upload

**Solutions**:
1. Check if build artifact `plugin.js` is correctly generated
2. Verify plugin configuration information is complete
3. Check browser console for error messages

### Best Practices

#### 1. Responsive Design
Plugins should adapt to different screen sizes and display well on various devices.

#### 2. Error Handling
Implement proper exception handling and provide friendly error messages.
...
- Follow Zadig design specifications

### Development Resources

- **Official Plugin Repository**: `https://github.com/koderover/zadig-idp-plugins`
- **Example Plugins**: Refer to example code in the official repository
- **API Documentation**: See Zadig Plugin Development API documentation for details

::: tip Development Support
- For plugin development support, please contact the Zadig technical support team
- Welcome to contribute quality plugins to the official plugin repository
- You can report issues encountered during development in GitHub Issues
:::
