...
- `Distribution Method` :
  ...
  - `Cloud Vendor Synchronization` : Use cloud vendor multi-region synchronization. Zadig is responsible for converting the image address to ensure that subsequent tasks can obtain the correct image address, while the actual image synchronization is completed by the cloud vendor.
