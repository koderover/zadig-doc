...
![Zadig MCP Server configuration success](../../../_images/mcp_settings_success_1.png)
![Zadig MCP Server connection success](../../../_images/mcp_settings_success_2.png)
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: zadig-mcp
          image: your-registry/zadig-mcp:<version>
          imagePullPolicy: IfNotPresent
          args: ["-t", "$(MCP_TRANSPORT_TYPE)", "-a", "$(MCP_LISTEN_ADDR)"]
          ports:
            - name: http
              containerPort: 3000
              protocol: TCP
    ...
    - host: mcp.your-company.com
      http:
        paths:
          - path: /mcp
            pathType: Prefix
            backend: { service: { name: zadig-mcp-http, port: { number: 80 } } }
          - path: /sse
            pathType: Prefix
            backend: { service: { name: zadig-mcp-http, port: { number: 80 } } }
          - path: /message
...
Replace `your-registry/zadig-mcp:<version>` with an accessible Zadig MCP Server image and version. To configure a default Token for the server, inject `ZADIG_API_TOKEN` through a Kubernetes Secret. Remote clients can also pass their own Token through a request header or URL parameter.
    ...
    spec:
      containers:
        - name: zadig-mcp
          ...
          envFrom: [{ configMapRef: { name: zadig-mcp-sse-config } }]
          readinessProbe:
            tcpSocket: { port: 3000 }
            initialDelaySeconds: 5
            periodSeconds: 10
    ...
    - host: mcp.your-company.com
      http:
        paths:
          - path: /sse
            pathType: Prefix
            backend: { service: { name: zadig-mcp-sse, port: { number: 80 } } }
          - path: /message
            pathType: Prefix
            backend: { service: { name: zadig-mcp-sse, port: { number: 80 } } }
```
...
![Feishu Aily connection success](../../../_images/aily_setting_success1.png)
![Feishu Aily connection success](../../../_images/aily_setting_success2.png)
