...
SSE mode supports the following two authentication methods:

1. `Header` parameter: suitable for clients that support custom request headers in MCP configuration, such as IDEs.
2. `token` URL parameter: suitable for clients that cannot easily customize request headers but support direct URL input, such as Feishu Aily.

##### IDE Configuration

Using `Header`-based authentication, configure `mcp.json` in the IDE Settings:
...
Using `token` URL parameter authentication, add the following URL in Feishu Aily:
