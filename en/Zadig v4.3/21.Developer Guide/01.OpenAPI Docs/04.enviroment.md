...
permalink: /en/Zadig v4.3/api/env/
---
...
<h4 id="SubEnv">SubEnv Parameters</h4>
...
## View Service Pod List

::: tip
1. Applicable to K8s YAML projects.
2. This API is used to get the current Pod list for a specified service.
3. To restart a single Pod, it is recommended to call this API first to obtain the target `pod_name`, then call the `Restart a Single Pod` API below.
:::
...
```
GET /openapi/environments/<environmentKey>/service/<serviceName>/pods?projectKey=<projectKey>
```
...
```
GET /openapi/environments/production/<environmentKey>/service/<serviceName>/pods?projectKey=<projectKey>
```
...
::: tip
1. Applicable to K8s YAML projects.
2. This API first checks whether the target Pod exists, then deletes it so that the owning workload recreates a new Pod automatically.
3. Because the path includes `environmentKey`, `serviceName`, and `podName`, it is recommended to call `View Service Pod List` first to confirm the target `pod_name`.
:::
...
```
POST /openapi/environments/<environmentKey>/service/<serviceName>/pod/<podName>/restart?projectKey=<projectKey>
```
...
```
POST /openapi/environments/production/<environmentKey>/service/<serviceName>/pod/<podName>/restart?projectKey=<projectKey>
```
...
<h4 id="SubEnv-1">SubEnv Parameters</h4>
...
<h4 id="HelmService">HelmService Parameters</h4>
...
<h4 id="OverrideKV">OverrideKV Parameters</h4>
...
<h4 id="ImportValuesFromGit">ImportValuesFromGit Parameters</h4>
