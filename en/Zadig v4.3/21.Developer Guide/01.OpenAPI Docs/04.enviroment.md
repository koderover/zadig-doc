...
> Applicable to K8s YAML/Helm projects.
...
```
GET /openapi/environments/<environmentKey>/pods?projectKey=<projectKey>
```
...
```
GET /openapi/environments/production/<environmentKey>/pods?projectKey=<projectKey>
```
...
```
GET /openapi/environments/<environmentKey>/pods/<podName>?projectKey=<projectKey>
```
...
```
GET /openapi/environments/production/<environmentKey>/pods/<podName>?projectKey=<projectKey>
```
...
::: tip
1. Applicable to K8s YAML/Helm projects.
2. Zadig restarts a single Pod by deleting the specified Pod and letting its owning workload recreate it automatically.
3. It is recommended to call `View Pod List` or `View Pod Details` first to confirm the target Pod name before restarting it.
:::
...
```
POST /openapi/environments/<environmentKey>/pods/<podName>/restart?projectKey=<projectKey>
```
...
```
POST /openapi/environments/production/<environmentKey>/pods/<podName>/restart?projectKey=<projectKey>
```
