...
![system_webhook](../../../_images/system%20webhook-en.jpg)
...
- `Trigger Events`: Select which system events should trigger notifications. Currently supported:
  - `Start Execute`
  - `Complete Execute`
...
- `event`: Event name. Currently supported:
  - `start_execute`
  - `complete_execute`
...
- `detail_url`: Link to the task details page.
- `error`: Error message.
- `create_time`: Task creation time.
- `start_time`: Task start time.
- `end_time`: Task end time.
- `stages`: Stage and job details.
- `task_creator`: Task creator name.
- `task_creator_id`: Task creator ID.
- `task_creator_email`: Task creator email.
- `task_type`: Task type. Currently `workflow`.
