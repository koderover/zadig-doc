...
Parameter Description:
- `Task Name` : Supports lowercase English letters, numbers, or hyphens within 32 characters, and must start with a lowercase English letter; in the same workflow, the task name must be unique
- `Environment` : Select the environment to be deployed, and see [variable assignment](/en/Zadig%20v4.3/project/common-workflow/#variable-assignment-method)
...
- `Deployment Content` : Includes the following three options:
  ...
  - `Service Variables` : Update the service variables. See the definition of variables: [Service variables](/en/Zadig%20v4.3/project/service/k8s/#variable-configuration)
    - Variable sources support runtime input and global variables/other task outputs
  - `Service Configuration` : Update the service configuration. See the service configuration source: [Service Configuration](/en/Zadig%20v4.3/project/service/k8s/#create-service)
...
- `Deployment Strategy` :
  - `K8s YAML Service` :
    ...
    - `Overwrite` : Fully replace existing namespace configuration with Zadig YAML; all differences in the same resource will be overwritten
  - `K8s Helm Chart Service` :
    - `Merge` : Merge with current environment variables (Helm CLI `--reuse-values` option)
    - `Overwrite` : Use input variables to overwrite current values
