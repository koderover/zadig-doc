...
3. Configure service builds. See: [Build](/en/Zadig%20v4.3/project/build).
...
::: tip
To manage service configuration on Zadig later, switch project type. See: [Switch project type](/en/Zadig%20v4.3/project/host-k8s-resources/#switch-a-k8s-hosting-project-to-a-k8s-yaml-project).
:::
...
2. Create new service: Sync service configuration from codebase. See: [Create service](/en/Zadig%20v4.3/project/service/k8s/#create-a-new-service).
...
4. Create new environment: Select existing namespace (e.g., dev/sit/uat/prod). See: [Create environment](/en/Zadig%20v4.3/project/env/k8s/#create-a-new-environment).
...
2. Create new service: Sync service configuration from codebase. See: [Create service](/en/Zadig%20v4.3/project/service/helm/chart/#create-a-new-service).
...
5. Execute workflows: Enter project -> Workflows, run workflow to update services. See: [Workflows](/en/Zadig%20v4.3/project/common-workflow/).
...
2. Create a new service: Use templates to create a new service. See: [Create a new service](/en/Zadig%20v4.3//project/service/k8s/#create-a-single-service-using-templates).
...
4. Configure builds: Use build templates to configure build scripts. See: [Build](/en/Zadig%20v4.3/project/build).

<img src="../../_images/project_admin_22.png" width="400">
<img src="../../_images/project_admin_23.png" width="400">


5. Configure workflows: Configure the corresponding workflows for test and production environments. See: [Workflows](/en/Zadig%20v4.3/project/common-workflow/).
...
2. Create a new service: Use templates to create a new service. See: [Create a new service](/en/Zadig%20v4.3/project/service/helm/chart/#create-a-single-service-using-templates).
![New project access](../../_images/project_admin_27.png)

3. Create a new environment: Select the service to create new dev/sit/uat/prod environments.
![New project access](../../_images/project_admin_28.png)

4. Configuration build: Use the build template to configure the build script. See: [Build](/en/Zadig%20v4.3/project/build/#%E6%9B%B4%E5%A4%9A%E6%9E%84%E5%BB%BA%E6%AD%A5%E9%AA%A4).
...
Preparation: Abstract the configuration of multiple services into a K8s YAML template. The following are the relevant configurations of multi-service-demo projects for reference. For more usage postures, see: [K8s YAML Template](/en/Zadig%20v4.3/template/k8s_yaml/), [create a new service using templates](/en/Zadig%20v4.3/project/service/k8s/#%E4%BD%BF%E7%94%A8%E6%A8%A1%E6%9D%BF%E6%96%B0%E5%BB%BA%E6%9C%8D%E5%8A%A1).
...
Preparation: abstract the Helm Chart configurations of multiple services into Helm Chart templates, and the following are the relevant configurations of multi-service-demo projects for reference. For more usage postures, see: [Helm Chart Template](/en/Zadig%20v4.3/template/helm_chart/), [create a new service using template](/en/Zadig%20v4.3/project/service/helm/chart/#%E4%BD%BF%E7%94%A8%E6%A8%A1%E6%9D%BF%E6%96%B0%E5%BB%BA%E5%8D%95%E4%B8%AA%E6%9C%8D%E5%8A%A1).
...
Preparation: Abstract the mirror building configuration of multiple services into a Dockerfile template. The following are the relevant configurations of multi-service-demo projects for reference. For more usage postures, see: [Dockerfile Template](/en/Zadig%20v4.3/template/dockerfile/), [build configuration](/en/Zadig%20v4.3/project/build/#%E6%9B%B4%E5%A4%9A%E6%9E%84%E5%BB%BA%E6%AD%A5%E9%AA%A4).
...
Abstract workflow templates based on project processes, quickly create workflows based on templates. For more details, see: [Workflow Templates](/en/Zadig%20v4.3/template/workflow/).
...
1. Based on K8s  YAML template, create new business services. See: [service](/en/Zadig%20v4.3/project/service/k8s/#%E4%BD%BF%E7%94%A8%E6%A8%A1%E6%9D%BF%E6%96%B0%E5%BB%BA%E6%9C%8D%E5%8A%A1).
![Business operations and maintenance operations](../../_images/daily_admin_1.png)

2. Based on the build template, configure the build for the service. See: [Build](/en/Zadig%20v4.3/project/build/template/).

::: tip Optimization Suggestions:
|    Suggestions   |                Description                           |                    Reference Documents              |
|:----------- | :-------------------------------------------- |:-----------------------------------------|
| Speed up the build process | 1. Include commonly used build dependency tools<br> 2. Customize cache directories| 1. [Custom build images](/en/Zadig%20v4.3/settings/custom-image/)<br> 2. [Configure the build cache](/en/Zadig%20v4.3/project/build/#%E9%AB%98%E7%BA%A7%E9%85%8D%E7%BD%AE) |
| Reduce maintenance burden | 1. Multiple services have similar builds; configure a single build script to reduce maintenance costs<br> 2. Standardize and unify the management of service build materials to reduce maintenance and management burdens| 1. [Share build scripts](/en/Zadig%20v4.3/project/build/#Zadig-%E5%85%B1%E4%BA%AB%E6%9E%84%E5%BB%BA)<br> 2. [Use the build template](/1) |
:::

![Business operation and maintenance operations](../../_images/project_admin_template_21_220.png)

Step 2: Add to the environment. See: [K8s YAML Environment](/en/Zadig%20v4.3/project/env/k8s/#%E5%9F%BA%E6%9C%AC%E6%93%8D%E4%BD%9C).
...
1. Delete service definition. See: [Delete service](/en/Zadig%20v4.3/project/service/k8s/#%E5%88%A0%E9%99%A4%E6%9C%8D%E5%8A%A1).
![Business operation and maintenance operations](../../_images/daily_admin_5.png)

2. Delete the service from the environment. See: [K8s YAML Environment](/en/Zadig%20v4.3/project/env/k8s/#%E5%88%A0%E9%99%A4%E6%9C%8D%E5%8A%A1).
...
- K8s YAML Project. See: [Create a new K8s YAML environment](/en/Zadig%20v4.3/project/env/k8s/#%E6%96%B0%E5%BB%BA%E7%8E%AF%E5%A2%83).
- K8s Helm Chart Project. See: [Create a new Helm Chart environment](/en/Zadig%20v4.3/project/env/helm/chart/#%E6%96%B0%E5%BB%BA%E7%8E%AF%E5%A2%83).
...
1. Use a single set of service configurations to create multiple test environments. See: [Configure Multiple Integrated Environments](/en/Zadig%20v4.3/env/multi-env/).
2. Different environments use different structures deployment , variable capabilities can be used. See: [K8s YAML](/en/Zadig%20v4.3/project/service/k8s/#%E5%8F%98%E9%87%8F%E9%85%8D%E7%BD%AE).
3. Different business lines can use different K8s cluster resources. For adding new cluster configurations, see: [Cluster Management](/en/Zadig%20v4.3/pages/cluster_manage/).
4. Environment configuration management. See: [Environment configuration](/en/Zadig%20v4.3/project/env/k8s/#%E7%8E%AF%E5%A2%83%E9%85%8D%E7%BD%AE).

### Online Production Service

Step 1: Configure production services. See: [production services](/en/Zadig%20v4.3/project/service/k8s/prod/#%E6%96%B0%E5%BB%BA).
![Business operation and maintenance operations](../../_images/daily_admin_9.png)

Step 2: Configure the production environment. See: [K8s YAML Production Environment](/en/Zadig%20v4.3/project/env/k8s/#production-environment).
![Business operation and maintenance operations](../../_images/daily_admin_10.png)

Step 3: Configure the workflow, add deployment tasks. See: [Workflow](/en/Zadig%20v4.3/project/workflow-jobs/#%E9%83%A8%E7%BD%B2%E4%BB%BB%E5%8A%A1).
...
See: [Offline service](/en/Zadig%20v4.3/project/workflow-jobs/#%E4%B8%8B%E7%BA%BF%E6%9C%8D%E5%8A%A1).

### Create a New Production Environment

See: [Production Environment](/en/Zadig%20v4.3/project/env/k8s/#production-environment).

## Workflow Configuration

For specific configuration, see: [Workflow](/en/Zadig%20v4.3/project/workflow-overview/)
...
Access project configuration -> permissions, configure roles and project members. See: [permission configuration](/en/Zadig%20v4.3/project/permission/#permission-configuration).
...
Access permission management -> Collaboration mode, just configure permissions for collaboration members, workflows, and environmental resources. For specific configuration process, see: [Collaboration mode](/en/Zadig%20v4.3/project/permission/#collaboration-mode).
