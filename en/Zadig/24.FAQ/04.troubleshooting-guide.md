...
permalink: /en/Zadig v5.0/faq/troubleshooting-guide/
---
...
![approval-shooting](./_images/troubleshooting-guide/approval-shooting.png)
...
1. Code repository uses submodule but code can't be pulled, git submodule update hangs: Recommend using Proxy in system settings, see: [Proxy Configuration](/en/Zadig%20v5.0/settings/system-settings/#proxy-configuration)
...
This is caused by insufficient space in the PVC mounted by Zadig system's spock-dind service. You can use our system settings - system configuration - cache cleanup feature to clean this data with one click. See [Cache Cleanup](/en/Zadig%20v5.0/settings/system-settings/#cache-cleaning)

### `Failed parsing or buffering the chunk-encoded client body`
![Image build error](./_images/troubleshooting-guide/docker_build_proxy.png)
...
When the above error occurs during build tasks, this is because a self-signed certificate is used when logging into a private image registry. This certificate is not issued by a trusted certificate authority (CA), so the Docker daemon doesn't trust the certificate and reports an x509 self-signed certificate error. You can refer to [Image Registry Configuration](/en/Zadig%20v5.0/settings/image-registry/#add-image-registry) to disable SSL verification.
...
![GitLab Configuration](./_images/troubleshooting-guide/gitlab_config.png)
...
After [configuring triggers in workflows](/en/Zadig%20v5.0/project/workflow-trigger/#git-trigger), there's no need to manually add Webhook settings in GitLab system.
...
![Check hosted repository information for K8s YAML service configuration](./_images/troubleshooting-guide/check_webhook_setting_for_service.png)
...
Edit the specific workflow, switch to the `Triggers` tab to view Webhook configuration details, and ensure code changes and Webhook events meet trigger conditions. For parameter descriptions in configuration, see [Webhook Configuration](/en/Zadig%20v5.0/project/workflow-trigger/#git-trigger).

![Check trigger configuration in workflow](./_images/troubleshooting-guide/check_webhook_setting.png)
![Check trigger configuration in workflow](./_images/troubleshooting-guide/check_webhook_setting_1.png)
...
![Check trigger configuration in GitHub](./_images/troubleshooting-guide/check_webhook_setting_in_github_1.png)

- In `Settings`, view specific Webhook configuration, and check recent Webhook Events and status in `Recent Deliveries`.

![Check trigger configuration in GitHub](./_images/troubleshooting-guide/check_webhook_setting_in_github_2.png)
...
![Check trigger configuration in GitLab](./_images/troubleshooting-guide/check_webhook_setting_in_gitlab_1.png)

- `Test` operation can test Webhook settings, click `Edit` to view recent Webhook Events and status.
![Check trigger configuration in GitLab](./_images/troubleshooting-guide/check_webhook_setting_in_gitlab_2.png)
...
![Check GitLab token expiration settings](./_images/troubleshooting-guide/check_gitlab_token_expire_setting.png)
...
> For more information, see [Workflow Image Version Rollback](/en/Zadig%20v5.0/workflow/image/rollback/)

![Check image version rollback switch in workflow](./_images/troubleshooting-guide/check_pipeline_setting.png)
