...
permalink: /en/Zadig v5.0/project/env/vm/
---
...
![Create a new environment](./_images/vm_environment/create_vm_env_1.png)

![Create a new environment](./_images/vm_environment/create_vm_env_2.png)
...
> Associating host resources requires integrating hosts in the system first; refer to [Host Integration](/en/Zadig%20v5.0/settings/vm-management/).
...
![Environment details](./_images/vm_environment/show_vm_env_list_500.png)
...
- `Environment Status`: Including two states: `Running` and `Updatable`. When there is any modification in [the service](/en/Zadig%20v5.0/project/service/vm/) after the environment is created (change/add/delete), the environment's status will become updatable.
- `Basic Operations`: Update the environment, set an alias, or delete the environment.

Service List Information Description:

- `Service Name`: The name of the service. When the service has been modified in [services](/en/Zadig%20v5.0/project/vm/service/), you can click the update button on the right side of the service to update it.
...
> It is necessary to note that the corresponding host is online, and can be viewed in [Host Management](/en/Zadig%20v5.0/settings/vm-management/#view-host)

![Log in to the host](./_images/vm_environment/login_vm_to_debug.png)

### Start and Stop Services

Refer to [service start and stop configuration](/en/Zadig%20v5.0/project/service/vm/#start-and-stop-configuration)

## Update the Environment

When the environment is updatable, click `Update Environment` in the basic operations to update the environment.

![Update the host environment](./_images/vm_environment/update_vm_env.png)
...
![Delete the host environment](./_images/vm_environment/delete_vm_env.png)