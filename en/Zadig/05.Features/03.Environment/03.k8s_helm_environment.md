...
permalink: /en/Zadig v5.0/project/env/helm/chart/
---
...
<img src="./_images/k8s_helm_environment/show_helm_test_env.png" width="400">
<img src="./_images/k8s_helm_environment/show_helm_test_env_resource_view.png" width="400">
...
- For AI environment inspection, refer to [AI Environment Inspection](/en/Zadig%20v5.0/ai/env-analysis/)
- Other introductions can be found K8s YAML Description in the test environment: [View the environment](/en/Zadig%20v5.0/project/env/k8s/#view-the-environment)
...
![Create a new Helm Chart environment](./_images/k8s_helm_environment/create_helm_chart_env_1.png)
![Create a new Helm Chart environment](./_images/k8s_helm_environment/create_helm_chart_env_2.png)
...
- `Creation Method`: Supports `New` and `Copy` methods. For copying an environment, refer to [Copy Environment](#copy-environment).
- `Resource Selection` :
...
- Supports direct pasting or importing configurations from the code base. For supported code sources, refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list)
- After the environment is created successfully, the newly created configuration here will take effect in the environment / cluster. You can also manage the environment configuration after the environment is created, refer to [the environment configuration management.](/en/Zadig%20v5.0/project/env/k8s/#environment-configuration)

![Create a new Helm Chart environment configuration](./_images/k8s_helm_environment/add_k8s_helm_chart_env_config.png)
...
- Import from the specified code base values. For supported code sources, refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list) .
...
![Import values from the code repository](./_images/k8s_helm_environment/auto_sync_helm_chart_values_1.png)
...
![Import values from the code repository](./_images/k8s_helm_environment/auto_sync_helm_chart_values.png)
...
Refer to [Create a New Environment](/en/Zadig%20v5.0/project/prod/env/k8s/#create-a-new-environment).

## Copy Environment

> Only the test environment supports copying environments.

Reference K8s YAML Description in the Test Environment: [Copy Environment](/en/Zadig%20v5.0/project/env/k8s/#copy-environment) .
...
- Import values files from the code base. For supported code base, refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list)
- Use variable groups. For variable group management, refer to [Variable Group Configuration](/en/Zadig%20v5.0/project/permission/#variable-groups)

![Update Helm Chart environment variables](./_images/k8s_helm_environment/update_helm_env_var.png)
...
The service configuration is managed in Zadig.

Click `Manage Services` -> `Add Service` to select the service you want to add and modify its values content as needed. After selecting a service, the system will automatically detect whether the release exists in the selected Kubernetes namespace. If the resource already exists, you can choose to import the existing service in one click or redeploy the service.

<img src="./_images/k8s_helm_environment/add_helm_chart_service_01.png" width="400">
<img src="./_images/k8s_helm_environment/add_helm_chart_service_02_430.png" width="400">

**Chart Repository**

The service configuration is managed in the Chart repository. Select `Chart Repository` > click Add > enter the service Release name > select the Chart repository and version. You can modify the values file content as needed.

![Manage Services](./_images/k8s_helm_environment/add_helm_chart_service_by_chart_0.png)
...
1. `Import Only`: The system automatically detects whether the Release already exists in the namespace. If it exists, you can import only: import the service configuration from the namespace into the system without deploying, without affecting the cluster.
2. `Execute Deployment`: Deploy the service to the cluster; if the service already exists, update the service configuration.
3. `Save Draft`: Save the service configuration as a draft without deploying, without affecting the cluster.
...
![Update Helm Chart Service](./_images/k8s_helm_environment/update_helm_chart_service.png)
...
![Delete Helm Chart Service](./_images/k8s_helm_environment/delete_helm_chart_service_341.png)
...
<img src="./_images/k8s_helm_environment/service_version_track_5.png" width="400">
<img src="./_images/k8s_helm_environment/service_version_track_6.png" width="400">
<img src="./_images/k8s_helm_environment/service_version_track_7.png" width="400">
<img src="./_images/k8s_helm_environment/service_version_track_8.png" width="400">

## Environment Configuration

For environment configuration Ingress ConfigMap , Secret , PVC resources, refer to [Environment Configuration Management](/en/Zadig%20v5.0/project/env/k8s/#environment-configuration) for details.

## Self-Test Mode

Refer to [Self-test mode](/en/Zadig%20v5.0/project/env/k8s/#self-test-mode) .

## Sleep and Wake Up

Refer to [Sleep and Wake Up](/en/Zadig%20v5.0/project/env/k8s/#sleep-and-wake-up) .

## Change Record
Click the icon in the upper right corner to view the operation log of the environment.

![Change Record](./_images/k8s_helm_environment/env_oplog_helm_chart.png)
...
![chart release view](./_images/k8s_helm_environment/chart_release_view.png)

If the update fails, the service will turn red, and you can view the specific reason for the failure.

![chart release view](./_images/k8s_helm_environment/chart_release_view_error_msg.png)
...
![Set an Alias](./_images/k8s_helm_environment/prod_env_alias_demo.png)
...
![Delete Helm Chart Environment](./_images/k8s_helm_environment/helm_chart_env_deleteble.png)
