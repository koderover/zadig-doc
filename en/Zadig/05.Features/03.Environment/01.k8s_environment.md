...
permalink: /en/Zadig v5.0/project/env/k8s/
---
...
<img src="./_images/k8s_environment/env_detail.png" width="400">
<img src="./_images/k8s_environment/env_detail_resource_view.png" width="400">
...
- `Update Global Variables` : Update the global variables of the environment, refer to [update the global variables of the environment](/en/Zadig%20v5.0/project/service/variable/#update-environment-global-variables)
...
![Change Record](./_images/k8s_environment/env_oplog.png)

### Service Details

Click on a specific service in the service list to view more information about the service. You can perform operations such as service restart, service scaling, configuration management, and image updates. For more details, refer to [Service Details](/en/Zadig%20v5.0/project/env/service/).

![Service Details](./_images/k8s_environment/service_detail_in_k8s_env.png)

### AI Environment Inspection

Refer to [AI Environment Inspection](/en/Zadig%20v5.0/ai/env-analysis/).
...
<img src="./_images/k8s_environment/create_env.png" width="400">
<img src="./_images/k8s_environment/create_env_1.png" width="400">
...
- `Creation Method` : Supports three methods: `New`, `Copy` and `Rollback`. For copy environment, refer to [Copy Environment](#copy-environment). For rollback environment, refer to [Rollback Environment](#rollback-environment).
- `Resource Selection` :
	- `K8s Cluster` : Select the cluster resource to be used when creating a new environment. `Local Cluster` refers to the cluster where the Zadig system is located. For cluster integration, refer to [Cluster Management](/en/Zadig%20v5.0/pages/cluster_manage/).
	...
	- `Image Repository` : Specify the image registry used after the environment is created, use the workflow to build and deploy services, and modify service images, and use the image registry specified here. For the integration of image registrys, refer to [image registry management](/en/Zadig%20v5.0/settings/image-registry/#add-a-mirror-repository) .
...
- Supports direct pasting or importing configurations from the code base. For supported code base, refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list)
...
![New Environment Configuration](./_images/k8s_environment/add_k8s_yaml_env_config.png)

#### Global Variables

When creating a new environment, the default values of global variables can be overwritten as needed to realize differentiated configurations of different environments. Refer to [Global variables](/en/Zadig%20v5.0/project/service/variable/#update-environment-global-variables) .

![Configure Global Variables](./_images/k8s_environment/config_k8s_yaml_env_var.png)
...
![New Environment Configuration](./_images/k8s_environment/servicelist.png)

### Production Environment

In the project, click the `New Environment` button, select the production environment, and enter the new environment page.

<img src="./_images/k8s_environment/create_prod_env_for_k8s.png" width="400">
<img src="./_images/k8s_environment/create_prod_env_for_k8s_1.png" width="400">
...
- `Image Repository` : Specify the image registry used after the environment is created, use the workflow to build and deploy services, and modify service images, and use the image registry specified here. For the integration of image registrys, please refer to [image registry management](/en/Zadig%20v5.0/settings/image-registry/#add-a-mirror-repository) .
...
<img src="./_images/k8s_environment/create_env.png" width="400">
<img src="./_images/k8s_environment/create_env_by_copy.png" width="400">
...
- `Resource Selection` : Refer to the description in [New Environment > Basic Information](#basic-information)
- `Service Selection` : By default, it is consistent with the copied environment and can be manually deleted
- `Environment Configuration` : Refer to the description in [New Environment > Environment Configuration](#environment-configuration)
...
> Create a new environment based on an existing service version to rollback the version.

When creating a new environment, you can create version rollback based on the specified version. You can refer to the creation version: [Create version](/en/Zadig%20v5.0/project/version/#k8s-yaml-%E9%A1%B9%E7%9B%AE) .

![Rollback Environment](./_images/k8s_environment/create_env_by_given_version.png)

- `Environment Name` : Customize according to actual semantics
- `Creation Method` : Select `Rollback`
- `Version` : Specify the version to be rolled back
- `Resource Selection` : Refer to the description in [New Environment > Basic Information](#basic-information)
- `Service Selection` : By default, it is consistent with the copied environment and can be manually deleted
- `Environment Configuration` : Refer to the description in [New Environment > Environment Configuration](#environment-configuration)
...
![Manage Services](./_images/k8s_environment/service_mgr_in_env.png)

### Add a Service

The service to be added needs to be defined in [Services](/en/Zadig%20v5.0/project/service/k8s/) in advance. After selecting a service, the system will automatically detect whether the resources defined in the service exist in the selected Kubernetes namespace. If the resources already exist, you can choose to import the existing service in one click or redeploy the service.

In `Variable Configuration` , configure the variable for the service, refer to the document [service variable](/en/Zadig%20v5.0/project/service/variable/#update-service-variables) .

![Manage Services](./_images/k8s_environment/add_service_in_k8s_env_430.png)

Operation instructions:

- `Import Only`: Import the service configuration from the namespace into the system without deploying, without affecting the cluster
- `Run Deploy`: Deploy the service to the cluster; if the service already exists, update the service configuration
- `Save Draft`: Save the service configuration as a draft without deploying, without affecting the cluster
...
![Manage Services](./_images/k8s_environment/update_service_in_k8s_env.png)

### Delete a Service

Remove the service from the current environment, and optionally delete specific resources such as Pod, Deployment, Service, Ingress, ConfigMap, Secret, PVC, etc.
...
:::
![Delete Service](./_images/k8s_environment/delete_service_in_k8s_env_341.png)
...
<img src="./_images/k8s_environment/service_version_track_3.png" width="400">
<img src="./_images/k8s_environment/service_version_track_4.png" width="400">
...
<img src="./_images/k8s_environment/show_env_config.png" width="300">
<img src="./_images/k8s_environment/show_secret_config_in_env.png" width="300">
<img src="./_images/k8s_environment/show_secret_config.png" width="300">
...
![Add Secret Configuration](./_images/k8s_environment/create_secret_config.png)
...
![Add Secret Configuration](./_images/k8s_environment/autosync_env_config.png)
...
![Edit Secret Configuration](./_images/k8s_environment/edit_secret_config.png)
...
![Diff Secret Version](./_images/k8s_environment/diff_secret_config.png)
...
![Delete Secret Configuration](./_images/k8s_environment/delete_secret_config.png)
...
![Enable Self-Test Mode](./_images/k8s_environment/enable_base_env.png)
...
![Enable Self-Test Mode](./_images/k8s_environment/enable_base_env_1.png)
...
<img src="./_images/k8s_environment/set_self_test_env_1.png" width="400">
<img src="./_images/k8s_environment/set_self_test_env_2.png" width="400">
...
<img src="./_images/k8s_environment/create_vice_env.png" width="250">
<img src="./_images/k8s_environment/create_vice_env_1.png" width="250">
<img src="./_images/k8s_environment/create_vice_env_2.png" width="250">
...
![Self-Test and Joint Debugging Effect](./_images/k8s_environment/service_request_details.png)
...
![Self-Test and Joint Debugging Effect](./_images/k8s_environment/service_request_details_1.png)
...
![Disable Self-Test Mode](./_images/k8s_environment/disable_base_env.png)
...
<img src="./_images/k8s_environment/sleep_mode_5_310.png" width="400">
<img src="./_images/k8s_environment/sleep_mode_5_config_310.png" width="400">

## Set an Alias

Click `Basic Operations` -> `More` -> `Set Alias` and enter a new name to set an alias for the environment and display it in the Tab.

![Set an Alias](./_images/k8s_environment/prod_env_alias_demo.png)
...
![Delete the Environment](./_images/k8s_environment/delete_env.png)
