...
Feishu group notifications, Feishu member notifications, DingTalk, Microsoft Teams, and email support dynamically specified recipients. WeChat Work does not support this feature.

- Use <span v-pre>`{{.variable}}`</span>, for example <span v-pre>`{{.workflow.params.NOTIFY_EMAIL}}`</span>.
- Variables can reference `workflow`, `payload`, and `job` input or output values.
...
- Complete the [Feishu IM integration](/en/Zadig%20v5.0/settings/approval/#feishu) before using dynamic recipients for Feishu notifications.
...
![Teams Notification Configuration](./_images/workflow-im/teams_webhook_notification_3_400.png)
...
Step 1: The system administrator goes to "System Settings" -> "System Configuration" -> "Mail Configuration" to configure the sending mail server configuration. For specific configuration, refer to [Email Configuration](/en/Zadig%20v5.0/settings/system-settings/#mail-configuration).

Step 2: Edit the workflow and add an email notification.

![Email Configuration Steps](./_images/workflow-im/email_imconfig_400.png)

Parameter Description:
- `Notification Members`: You can select the workflow executor, stage executor, user, or user group. Make sure that the notification member has [configured personal email](/en/Zadig%20v5.0/preferences/#account-settings) in "Account Settings".
- `Notification Events`: Configure the notification rules, and multiple workflow statuses can be selected.
