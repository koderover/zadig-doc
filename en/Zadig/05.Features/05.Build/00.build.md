...
permalink: /en/Zadig v5.0/project/build/
---
...
<img alt="Create a build" title="Create a build" src="./_images/build/build_220.png">

- Supports configuring multiple builds for a service, and select on demand in the workflow configuration. For more details, please refer to [Workflow - Build task configuration](/en/Zadig%20v5.0/project/workflow-jobs/#%E6%9E%84%E5%BB%BA%E4%BB%BB%E5%8A%A1)
- It supports multiple services using the same build configuration. For more details, please refer to [Shared Build](/en/Zadig%20v5.0/project/build/share/)
- Supports modifying the generation rules for building deliverables. For more details, please refer to [Build deliverable generation rules](/en/Zadig%20v5.0/project/service/k8s/#policy-configuration)
...
![Build General Configuration](./_images/build/build_config_1_220.png)
...
<img alt="Create a build" title="Create a build" src="./_images/build/build_2_v180.png">

#### Using Kubernetes

The build process runs in a K8s Pod, and you can configure operating system and software package dependencies.

- `Operating System`: The platform provides by default Ubuntu 18.04 / Ubuntu 20.04 as the basic build system, it supports adding custom images as the build operating system, refer to [Build image management](/en/Zadig%20v5.0/settings/custom-image/#%E6%AD%A5%E9%AA%A4-1-%E7%94%9F%E6%88%90%E6%9E%84%E5%BB%BA%E9%95%9C%E5%83%8F)
...
::: tip
1. The default Ubuntu series build systems provided by the platform already support the following tools/commands, which can be used directly in [Software Package Management](/en/Zadig%20v5.0/settings/app/) and build scripts:
...
3. The platform has already built in some common software packages. If there are other software packages or version requirements, the system administrator can configure their installation scripts in [Software Package Management](/en/Zadig%20v5.0/settings/app)
:::
...
<img alt="Create a build" title="Create a build" src="./_images/build/build_4.png">

Parameter Description:
- `Code Source`: The platform where the code base is located. Please refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list) for supported code sources
...
Where:
- `$WORKSPACE` is the built-in variable of the Zadig system build module, for a more detailed description, refer to [System built-in environment variables](/en/Zadig%20v5.0/project/build/#build-variables)
...
![Create a build](./_images/build/build_5_420.png 'Create a build')
...
|`IMAGE`|The image name generated by the system according to the built-in rules is used for deployment updates of container services. The naming rules can be modified in the service's [Policy configuration](/en/Zadig%20v5.0/project/service/k8s/#policy-configuration)|
|`PKG_FILE`|The system can modify the naming rules of the delivery file name generated by the built-in rules, and can modify its naming rules in the service's [Policy configuration](/en/Zadig%20v5.0/project/service/k8s/#policy-configuration)|
...
<img alt="Create a build" title="Create a build" src="./_images/build/build_5_1_220.png">

**Custom Build Variables**

![Build Variable Configuration](./_images/build/build-variable.png 'Build Variable Configuration')
...
<img alt="Modify build variables when running a workflow" title="Modify build variables when running a workflow" src="./_images/build/update_env_when_run_pipeline_330.png">
...
<img alt="Create a build" title="Create a build" src="./_images/build/build_6.png">

### Advanced Configuration

<img alt="Advanced configuration" title="Advanced configuration" src="./_images/build/build_6_1_400.png">
...
- `Cluster Selection`: Select the cluster resource to be used when the build task runs, where the local cluster refers to the cluster where the Zadig system is located. For cluster integration, refer to [Cluster Management](/en/Zadig%20v5.0/pages/cluster_manage/)
...
- `Scheduling Policy`: Select the cluster scheduling policy, and the default is `Random Scheduling` policy. Please refer to [Set scheduling policy](/en/Zadig%20v5.0/pages/cluster_manage/#%E8%AE%BE%E7%BD%AE%E8%B0%83%E5%BA%A6%E7%AD%96%E7%95%A5) management
...
- `Output Variables`: Output the environment variables in the build, which can realize the variable transfer between different tasks in the workflow, refer to [Variable passing](/en/Zadig%20v5.0/project/workflow-config/#%E5%8F%98%E9%87%8F%E4%BC%A0%E9%80%92)

### More Build Steps

In `Add Steps`, you can add more build steps as needed.

<img alt="Add more build steps" title="Add more build steps" src="./_images/build/build_6_2_400.png">
...
- `Dockerfile Source`: Includes `Code Repository` and `Template Library`. For more information on the template library, see [Dockerfile Template Management](/en/Zadig%20v5.0/template/dockerfile/)
