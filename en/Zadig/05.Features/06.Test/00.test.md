...
permalink: /en/Zadig v5.0/project/test/
---

Zadig supports many test scenarios, including performance, function, interface, UI, and end-to-end automation testing. It comes with built-in mainstream testing frameworks such as jMeter and ginkgo, and can also [install software packages](/en/Zadig%20v5.0/settings/app/) to meet more diverse automation testing needs.
...
![Test Management](./_images/test/test_220.png)
...
![Test Management](./_images/test/add_test_310.png)

- `Infrastructure`: Supports executing test tasks on Kubernetes and hosts

- `Operating System`: Currently, the platform provides Ubuntu 18.04 / Ubuntu 20.04 for choice, and you can also customize the test execution environment. For details, refer to [Build Image Management](/en/Zadig%20v5.0/settings/custom-image/#%E6%AD%A5%E9%AA%A4-1-%E7%94%9F%E6%88%90%E6%9E%84%E5%BB%BA%E9%95%9C%E5%83%8F)
...
2. If there are other software packages or version requirements, the system administrator can configure the installation scripts in [Package Management](/en/Zadig%20v5.0/settings/app/)
:::

### Code Information
For test configuration code information, the code will be pulled according to the specified configuration when the test is executed. For supported code sources, refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list). For specific fields in the code information, refer to [Code information field description](/en/Zadig%20v5.0/project/build/#%E4%BB%A3%E7%A0%81%E4%BF%A1%E6%81%AF).

### Test Variables
Includes system-built-in variables and custom variables, which can be used directly in test scripts.

> Tip: Add `env` commands to [Test script](#test-scripts) to view all test variables.

![Test Management](./_images/test/test_var_420.png)
...
![Test Management](./_images/test/test_report.png)
...
![Test Management](./_images/test/upload_test_files.png)

## Advanced Configuration
![Test Management](./_images/test/senior_test_config.png)
...
![Test Management](./_images/test/export_test_files.png)
...
- `Cluster`: Select the cluster resource used when the test task runs. The local cluster refers to the cluster where the Zadig system is located. For cluster integration, refer to [Cluster Management](/en/Zadig%20v5.0/pages/cluster_manage/)
...
- `Scheduling`: Select the cluster scheduling policy, and the default is `Random Scheduling` policy. Refer to [Set scheduling policy](/en/Zadig%20v5.0/pages/cluster_manage/#%E8%AE%BE%E7%BD%AE%E8%B0%83%E5%BA%A6%E7%AD%96%E7%95%A5) management
...
### Output Variables
Output environment variables in the test can realize variable transfer between different tasks in the workflow, refer to [Variable passing](/en/Zadig%20v5.0/project/workflow-config/#%E5%8F%98%E9%87%8F%E4%BC%A0%E9%80%92).

### Webhook Trigger
Add trigger configuration to automatically trigger the specified event Webhook. Refer to [Code Source Information](/en/Zadig%20v5.0/settings/codehost/overview/#function-compatibility-list) for supported code sources.

![Test Management](./_images/test/test_webhook_config.png)
...
![WeChat Work Configuration Steps](./_images/test/test_wechat.png)
...
![DingTalk Configuration Steps](./_images/test/test_dingding_config.png)
...
![Feishu Configuration](./_images/test/test_lark_add_bot_4.png)

The notification effect is shown below. Click the link in the test results to view the HTML test report.

![Feishu Notification Effect](./_images/test/lark_webhook_notification_test.png)

## Viewing the Test Report

Click the test results link in the test task to view the test report.

![View the Test Report](./_images/test/show_test_result.png)
...
![View the Test Report](./_images/test/show_test_result_2.png)