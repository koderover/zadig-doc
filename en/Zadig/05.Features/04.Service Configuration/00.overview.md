...
permalink: /en/Zadig v5.0/project/service/overview/
---

This article introduces the fundamental concepts of Zadig services.

## Service

Services in Zadig can be understood as a set of Kubernetes resources, including Ingress, Service, Deployment/StatefulSet/DaemonSet/CronJob, InitContainer, ConfigMap, etc., or a complete Helm Chart or cloud host/physical machine service. After successful deployment, they can provide service capabilities externally.
...
![Service configuration](./_images/overview/Zadig-System-Architecture_220.png)
...
For standard Kubernetes resources such as Deployment, StatefulSet, DaemonSet, CronJob, and InitContainer, Zadig automatically identifies service components. The following figure is an example:

- `Service Component`: The name of the container in the Deployment/StatefulSet/DaemonSet/CronJob/InitContainer resource
...
![Service configuration](./_images/overview/k8s_service_module_demo.png)

For CR resources, you can declare service components in a custom way. When creating a service, paste the CR YAML and add service components. The system automatically injects the image variable `$servicename-image$`, which you can substitute at the positions that need to be updated.

<img src="./_images/overview/k8s_service_module_cr_1.png" width="400">
<img src="./_images/overview/k8s_service_module_cr_2.png" width="400">
...
![Service configuration](./_images/overview/helm_chart_service_module_demo.png)

##### Service Component Parsing Rules

Includes built-in rules and custom rules. After importing the Helm Chart configuration file, the system will parse the mirrored content in the values.yaml file according to the built-in rules. When the built-in rules do not meet the needs, you can click `Update Matching Rules` on the right side of the service component to customize the mirror resolution rules.

![Helm Chart Service Component Customization](./_images/overview/helm_chart_service_component_define_1.png)
...
The information of the service component comes from the managed service instances. Currently, it supports Deployment/StatefulSet/DaemonSet/CronJob/InitContainer type service instances. The following figure is an example:

- `Service Component`: The name of the container in the Deployment/StatefulSet/DaemonSet/CronJob/InitContainer instance
- `Current Image Version`: The image version of the managed service instance resource, i.e., the image information
- `Image Name`: Parsed from `Current Image Version`, the format is: `Repository Address/Namespace/Image Name:Tag Name`

<img src="./_images/overview/host_service_module_demo_1.png" width="400">
<img src="./_images/overview/host_service_module_demo_2.png" width="400">
...
- [Build configuration](/en/Zadig%20v5.0/project/build/#zadig-build)
- [Workflow deployment task configuration](/en/Zadig%20v5.0/project/workflow-jobs/#deployment)

2. Directly modify the service component image in the environment to update it, see [Service instance operation](/en/Zadig%20v5.0/project/env/service/#service-instance-operations) .
3. Update service components by modifying the image information in the service configuration file and updating the service in the environment. See [Update a Service](/en/Zadig%20v5.0/project/env/k8s/#update-a-service).
