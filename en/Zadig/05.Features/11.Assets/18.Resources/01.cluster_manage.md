...
permalink: /en/Zadig v5.0/pages/cluster_manage/
---
...
![cluster](./_images/cluster_manage/cluster_add_330.png)
...
- `Production Cluster`: After opening, build / Test / code scan / general script execution / custom task / grayscale release /istio release / blue and green release / canary release / K8s YAML Update / new environment selection clusters, etc. all show whether it is production.
...
- `Specify Project Scope`: Specifies which projects the cluster can be used by. `All Projects` includes subsequently created projects.
...
![cluster_add_kubectl](./_images/cluster_manage/cluster_add_kubectl_220.png)
...
![irsa](./_images/cluster_manage/aws_irsa_330.png)
...
![Modify cluster scheduling policy](./_images/cluster_manage/cluster_advanced_config_330.png)
...
Uses integrated object storage to cache resources. You can integrate object storage in [Object Storage Management](/en/Zadig%20v5.0/settings/object-storage/).
...
![Modify cluster cache resource configuration](./_images/cluster_manage/cluster_update_1_330.png)

### Using Cluster Storage

![Modify cluster cache resource configuration](./_images/cluster_manage/cluster_update_2_330.png)
...
![Modify cluster cache resource configuration](./_images/cluster_manage/cluster_share_storage_config_330.png)

### How to Use
Use shared storage resources in the workflow to realize storage resource sharing between multiple tasks. Reference document: [Configure shared storage in the workflow](/en/Zadig%20v5.0/project/workflow-config/#%E5%85%B1%E4%BA%AB%E5%AD%98%E5%82%A8) .
...
- `Storage Driver`: Storage driver configuration used by the Dind component.

![Dind resource configuration](./_images/cluster_manage/cluster_update_dind_410.png)

## Agent Configuration

Configure the deployment information related to the Agent component of the cluster.
- `Enable host network (hostNetwork)`: When enabled, the Agent component uses the host network, avoiding issues where workflow tasks cannot access host network resources due to network isolation.
...
![Agent Configuration](./_images/cluster_manage/cluster_agent_config_430.png)

## Cluster Usage

1. After connecting to an external cluster in the Zadig , you can create a new environment on the cluster and use workflow to update the new environment. The environment operation can refer to [the new environment](/en/Zadig%20v5.0/project/env/k8s/#create-a-new-environment) .
2. Supports running workflows on specified clusters / Test / code scanning tasks, specify cluster resources in its advanced configuration, refer to the documentation (for building example): [build configuration](/en/Zadig%20v5.0/project/build/#advanced-configuration) .