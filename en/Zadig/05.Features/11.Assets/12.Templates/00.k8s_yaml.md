...
permalink: /en/Zadig v5.0/template/k8s_yaml/
---

::: tip Background
<img style="width:22px; height:22px" src="./_images/k8s_yaml/k8s.svg"></img> The K8s YAML template is suitable for projects deployed with K8s YAML. It supports users in creating new services on a common template, providing greater scalability.
:::
...
![Create a K8s YAML template](./_images/k8s_yaml/create_k8s_yaml_template.png)
...
![Create a K8s YAML template](./_images/k8s_yaml/create_k8s_yaml_template_1.png)
...
In addition to supporting the use of constant values, it also supports using built-in variables in [the service configuration](/en/Zadig%20v5.0/project/service/k8s/#variables-configuration) to assign values to custom variables in templates. For example, in the following example, using `$Service$` to assign values to `cmd`.

![Advanced usage of K8s YAML template variables](./_images/k8s_yaml/furtuer_usage_of_variables_in_k8s_yaml_template.png)

## Using Templates
When creating a new service in [the K8s YAML project](/en/Zadig%20v5.0/project/k8s-yaml/), you can choose to import services from templates, and refer to [using templates to create a new service](/en/Zadig%20v5.0/project/service/k8s/#create-a-new-service).
...
![View the list of K8s YAML template references](./_images/k8s_yaml/show_k8s_yaml_template_ref.png)
...
::: tip
1. Turn on the `Automatic Synchronization` operation for the service. Refer to [the use template to create a new service](/en/Zadig%20v5.0/project/service/k8s/#create-a-new-service)
2. When custom variables are changed, the update logic for service configuration refers to [the update service configuration](/en/Zadig%20v5.0/project/service/k8s/#update-a-service-using-a-new-template)
:::

![Apply to the service](./_images/k8s_yaml/apply_k8s_template_to_service.png)
