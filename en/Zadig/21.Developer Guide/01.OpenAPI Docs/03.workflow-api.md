...
permalink: /en/Zadig v5.0/api/workflow/
---
    ...
    "stages": [                                 # Workflow task includes all stage details
        ...
        },
        {
            ...
            "jobs": [
                {
                    "name": "echo-hello",
                    "type": "freestyle",                # General Script Execution
                    "status": "passed",
                    "start_time": 1664172990,
                    "end_time": 1664172993,
                    "spec": {                           # General Script Execution detailed information
                        "repos": [
                            {
                                "source": "gitee",
                                "repo_owner": "kr-test-dev",
...
| `notify_inputs` | [][NotifyInputs](#notify_input_arg) | Notification parameters                        | None          | No             |
...
The following will introduce the trigger parameters of built-in build tasks, deployment tasks, K8s Service Deployment tasks, and general script execution separately.
...
**K8s Service Deployment tasks**

```
{
    "job_name": "deploy-my-app",                                     # K8s Service Deployment task name
    "job_type": "custom-deploy",                                     # K8s Service Deployment task type, specified as custom-deploy
    "parameters": {
...
```
{
    "job_name": "jira-issue-update",            # General Script Execution task name
    "job_type": "freestyle",                    # General Script Execution task type, specified as freestyle
    "parameters": {                             # Parameters and values in general script execution, if set to a fixed value or global variable, no need to specify
        ...
        "repo_info": [                          # Code repository information in general script execution
            {
                "codehost_name": "koderover",   # Code source identifier
                "repo_namespace": "koderover",  # Code repository organization name/username
                "repo_name": "zadig",           # Code repository name
                "branch": "task-mgr",           # Code repository branch
                "remote_name": "origin",        # Remote name, defaults to origin
                "checkout_path": "zadig",       # Repository checkout path
                "submodules": false              # Whether to check out Git submodules
            }
        ]
    }
}
...
For general script execution tasks, `repo_info` can specify any repository from a code source integrated with Zadig; the repository does not need to be configured in the task beforehand. If the field is omitted or an empty array is passed, the task's original repository configuration is retained. A non-empty array replaces the original repository list. In multi-service mode, `repo_info` is applied independently to each service.

Git checkout targets use the following priority: specified commit > branch > single PR. Use `prs` to merge multiple PRs into the specified branch in order.

The following fields can be passed in `parameters.repo_info` of a generic-script execution job via `POST /openapi/workflows/custom/task` to override the code repository configuration used when the job is executed.

<h4 id="freestyle_repo_info">General Script Execution RepoInfo Parameters</h4>

| Parameter Name   | Type   | Description                                              | Default Value | Required |
| ---------------- | ------ | -------------------------------------------------------- | ------------- | -------- |
| `codehost_name`  | string | Alias of a code source integrated with Zadig             | None          | Yes      |
| `repo_namespace` | string | Repository namespace (organization/user)                 | None          | Yes      |
| `repo_name`      | string | Repository name                                          | None          | Yes      |
| `branch`         | string | Branch name, required for branch checkout or with `prs`   | None          | Conditional |
| `pr`             | int    | Single PR number, used when no commit or branch is specified | None       | Conditional |
| `prs`            | []int  | PRs to merge in order into the branch specified by `branch` | None        | No       |
| `enable_commit`  | bool   | Whether to check out a specified commit                  | `false`       | No       |
| `commit_id`      | string | Commit ID, used when `enable_commit` is `true`            | None          | Conditional |
| `remote_name`    | string | Git remote name                                          | `origin`      | No       |
| `checkout_path`  | string | Repository checkout path                                 | None          | No       |
| `submodules`     | bool   | Whether to check out Git submodules                      | `false`       | No       |
...
<h4 id="notify_input_arg">Notify Input Parameter Description</h4>
...
Except for WeChat Work, these notification methods support dynamic recipients through `dynamic_recipients`. The field is an array of variables in the <span v-pre>`{{.variable}}`</span> format. Variables can reference `workflow`, `payload`, and `job` input or output values. The final field name must be `email`, `mobile`, `phone`, `user_id`, `userId`, or `uid`, or end with one of these names. Field names are case-insensitive.
...
```
{
    ...
    "lark_hook_notification_config": {
        ...
        "dynamic_recipients": ["{{.workflow.params.NOTIFY_EMAIL}}"],
        "is_at_all": false
    }
}
```
...
```
{
    ...
    "dingding_notification_config": {
        ...
        "dynamic_recipients": ["{{.workflow.params.NOTIFY_PHONE}}"],
        "is_at_all": false
    }
}
```
...
<img src="./_images/workflow-api/custome-workflow-yaml-demo.png" width="400">
:::
...
```
GET /openapi/logs/sse/v4/workflow/<workflowName>/<taskID>/<jobName>/<lines>
```
...
| -------------- | ------------------------------------------------------------ | ------ | -------- |
| `workflowName` | Workflow key                                                 | string | Yes      |
| `taskID`       | Workflow task ID                                             | int    | Yes      |
| `jobName`      | Workflow JobTask key, available from workflow task details   | string | Yes      |
| `lines`        | Number of log lines                                          | int    | Yes      |
...
```
GET /openapi/logs/log/v4/workflow/<workflowName>/<taskID>/<jobName>
```
