...
| `release_plan` | null | Release plan event field. It is always `null` for workflow events. |
...
| `workflow.release_plan` | object | Release plan information when the workflow is triggered by a release plan; otherwise omitted. |
| `workflow.release_plan.id` | string | Release plan ID. |
| `workflow.release_plan.name` | string | Release plan name. |
| `workflow.release_plan.index` | int | Release plan index. |
