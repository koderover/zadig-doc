...
![common_workflow_config](../../../_images/common_workflow_config_2_430.png)

The stage supports two execution modes: `Automatic` and `Manual`. Manual execution allows you to specify the stage executor and re-enter task parameters during execution.

### Task
Click `+Task` below the stage to add task configuration. For the task types and specific configurations supported by the system, see [Workflow Tasks](/en/Zadig%20v5.0/project/workflow-jobs/).
...
|  |  <span v-pre>`{{.workflow.task.id}}`</span>        | Workflow task ID, e.g. `1`                   |
| |  <span v-pre>`{{.workflow.task.creator.id}}`</span>   | Username of the workflow task executor, e.g. `admin`          |
| |  <span v-pre>`{{.workflow.task.creator}}`</span>   | Nickname of the workflow task executor, e.g. `admin-nickname`     |
| |  <span v-pre>`{{.workflow.task.is_release_plan_trigger}}`</span>   | Whether the workflow task was triggered by a release plan, e.g. `true` |
| |  <span v-pre>`{{.workflow.task.timestamp}}`</span> | Unix timestamp when the workflow task was created, e.g. `1686642562` |
| |  <span v-pre>`{{.workflow.task.datetime}}`</span> | Creation time of the workflow task, e.g. `2026-04-17 18:00:00` |
...
- `Type`: Supports string, multi-line text, single-choice, multiple-choice, file, code repository, image, and [dynamic variables](/en/Zadig%20v5.0/workflow/dynamic-variable/).
  - `Code repository` variables are only available in workflow global variables.
  - Among them, `image` and `dynamic variables` are only available in general script execution.
...
1. Use shell environment variables in build, code scanning, testing, and general script execution
...
The configuration output variables are supported in building/testing/code scanning/general script execution. The output variables of different tasks are as follows:
...
- General Script Execution: <span v-pre>`{{.job.<Task Name>.output.<Variable Name>}}`</span>
...
For tasks that support `Global Variables/Other Task Output` configuration mode, please refer to the documentation: [the configuration modes supported in different tasks](/en/Zadig%20v5.0/project/workflow-jobs/#%E4%B8%8D%E5%90%8C%E4%BB%BB%E5%8A%A1%E4%B8%AD%E6%94%AF%E6%8C%81%E7%9A%84%E9%85%8D%E7%BD%AE%E6%96%B9%E5%BC%8F) .<br>
As an example with a general script execution custom variable, specify `Global Variables/Other Task Output` when assigning a variable in general script execution, and choose the output variable configured in the preceding task to achieve variable passing between different tasks.
...
![run_job](../../../_images/run_job_01_410.png)
...
![run_job](../../../_images/run_job_02_410.png)
...
![run_job](../../../_images/run_job_03_410.png)
...
Configure triggers for workflows. When an event that meets the trigger conditions occurs, the workflow execution will be automatically triggered. It supports Git triggers, timers, JIRA triggers, Feishu project triggers, and general triggers. For details, please refer to the document: [Workflow Triggers](/en/Zadig%20v5.0/project/workflow-trigger/).
...
- `Trigger Event`: Specifies the Webhook event that triggers the workflow to run. The optional events are as follows:
    - Triggered on `Push commits` events (merge operations). Changes to the `description` field in the repository are filtered and will not trigger workflow execution.
...
Currently, it supports configuration workflow notifications to email / Feishu / Enterprise WeChat / DingTalk /Webhook in the detailed configuration parameter description can be found: [IM status notification](/en/Zadig%20v5.0/workflow/im) .
...
> Supports debugging of build tasks, test tasks, and general script execution.

Click `debug` to start the workflow and enter debugging mode. You can add breakpoint operations in the Shell script of the build/test/general script execution before execution, which facilitates debugging the workflow configuration.
...
- Branch Information
- Release Plan
- Feishu Project Work Item

![debug_wordefine_workflow_task_columnkflow](../../../_images/define_workflow_task_column_500.png)
...
Reference document: [Cluster shared storage resource configuration](/en/Zadig%20v5.0/pages/cluster_manage/#shared-storage-resource-configuration) .
...
For tasks that support advanced configuration, enable shared storage in the advanced configuration, select a shared directory, and configure the shared directory. For example, with general script execution:

> For tasks that support advanced configuration, please refer to the documentation: [Advanced Configuration](/en/Zadig%20v5.0/project/workflow-jobs/#advanced-configuration) .
