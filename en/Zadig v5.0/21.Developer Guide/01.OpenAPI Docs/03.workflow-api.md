...
All fields below can be passed in a general script execution task's `parameters.repo_info` to `POST /openapi/workflows/custom/task`. `OpenAPIFreestyleRepoInputToRepository` writes them to the repository configuration used during execution. The server selects the input parser from the actual type of the job identified by `job_name`; the request's `job_type` does not select the server-side job type.
