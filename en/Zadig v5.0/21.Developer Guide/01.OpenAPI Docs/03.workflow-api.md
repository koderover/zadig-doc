...
### Get Workflow Manual Execution Context

Returns the stage currently waiting for manual execution, its configured executors, and whether the authenticated user can execute it.

**Request**

```
GET /openapi/workflows/custom/:workflowName/task/:taskID/manual-exec?projectKey=<projectKey>
```
...
| -------------- | ------ | ---------------- | -------- |
| `workflowName` | string | Workflow key     | Yes      |
| `taskID`       | int    | Workflow task ID | Yes      |
...
| `task_id`                 | int      | Workflow task ID                                              |
| `status`                  | string   | Workflow task status; `pause` while awaiting manual execution |
| `stage_name`              | string   | Stage currently waiting for manual execution                  |
| `can_execute`             | bool     | Whether the authenticated user can execute the stage          |
| `executors`               | []object | Configured executors                                          |
| `executors[].type`        | string   | Executor type: `user`, `group`, or `task_creator`             |
| `executors[].user_id`     | string   | User UID                                                      |
| `executors[].user_name`   | string   | User name                                                     |
| `executors[].group_id`    | string   | User group ID                                                 |
| `executors[].group_name`  | string   | User group name                                               |
...
```json
{
    ...
    "executors": [
        {
            "type": "user",
            "user_id": "45eb4cf9-3715-11f1-8976-42f9e9fb45c1",
            "user_name": "admin"
        }
    ]
}
...
### Manually Execute a Workflow Stage

Executes the workflow stage currently waiting for manual execution. The authenticated user must have workflow execution permission and must be a configured executor for the stage. Zadig performs the final authorization check on the server.

**Request**

```
POST /openapi/workflows/custom/:workflowName/task/:taskID/manual-exec?projectKey=<projectKey>
```
...
| Parameter Name | Type   | Description                                      | Required |
| -------------- | ------ | ------------------------------------------------ | -------- |
| `stage_name`   | string | Stage currently waiting for manual execution     | Yes      |
...
```
PUT /openapi/workflows/custom/:name
```
