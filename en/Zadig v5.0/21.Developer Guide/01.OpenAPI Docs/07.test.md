...
permalink: /en/Zadig v5.0/api/test/
---
...
| `repo_info`    | Runtime repository configuration; defaults to the test configuration when omitted | [][RepoInfo](#RepoInfo) | No |
| `inputs`       | Runtime inputs; unspecified inputs use values from the test configuration | [][Input](#Input) | No |

<h4 id="RepoInfo">RepoInfo Parameter Description</h4>

| Parameter Name    | Description                                               | Type   | Required |
| ----------------- | --------------------------------------------------------- | ------ | -------- |
| `codehost_name`   | Code host name                                            | string | Yes      |
| `repo_namespace`  | Repository namespace (organization/user)                 | string | Yes      |
| `repo_name`       | Repository name                                           | string | Yes      |
| `branch`          | Branch name; required when `enable_commit` is `false`     | string | No       |
| `pr`              | Pull Request number                                       | int    | No       |
| `prs`             | Pull Request number list                                  | []int  | No       |
| `enable_commit`   | Whether to use a specified commit                         | bool   | No       |
| `commit_id`       | Commit ID; required when `enable_commit` is `true`        | string | No       |

Repositories in `repo_info` must already be configured in the test. Perforce repository overrides are not supported.

<h4 id="Input">Input Parameter Description</h4>

| Parameter Name | Description                                      | Type   | Required |
| -------------- | ------------------------------------------------ | ------ | -------- |
| `key`           | Input name; must already be configured in the test | string | Yes |
| `value`         | Input value                                      | string | No  |
| `type`          | Input type                                       | string | No  |
| `is_credential` | Whether the input is sensitive                   | bool   | No  |
| `file_id`       | File ID                                          | string | No  |
| `file_name`     | File name                                        | string | No  |
| `file_path`     | File path                                        | string | No  |
...
Returns the runtime input definitions for a test. The `value` of a credential input is not returned; use `has_value` to determine whether a value is configured.

**Request**

```
GET /openapi/quality/testing/:testName?projectKey=<projectKey>
```
...
| `inputs`       | [][TestInput](#TestInput) | Runtime input definitions |

<h4 id="TestInput">TestInput Parameter Description</h4>

| Parameter Name  | Type     | Description                                      |
| --------------- | -------- | ------------------------------------------------ |
| `key`           | string   | Input name                                       |
| `value`         | string   | Default value; omitted for credential inputs     |
| `type`          | string   | Input type                                       |
| `choice_option` | []string | Available choices                                |
| `required`      | bool     | Whether the input is required                    |
| `is_credential` | bool     | Whether the input contains sensitive information |
| `has_value`     | bool     | Whether a value is configured                    |
