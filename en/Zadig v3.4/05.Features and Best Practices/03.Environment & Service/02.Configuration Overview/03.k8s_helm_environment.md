...
- `Resource Selection` :
	...
	- `K8s Namespace`: Different environments will use independent Kubernetes namespaces to achieve isolation. The default rule for creating namespaces in the Zadig system is `项目名-env-环境名`. Users can also customize or select an existing namespace.
...
![Delete Helm Chart Service](../../../../_images/delete_helm_chart_service.png)
