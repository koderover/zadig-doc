...
|`scanner_type` |Code scanning tools have the following two scanning tools: `sonarQube` and `other`| string|Yes|
|`image_name` |The image name of the scan environment| string|Yes|
|`sonar_system` |SonarQube system identification| string|`scanner_type` = `sonarQube` required|
|`repo_info` |Codebase information| [][RepoInfo](#RepoInfo)|Yes|
|`addons` |Relying on software package information, just fill in the empty array if you don't need it.| [][AddOn](#AddOn)|Yes|
|`sonar_parameter` |Code scanning script| string|No|
|`script` |Code scanning script| string|No|
|`enable_quality_gate` |Whether to enable quality access control inspection| bool|Yes|
|`advanced_settings` |Advanced Configuration| [AdvancedSettings](#AdvancedSettings)|Yes|

<h4 id="!">RepoInfo Parameter description</h4>

|Parameter Name|Description|Required|Required|
|---|---|---|---|
|`codehost_name`|Code source identification|string|Yes|
|`repo_namespace`|The namespace to which the code base belongs (organization/personal)|string|Yes|
|`repo_name`|Codebase name|string|Yes|
...
|Parameter Name|Description|Required|Required|
|---|---|---|---|
|`cpu_limit`| Maximum CPU resource, unit m|int|Yes|
|`memory_limit`|Maximum memory resource, unit Mi |int|Yes|
|`cpu_request`|Minimum CPU resource, unit m|int|Yes|
|`memory_request`|Minimum memory resource, unit Mi|int|Yes|

<h4 id="!">Webhooks Parameter description</h4>

|Parameter Name|Description|Required|Required|
|---|---|---|---|
|`enabled`| Whether to enable triggers|bool|Yes|
...
|`events`| Trigger event type, optional: `push` , `pull_request` , `tag`|[]string|Yes|
|`match_folders`| File Directory|[]string|Yes|
...
| Parameter Name       | Description       | Required              | Required |
| ------------ | ---------- | ----------------- | ---- |
| `scan_repos` | Codebase information | [][Repo](#repo-1) | Yes   |
...
| `repo_name`  | Codebase name                                                   | string | Yes   |
| `repo_owner` | Organization name/user name                                                | string | Yes   |
...
| ---------- | ------ | --------------- | ---- |
| `scanName` | string | Code scan name    | Yes   |
| `taskID`   | int    | Code scanning task ID | Yes   |
