...
- `服务组件`：选择服务组件以及该服务组件使用的构建名称，点击展开构建配置的 `设置` 可配置所选中的变量值以及代码库默认分支
  - `变量`：参考 [构建任务变量](/Zadig%20v1.15.0/project/common-workflow/#构建任务-2)
...
参数说明：
- `任务名称`：支持 32 位以内的小写英文字母、数字或者中划线，且以小写英文字母开头；在同一个自定义工作流中，任务名称需唯一
- `环境`：选择要部署的环境，支持三种配置方式，具体请参考 [部署任务变量](/Zadig%20v1.15.0/project/common-workflow/#部署任务-2) 中关于环境信息的描述
- `服务`：配置要部署的服务，支持两种配置方式，具体请参考 [部署任务变量](/Zadig%20v1.15.0/project/common-workflow/#部署任务-2) 中关于服务信息的描述
- `服务状态检测`：若开启，部署任务会轮询服务运行状态，在部署超时之前该服务正常运行，任务状态才为成功

> 部署超时设置可参考 [服务策略配置](/Zadig%20v1.15.0/project/service/k8s/#策略配置)

#### 通用任务

支持拉取代码、执行 Shell 脚本、文件存储等功能。

- `执行环境`：参考 [构建环境配置](/Zadig%20v1.15.0/project/build/#构建环境)
- `代码信息`：参考 [代码信息配置](/Zadig%20v1.15.0/project/build/#代码信息)
- `变量`：参考 [通用任务变量](/Zadig%20v1.15.0/project/common-workflow/#通用任务-2)
- `添加步骤`：包括添加 Shell 脚本执行以及文件存储，可参考 [更多构建配置](/Zadig%20v1.15.0/project/build/#更多构建步骤)
- `高级配置`：参考 [高级配置](/Zadig%20v1.15.0/project/build/#高级配置)
...
#### 自定义任务
自己定义任务的实现，在工作流中配置自定义任务，详细使用方法请参考 [自定义任务](/Zadig%20v1.15.0/settings/custom-task/)。
...
变量的键和类型在此处不可更改，变量定义请阅读 [构建配置](/Zadig%20v1.15.0/project/build/#构建变量) 中的自定义构建变量部分。支持以下三种方式设置变量的值：

- `运行时输入`：执行工作流时再指定该变量的值
- `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量`：使用全局变量为构建任务中的变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.15.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.15.0/project/common-workflow/#工作流)
...
- `全局变量`：使用全局变量为环境赋值，工作流执行时会自动对全局变量进行渲染替换。全局变量包括 [项目变量](/Zadig%20v1.15.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.15.0/project/common-workflow/#工作流)
...
- `全局变量`：使用全局变量为测试任务中的变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.15.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.15.0/project/common-workflow/#工作流)
...
内置变量请参考 [构建变量](/Zadig%20v1.15.0/project/build/#构建变量) 中关于内置构建变量的描述，自定义变量说明如下：
...
- `值`：自定义变量的值，支持以下三种配置方式：
  - `运行时输入`：执行工作流时再指定该变量的值
  - `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
  - `全局变量`：使用全局变量为通用任务中的自定义变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.15.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.15.0/project/common-workflow/#工作流)

### 自定义任务
> 自定义任务中的变量。

在自定义任务的 YAML 文件中定义，YAML 定义请参考 [工作流任务](/Zadig%20v1.15.0/settings/custom-task/)，变量值的配置支持以下三种方式：
- `运行时输入`：执行工作流时再指定该变量的值
- `固定值`：指定变量的值，工作流执行时即使用该固定值，页面中该变量不可见
- `全局变量`：使用全局变量为通用任务中的自定义变量赋值，工作流执行时会自动对全局变量进行渲染。全局变量包括 [项目变量](/Zadig%20v1.15.0/project/common-workflow/#项目) 和 [工作流变量](/Zadig%20v1.15.0/project/common-workflow/#工作流)
...
- 其他参数：请参考 [触发器基本配置](/Zadig%20v1.15.0/project/workflow/#gui-方式) 中的描述
...
触发方式以及时间配置可参考 [定时器配置](/Zadig%20v1.15.0/project/workflow/#定时器)，在工作流执行变量中可设置定时运行的任务以及任务中的变量配置。
...
目前支持配置工作流通知到飞书/企业微信/钉钉中，详细的配置参数说明可参考：[IM 状态通知](/Zadig%20v1.15.0/project/workflow/#im-状态通知)。
