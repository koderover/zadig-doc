...
在项目中，点击`创建环境`按钮，选择测试环境后进入创建环境页面。（生产环境相关内容可查看：[生产环境](/ZadigX%20v1.5.0/project/env/release/)）
...
- `环境配置`：为环境配置 Ingress、ConfigMap、Secret、PVC 资源，细节描述请阅读[环境配置管理](/ZadigX%20v1.5.0/project/env/k8s/#环境配置管理)
...
可对环境中的 Ingress、ConfigMap、Secret、PVC 配置进行操作，参考[环境配置管理](/ZadigX%20v1.5.0/project/env/k8s/#环境配置管理)。

## 自测模式

参考[自测模式](/ZadigX%20v1.5.0/project/env/k8s/#自测模式)。
