...
## 托管 K8s YAML 服务
> 在 Zadig <Badge text="v1.5.0"/>及以上版本中，支持对 K8s 集群服务的托管管理。通过在 Zadig 中创建环境并托管服务，实现跨集群的服务管理。
...
> 2. 关于其他集群的配置请参考[集群管理](/pages/cluster_manage/)。
...
### 添加/删除托管服务
- 在托管项目中点击环境部分，进入环境管理页面，进行托管服务管理。
...
> [Helm](https://helm.sh/) 是 Kubernetes 应用的包管理工具，用来管理 Chart - 封装 Kubernetes 应用程序的一些列 YAML 文件。
...
Zadig 系统支持 3 种方式创建 Helm Chart 来管理 Kubernetes 服务：

- `指定 Git 仓库导入`：Helm Chart 相关配置托管于指定的 Git 仓库中，将配置从 Git 仓库中导入至 Zadig 系统内。
- `模板库导入`：Helm Chart 相关配置托管于模板库中，将配置从模板库导入至 Zadig 系统内。
- `Chart 仓库导入`：[建设中]Helm Chart 相关配置托管于 Chart 仓库中，将配置从 Chart 仓库中导入至 Zadig 系统内。

#### Git 仓库导入
- 点击 `+` 新建服务，填写代码仓库相关信息及 Helm Chart 配置文件目录路径，加载服务。
> 前提: 需要先集成代码源，参考 [GitLab 集成](/settings/codehost/gitlab)/[GitHub 集成](/settings/codehost/github)/[Gerrit 集成](/settings/codehost/gerrit)/[CodeHub 集成](/settings/codehost/codehub)
