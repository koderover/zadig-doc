...
本文主要介绍 Zadig 的构建模块。Zadig 构建模块提供模版化构建能力，构建过程通过配置构建脚本实现，简单易用。整个构建模块分成通用配置、编译环境、应用、代码库信息、敏感信息、构建脚本、镜像构建、二进制包归档和 Shell 脚本。
## 新建构建

在项目页面中点击`构建`进入构建的详情页面。
...
- `镜像编译参数:` 支持所有 Docker build 的参数，比如：`--build-arg key=value`。无需指定 `-t` 和 `-f` 参数。
