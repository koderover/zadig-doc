...
| IP                          |                              |和 DOMAIN 二选一  | 服务器 IP 地址，用于访问 Zadig 系统，后续更新该字段使用 UPDATE_ENDPOINT 变量                                                    |
...
```bash
sudo kubectl -n zadig get po
```
## 步骤 5：访问系统

![访问地址](./_images/all_in_one_get_endpoint.png)

在安装结果输出中，您可以获得系统的访问地址。

::: tip
如需执行升级过程，请使用[在现有的 Kubernetes 集群安装](/install/install-on-k8s/#步骤-4-开始安装)
:::
## 卸载
