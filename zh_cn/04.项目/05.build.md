...
2. 系统有内置的规则生成 `IMAGE` 和 `PKG_FILE`，关于规则的查看和修改，请参考：[构建交付物生成规则](/project/build/#构建交付物生成规则)。
...
- `Dockerfile 来源:` 包括`代码库`和`模板库`，模板库功能可参考 [Dockerfile 模板管理](/template/dockerfile/)
...
::: tip 小贴士
- `自定义镜像`对应[构建脚本](/project/build/#构建脚本)中的 `IMAGE` 变量
- `自定义 TAR 包`对应[构建脚本](/project/build/#构建脚本)中的 `PKG_FILE` 变量
:::
