...
permalink: /release-notes/v1.7.0/
---
...
::: danger 前提条件
如果当前系统版本 < v1.6.0，请先升级到 v1.6.0，升级方式参考文档 [v1.6.0 升级方式](/release-notes/v1.6.0/#版本升级方式)。再按照以下方式升级到v1.7.0。**不支持跨版本升级**
:::
...
#### 步骤 2：使用脚本卸载 Zadig
1. 从 GitHub/墙内下载卸载脚本: [Zadig 卸载脚本](https://github.com/koderover/zadig/releases/tag/v1.6.0)
...
2. 下载[数据同步工具](https://resources.koderover.com/dist/1.7.0-upgrade.yaml)，执行升级 Job。
    ...
    2. 执行如下命令。
    ```bash
    kubectl apply -f 1.7.0-upgrade.yaml -n <Zadig 所在namespace>
    ```
3. 确保 job 执行成功
...
1. 版本信息确认
    ...
    2. 查看右下角 info 提示中的 Version 信息，Version 信息为 1.7.0
2. 新版本正确性检查，推荐校验点
    1. 管理员访问系统中任一正常使用的项目
...
# 执行以下命令进行 port 转发
kubectl port-forward svc/zadig-mongodb -n <Zadig 所在的namespace> 27090:27017
# 对数据进行恢复
