...
permalink: /v1.14.0/project/workflow/
---
...
关于镜像版本回退，更多信息可阅读：[工作流的镜像版本回退](/v1.14.0/workflow/image/rollback/)。
...
> 服务构建配置细节可参阅[构建配置](/v1.14.0/project/build/)

![workflow](../_images/workflow_build.png)

字段说明：

- `服务`：内容的格式为`服务名/服务组件名`，点击服务可跳转查看该服务的配置详情。关于服务组件的概念可阅读[服务组件](/v1.14.0/quick-start/concepts/#服务组件)。
...
通过修改工作流的测试配置，来实现测试步骤，关于测试配置的创建和使用，请参阅[测试管理](/v1.14.0/project/test/)。
...
- 分发方式：包括镜像分发和对象存储分发
- 对象存储：构建的文件产物（一般为压缩包）被分发的目标对象存储，可在系统设置的 [对象存储](/v1.14.0/settings/object-storage) 中查阅
- 镜像仓库：构建的镜像产物被分发的目标镜像仓库，可在系统设置的 [镜像仓库](/v1.14.0/settings/image-registry/) 中查阅
...
参数说明：
- URL：包括两部分，集成的[外部系统](/v1.14.0/settings/others/)，以及自定义的 Path 部分。
...
具体操作可以参阅[工作流的触发](/v1.14.0/workflow/trigger/)部分获取更多的信息。
