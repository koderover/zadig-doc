...
permalink: /v1.14.0/project/scan/
---
...
- `扫描环境`：执行扫码任务时的具体运行环境
    ...
    - 若使用其他工具进行代码扫描，可参考[自定义镜像](/v1.14.0/settings/custom-image/)按需添加
- `Sonar 地址`：使用 `SonarQube` 工具做代码扫描时需配置该项
    - 需要事先在系统中集成 Sonar，参考文档：[Sonar 集成](/v1.14.0/settings/sonar/)
    - 当代码扫描执行完成后，Zadig 会将结果上传到此处指定的系统中

### 代码信息
> 需事先在 Zadig 中集成代码源，参考文档：[代码源集成](/v1.14.0/settings/codehost/gitlab/)。
...
- `资源配置`：选择代码扫描任务运行时所使用的集群资源，其中本地集群指 Zadig 系统所在的集群，可参考[集群管理](/v1.14.0/pages/cluster_manage/)集成更多集群资源
