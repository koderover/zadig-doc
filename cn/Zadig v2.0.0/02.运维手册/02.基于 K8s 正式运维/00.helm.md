...
permalink: /cn/Zadig v2.0.0/install/helm-deploy/
---
...
1. Kubernetes 集群，版本：v1.16 ~ v1.28，推荐配置：8C16G，且对该集群有管理员操作权限 
...
3. 网络：
    ...
    - 如果选择`域名`方式访问 Zadig 系统，需要准备可用的域名
...
### 步骤 2：备份数据以及安装参数
- 参考文档：[备份数据](/cn/Zadig%20v2.0.0/stable/backup-and-restore/#数据备份)进行数据备份。
...
helm repo update
helm upgrade {releaseName} -n {Namespace} --version=2.0.0 koderover-chart/zadig -f zadig.yaml
```
...
::: tip
如果有自己的 S3 对象存储，可以选择不安装 MinIO。在 Zadig 安装完后成访问 `系统设置` -> `对象存储` 来配置即可，配置过程详见[对象存储](/cn/Zadig%20v2.0.0/settings/object-storage/)。
:::
