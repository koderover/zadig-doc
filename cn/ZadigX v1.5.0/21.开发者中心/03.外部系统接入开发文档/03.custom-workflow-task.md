...
permalink: /cn/ZadigX v1.5.0/settings/custom-task/
---
...
1. 参考 [代码源集成](/ZadigXX%20v1.5.0/settings/codehost/overview/)，将上述代码库在 ZadigX 中集成。
2. 参考 [镜像仓库集成](/ZadigXX%20v1.5.0/settings/image-registry/)，将构建镜像对应的镜像仓库在 ZadigX 中集成。
...
配置完毕后，在自定义工作流中添加任务，选择任务后保存工作流即可，更多进阶功能可参考 [自定义工作流](/ZadigXX%20v1.5.0/project/common-workflow/)。
...
- `Jira Issue 状态变更`：结合 ZadigX 工作流阶段来变更 Jira Issue 状态，打通与项目管理的联动。源码位于 [jira-updater](https://github.com/koderover/ZadigX/tree/release-1.14.0/pkg/microservice/aslan/core/workflow/service/workflow/plugins/jira-updater)。
- `执行 Jenkins Job`：传入 Jenkins 构建参数，触发指定的 Jenkins Job 执行，源码位于 [jenkins-runner](https://github.com/koderover/ZadigX/tree/release-1.14.0/pkg/microservice/aslan/core/workflow/service/workflow/plugins/jenkins-runner)。
- `MySQL 数据库变更`：对 MySQL 数据库执行 SQL 语句，实现数据和代码变更联动变更，源码位于 [mysql-runner](https://github.com/koderover/ZadigX/tree/release-1.14.0/pkg/microservice/aslan/core/workflow/service/workflow/plugins/mysql-runner)。

此外，还开放了工作流任务共享代码库：[koderover/ZadigX-jobs](https://github.com/koderover/ZadigX-jobs)，其中包含若干 demo 实现和场景描述，供开发者使用和参考。
