...
permalink: /cn/Zadig v4.2/template/build/
---
...
依次访问`资产管理`→ `模板库` → `构建` 进入构建模板管理页面。点击 `+` 按钮，填写模板名称，参考[构建配置](/cn/Zadig%20v4.2/project/build/)完成构建模板的配置。
...
参考[使用构建模板](/cn/Zadig%20v4.2/project/build/template/)。
