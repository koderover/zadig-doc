...
permalink: /cn/Zadig v4.2/workflow/im/
---
...
第一步：在 Zadig 上进行 [飞书 IM 集成](/cn/Zadig%20v4.2/settings/approval/#飞书)。
...
第一步：系统管理员前往「系统设置」->「系统配置」-> 「邮件配置」配置发送邮件服务器配置，具体配置参考 [邮件配置](/cn/Zadig%20v4.2/settings/system-settings/#邮件配置)。
...
参数说明：
- `通知成员`：可选择工作流执行人、用户或者用户组，需确保通知成员已在「账号设置」中 [配置个人邮箱](/cn/Zadig%20v4.2/preferences/#账号设置)。
- `通知事件`：配置通知的规则，工作流状态可多选。
