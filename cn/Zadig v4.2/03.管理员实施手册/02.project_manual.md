...
permalink: /cn/Zadig v4.2/project-manual/
---

## 概述

在 Zadig 中，[项目](/cn/Zadig%20v4.2/quick-start/concepts/#项目)是核心的业务管理单元，通常用于独立可自治的团队或产品线。每个项目包括工作流、环境、服务、构建、测试、代码扫描和版本等资源。本手册涵盖了 Zadig 项目中的主要运维操作，包括项目接入，标准模板管理，服务与环境配置，工作流配置，权限和协作管理等。通过本手册，您将能够快速了解和使用 Zadig，提高配置和管理效率，快速支持研发流程。
...
3. 配置服务构建，参考 [构建](/cn/Zadig%20v4.2/project/build)。
...
::: tip
若后续需要在 Zadig 上管理服务配置，可以通过切换项目类型来实现，参考 [切换项目类型](/cn/Zadig%20v4.2/project/host-k8s-resources/#托管项目切换-k8s-yaml-项目)。
:::
...
2. 新建服务：从代码库中同步服务配置，参考 [新建服务](/cn/Zadig%20v4.2/project/service/k8s/#新建服务)。
![现有项目导入](../../_images/project_admin_7.png)

3. 为服务添加构建：选择服务 -> 变量 -> 添加构建，配置服务构建脚本
![现有项目导入](../../_images/project_admin_9_220.png)

4. 新建环境：选择已有命名空间新建环境（如：dev/sit/uat/prod 环境），参考 [新建环境](/cn/Zadig%20v4.2/project/env/k8s/#新建环境)。
...
2. 新建服务：从代码库中同步服务配置，参考 [新建服务](/cn/Zadig%20v4.2/project/service/helm/chart/#从代码库同步服务)。
...
5. 执行工作流：进入项目 -> 工作流，执行工作流更新指定环境的服务。工作流的更多功能可参考 [工作流](/cn/Zadig%20v4.2/project/common-workflow/)。
...
2. 新建服务：使用模板新建服务，参考 [新建服务](/cn/Zadig%20v4.2//project/service/k8s/#使用模板新建服务)。
...
4. 配置构建：使用构建模板配置构建脚本，参考 [构建](/cn/Zadig%20v4.2/project/build)。

<img src="../../_images/project_admin_22.png" width="400">
<img src="../../_images/project_admin_23.png" width="400">


5. 配置工作流：配置测试和生产环境对应工作流，参考 [工作流](/cn/Zadig%20v4.2/project/common-workflow/)。
...
2. 新建服务：使用模板新建服务，参考 [新建服务](/cn/Zadig%20v4.2/project/service/helm/chart/#使用模板新建单个服务)。
![新项目接入](../../_images/project_admin_27.png)

3. 新建环境：选择服务新建 dev/sit/uat/prod 环境。
![新项目接入](../../_images/project_admin_28.png)

4. 配置构建：使用构建模板配置构建脚本，参考 [构建](/cn/Zadig%20v4.2/project/build/#zadig-构建)。

<img src="../../_images/project_admin_29.png" width="400">
<img src="../../_images/project_admin_30_220.png" width="400">

5. 配置工作流：配置测试环境和生产环境对应工作流，参考 [工作流](/cn/Zadig%20v4.2/project/common-workflow/)。
...
准备工作：将多个服务的配置抽象成 K8s YAML 模板，以下为 multi-service-demo 项目的相关配置供参考。更多使用姿势可参考 [K8s YAML 模板](/cn/Zadig%20v4.2/template/k8s_yaml/)、[使用模板新建服务](/cn/Zadig%20v4.2/project/service/k8s/#使用模板新建服务)。
...
准备工作：将多个服务的 Helm Chart 配置抽象成 Helm Chart 模板，以下为 multi-service-demo 项目的相关配置供参考。更多使用姿势可参考 [Helm Chart 模板](/cn/Zadig%20v4.2/template/helm_chart/)、[使用模板新建服务](/cn/Zadig%20v4.2/project/service/helm/chart/#使用模板新建单个服务)。
...
准备工作：将多个服务的镜像构建配置抽象为 Dockerfile 模板，以下为 multi-service-demo 项目的相关配置供参考。更多使用姿势可参考 [Dockerfile 模板](/cn/Zadig%20v4.2/template/dockerfile/)、[构建配置](/cn/Zadig%20v4.2/project/build/#更多构建步骤)。
...
2. 使用构建模板：使用模板配置构建，分别配置多个服务构建所需的代码信息。对于多个服务使用同一构建模板的情况，可使用批量录入快速配置，参考 [批量录入](/cn/Zadig%20v4.2/project/build/#批量录入)。

![标准模板管理](../../_images/project_admin_template_21_220.png)

### 工作流模板

根据项目流程抽象工作流模板，基于模板快速创建工作流，具体参考 [工作流模板](/cn/Zadig%20v4.2/template/workflow/)。
...
第 1 步：配置测试服务
1. 基于 K8s  YAML 模板，新建业务服务，参考 [服务](/cn/Zadig%20v4.2/project/service/k8s/#使用模板新建服务)。
![业务运维操作](../../_images/daily_admin_1.png)

2. 基于构建模板，为服务配置构建，参考 [构建](/cn/Zadig%20v4.2/project/build/template/)。

::: tip 优化建议：
|    建议项   |                说明                           |                    参考              |
|:----------- | :-------------------------------------------- |:-----------------------------------------|
| 构建过程提速 | 1. 把常用构建的依赖工具打进去<br> 2. 自定义缓存目录| 1. [自定义构建镜像](/cn/Zadig%20v4.2/settings/custom-image/)<br> 2. [配置构建缓存](/cn/Zadig%20v4.2/project/build/#高级配置) |
| 减少维护负担 | 1. 多个服务构建相似，配置一份构建脚本，减少维护成本<br> 2. 规范并统一服务构建物料的管理方式，减少维护和管理负担| 1. [共享构建脚本](/cn/Zadig%20v4.2/project/build/#Zadig-共享构建)<br> 2. [使用构建模板](/cn/Zadig%20v4.2/project/build/template/) |
:::

![业务运维操作](../../_images/project_admin_template_21_220.png)

第 2 步：添加到环境，参考 [K8s YAML 环境](/cn/Zadig%20v4.2/project/env/k8s/#基本操作)。
...
1. 删除服务定义，参考 [删除服务](/cn/Zadig%20v4.2/project/service/k8s/#删除服务)。
![业务运维操作](../../_images/daily_admin_5.png)

2. 从环境中删除服务，参考 [K8s YAML 环境](/cn/Zadig%20v4.2/project/env/k8s/#删除服务)。
![业务运维操作](../../_images/daily_admin_6_220.png)

### 新建测试环境

进入项目 -> 环境 -> 新建环境 -> 测试环境，填写参数后新建。
- K8s YAML 项目，参考 [新建 K8s YAML 环境](/cn/Zadig%20v4.2/project/env/k8s/#新建环境)。
- K8s Helm Chart 项目，参考 [新建 Helm Chart 环境](/cn/Zadig%20v4.2/project/env/helm/chart/#新建环境)。
![业务运维操作](../../_images/daily_admin_7.png)
![业务运维操作](../../_images/daily_admin_8.png)

提供部分最佳实践供参考：
1. 使用一套服务配置创建多套测试环境 ，参考 [配置多套集成环境](/cn/Zadig%20v4.2/env/multi-env/)。
2. 不同环境使用不同结构的 deployment，可使用变量能力，参考 [K8s YAML 服务](/cn/Zadig%20v4.2/project/service/k8s/#变量配置)。
3. 不同业务线可以使用不同的 K8s 集群资源，新增集群配置参考 [集群管理](/cn/Zadig%20v4.2/pages/cluster_manage/)。
4. 环境配置管理，参考 [环境配置](/cn/Zadig%20v4.2/project/env/k8s/#环境配置)。

### 上线生产服务

第 1 步：配置生产服务，参考 [生产服务](/cn/Zadig%20v4.2/project/service/k8s/prod/#新建)。
![业务运维操作](../../_images/daily_admin_9.png)

第 2 步：配置生产环境，参考 [K8s YAML 生产环境](/cn/Zadig%20v4.2/project/env/release/)。
![业务运维操作](../../_images/daily_admin_10.png)

第 3 步：配置工作流，添加部署任务，参考 [工作流](/cn/Zadig%20v4.2/project/workflow-jobs/#部署任务)。
...
参考 [下线服务](/cn/Zadig%20v4.2/project/workflow-jobs/#下线服务)。

### 新建生产环境

参考 [生产环境](/cn/Zadig%20v4.2/project/env/release/#新建环境)。

## 工作流配置

参考 [工作流](/cn/Zadig%20v4.2/project/workflow-overview/)
...
访问项目配置 -> 权限，配置角色和项目成员即可，参考 [权限配置](/cn/Zadig%20v4.2/project/permission/#权限配置)。
...
访问权限管理 -> 协作模式，配置协作成员以及工作流、环境资源的权限即可，具体配置过程参考 [协作模式](/cn/Zadig%20v4.2/project/permission/#协作模式)。
