...
```
GET /openapi/environments/<环境标识>/pods?projectKey=<项目标识>
```
...
```
GET /openapi/environments/production/<环境标识>/pods?projectKey=<项目标识>
```
...
```
GET /openapi/environments/<环境标识>/pods/<Pod 名称>?projectKey=<项目标识>
```
...
```
GET /openapi/environments/production/<环境标识>/pods/<Pod 名称>?projectKey=<项目标识>
```
...
::: tip
1. 适用于 K8s YAML/Helm 项目。
2. 调用该接口时，Zadig 会删除指定 Pod，并由其所属工作负载自动拉起新的 Pod，以实现单个 Pod 重启。
3. 建议先调用“查看 Pod 列表”或“查看 Pod 详情”接口确认目标 Pod 名称，再执行重启操作。
:::
...
```
POST /openapi/environments/<环境标识>/pods/<Pod 名称>/restart?projectKey=<项目标识>
```
...
```
POST /openapi/environments/production/<环境标识>/pods/<Pod 名称>/restart?projectKey=<项目标识>
```
