...
permalink: /cn/Zadig v4.3/api/env/
---
...
| `sub_env`          | 子环境       | [][SubEnv](#SubEnv)                 | 否   |

<h4 id="SubEnv">子环境参数说明</h4>

| 参数名     | 说明           | 类型   | 必填 |
| ---------- | -------------- | ------ | ---- |
| `base_env` | 基础环境名称   | string | 是   |
| `enable`   | 是否启用子环境 | bool   | 是   |
...
```
POST /openapi/environments/helm?projectKey=<项目标识>
```
...
| 参数名        | 说明           | 类型                  | 必填 |
| ------------- | -------------- | --------------------- | ---- |
| `env_name`    | 环境标识       | string                | 是   |
| `alias`       | 环境名称       | string                | 否   |
| `cluster_id`  | K8s 集群 ID    | string                | 是   |
| `namespace`   | K8s 命名空间   | string                | 是   |
| `registry_id` | 镜像仓库 ID    | string                | 是   |
| `project_key` | 项目标识       | string                | 是   |
| `production`  | 是否为生产环境 | bool                  | 否   |
| `sub_env`     | 子环境         | [][SubEnv](#SubEnv-1) | 否   |

<h4 id="SubEnv-1">子环境参数说明</h4>
...
```
POST /openapi/environments/helm/<环境名称>/services?projectKey=<项目标识>
```
...
| `env_name`   | 环境标识       | string                        | 是   |
| `production` | 是否为生产环境 | bool                          | 否   |
| `services`   | Helm 服务      | [][HelmService](#HelmService) | 是   |

<h4 id="HelmService">Helm 服务参数说明</h4>

| 参数名                   | 说明                            | 类型                                        | 必填 |
| ------------------------ | ------------------------------- | ------------------------------------------- | ---- |
| `service_name`           | 服务名称                        | string                                      | 是   |
| `deploy_strategy`        | 部署策略，支持 import 和 deploy | string                                      | 是   |
| `values_yaml`            | values yaml 内容                | string                                      | 否   |
| `override_kvs`           | 覆盖 KV 变量列表                | [][OverrideKV](#OverrideKV)                 | 否   |
| `import_values_from_git` | 从 Git 导入 values 配置         | [ImportValuesFromGit](#ImportValuesFromGit) | 否   |

<h4 id="OverrideKV">OverrideKV 参数说明</h4>
...
<h4 id="ImportValuesFromGit">ImportValuesFromGit 参数说明</h4>
...
| `value_path`    | values 文件路径 | string | 是   |
| `auto_sync`     | 自动同步        | bool   | 否   |
...
| `not_delete_resource` | 是否不删除 K8S 资源        | bool     | 否   |
...
::: tip
1. 适用于 K8s YAML 项目。
2. 该接口用于获取指定服务当前关联的 Pod 信息。
3. 若要重启单个 Pod，建议先调用该接口获取目标 `pod_name`，再调用下方“重启单个 Pod”接口。
:::
...
```
GET /openapi/environments/<环境标识>/service/<服务名称>/pods?projectKey=<项目标识>
```
...
```json
{
  ...
  "pods": [
    {
      "pod_name": "service1-5b94c8f98f-hsqfl",
      "status": "Running",
      "pod_ready": true,
      "containers_ready": true,
      "create_time": 1704444315,
      "ip": "10.200.2.76",
      "images": [
        "koderover.tencentcloudcr.com/test/service1:20231214162411"
      ],
      "workload_name": "service1",
      "workload_type": "Deployment"
    }
  ]
}
...
```
GET /openapi/environments/production/<环境标识>/service/<服务名称>/pods?projectKey=<项目标识>
```
...
::: tip
1. 适用于 K8s YAML 项目。
2. 该接口会校验目标 Pod 是否存在，然后删除该 Pod，由所属工作负载自动拉起新的 Pod，以实现单个 Pod 重启。
3. 接口路径同时包含环境名、服务名和 Pod 名，因此推荐先通过“查看服务 Pod 列表”接口确认 `pod_name`。
:::
...
```
POST /openapi/environments/<环境标识>/service/<服务名称>/pod/<Pod 名称>/restart?projectKey=<项目标识>
```
...
```
POST /openapi/environments/production/<环境标识>/service/<服务名称>/pod/<Pod 名称>/restart?projectKey=<项目标识>
```
