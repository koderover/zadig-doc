...
permalink: /cn/Zadig v4.3/install/helm-deploy/
---
...
### 步骤 2：备份数据以及安装参数
- 参考 [备份数据](/cn/Zadig%20v4.3/stable/backup-and-restore/#数据备份)进行数据备份。
...
helm repo update
helm upgrade {releaseName} -n {Namespace} --version=4.3.0 koderover-chart/zadig -f zadig.yaml
```
...
::: tip
如果有自己的 S3 对象存储，可以选择不安装 MinIO。在 Zadig 安装完后进入系统配置即可，配置过程详见[对象存储](/cn/Zadig%20v4.3/settings/object-storage/)。
:::
