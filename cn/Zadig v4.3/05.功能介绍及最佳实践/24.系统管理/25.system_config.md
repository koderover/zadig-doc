...
- `task_id`：工作流任务 ID。
- `project_name`：项目标识。
- `project_display_name`：项目名称。
- `workflow_name`：工作流标识。
- `workflow_display_name`：工作流名称。
- `status`：当前任务状态。
- `remark`：执行备注。
- `detail_url`：任务详情链接。
- `error`：错误信息。
...
- `end_time`：任务结束时间。
- `stages`：阶段与任务明细。
- `task_creator`：任务创建者名称。
- `task_creator_id`：任务创建者 ID。
- `task_creator_email`：任务创建者邮箱。
- `task_type`：任务类型，当前为 `workflow`。
