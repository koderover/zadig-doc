...
```
POST /openapi/service/loader/load/:codehostId?repoName=<代码库名称>&branchName=<分支名称>&remoteName=<远端名称>&repoOwner=<仓库拥有者>&namespace=<仓库命名空间>&production=<是否生产服务>
```
...
从代码源 `1` 的 `main` 分支为项目 `demo` 新建测试服务：
...
| `publicRepo`       | 无需接入代码源的公开 GitHub 代码库                                 |
...
从代码源 `1` 的 `main` 分支为项目 `demo` 新建 Helm 服务：
