...
```
POST /openapi/service/loader/load/:codehostId?repoName=<代码库名称>&repoUUID=<代码库唯一标识>&branchName=<分支名称>&remoteName=<远端名称>&repoOwner=<仓库拥有者>&namespace=<仓库命名空间>&production=<是否生产服务>
```
...
| 参数名       | 类型   | 描述                                                   | 必填 |
| ------------ | ------ | ------------------------------------------------------ | ---- |
| `repoName`   | string | 代码库名称，当前服务加载逻辑使用该字段                 | 是   |
| `repoUUID`   | string | 接口接收该字段并用于非空校验，但当前加载逻辑不使用，不能替代 `repoName` | 否   |
