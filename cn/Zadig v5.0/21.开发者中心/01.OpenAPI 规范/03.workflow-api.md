...
除企业微信外，以上通知方式均支持通过 `dynamic_recipients` 动态指定收件人。字段值为变量数组，变量格式为 <span v-pre>`{{.变量名}}`</span>；支持 `workflow`、`payload` 变量，以及 `job` 任务输入、输出变量。变量名的末级字段需为 `email`、`mobile`、`phone`、`user_id`、`userId`、`uid`，或以上述名称作为后缀；字段名不区分大小写。
...
| `concurrency_limit` | int               | 工作流任务并发数，-1 表示无限制，0 表示不可执行 |
| `stages`            | [][Stage](#Stage) | 阶段配置                                        |
