...
permalink: /cn/Zadig v5.0/api/workflow/
---
...
| `notify_inputs` | [][NotifyInputs](#notify_input_arg) | 通知的具体参数       | 无     | 否       |
...
下面将分开介绍内置构建任务、部署任务、K8s 服务部署任务、通用脚本执行的触发参数。
        ...
        "service_list": [                                 # 要构建的服务信息
            {
                ...
                "repo_info": [{                           # 构建该服务组件的代码库信息
                    ...
                    "branch": "main",                     # 代码库分支
                    "enable_commit": true,                # 是否开启使用 commit id 构建
                    "commit_id": "e1f10409e99c3f13b0777d2cd8b1a42750b093e1"    # 构建时使用的 commit id，仅当 enable_commit 为 true 时生效
                }]
...
```
{
    "job_name": "deploy-my-app",                                     # K8s 服务部署任务名称
    "job_type": "custom-deploy",                                     # K8s 服务部署任务类型，指定为 custom-deploy
    "parameters": {
...
```
{
    "job_name": "jira-issue-update",            # 通用脚本执行任务名称
    "job_type": "freestyle",                    # 通用脚本执行任务类型，指定为 freestyle
    "parameters": {                             # 通用脚本执行中的参数和值，若设置为固定值或全局变量，则无需指定
        // 多服务参数，当执行方式为多服务执行时，使用该字段
        "services": [
            {
                "service_name": "service1",
                "service_module": "service1",
...
```
{
    ...
    "job_type": "zadig-test",                             # 测试任务类型，指定为 zadig-test
    "parameters": {
        // 多服务参数，当执行方式为多服务执行时，使用该字段
        "service_list": [
            {
...
```
{
    ...
    "parameters": {
        ...
        "iteration_ids": ["1143976133001000001", "1143976133001000002"],     # Tapd 迭代 ids
    }
}
```
...
<h4 id="notify_input_arg">Notify Input 参数说明</h4>

下面将分开介绍飞书群组通知（自定义机器人）、飞书群组通知（自建应用）、飞书成员通知、钉钉、企业微信、Teams、邮件通知的触发参数。

**飞书群组通知（自定义机器人）**
```
{
    "id": 1,             // 工作流配置中第几个通知，从 0 开始
    "type": "feishu",
    "lark_hook_notification_config": {
        "at_users": ["35xxxx91"], // 飞书用户 ID，id_type 为 user_id
        "is_at_all": false
    }
}
```
...
```
{
    ...
    "lark_group_notification_config": {
        ...
        "at_users": [
            {
                "id": "35xxxx91",
                "id_type": "user_id"       # 支持 user_id 和 open_id 类型
            }
        ]
    }
}
...
```
{
    ...
    "mail_notification_config": {
        "user_ids": ["1234567890", "0987654321"]         // Zadig 的用户 ID
    }
}
```
...
查询处于手动执行等待状态的工作流任务，返回当前等待执行的阶段、配置的执行人，以及当前用户是否可以执行。

**请求**

```
GET /openapi/workflows/custom/:workflowName/task/:taskID/manual-exec?projectKey=<项目标识>
```
...
| 参数名         | 类型   | 描述       | 必填 |
| -------------- | ------ | ---------- | ---- |
| `workflowName` | string | 工作流标识 | 是   |
| `taskID`       | int    | 工作流任务 ID | 是 |
...
| `status`                   | string     | 工作流任务状态，等待手动执行时为 `pause`            |
| `stage_name`               | string     | 当前等待手动执行的阶段名称                          |
| `can_execute`              | bool       | 当前用户是否可以执行该阶段                          |
| `executors`                | []object   | 配置的执行人列表                                    |
| `executors[].type`         | string     | 执行人类型：`user`、`group` 或 `task_creator`       |
| `executors[].user_id`      | string     | 用户 UID                                            |
| `executors[].user_name`    | string     | 用户名称                                            |
| `executors[].group_id`     | string     | 用户组 ID                                           |
| `executors[].group_name`   | string     | 用户组名称                                          |
...
执行当前处于手动等待状态的工作流阶段。调用用户除了需要具备工作流执行权限，还必须是该阶段配置的执行人；最终权限由 Zadig 服务端校验。

**请求**

```
POST /openapi/workflows/custom/:workflowName/task/:taskID/manual-exec?projectKey=<项目标识>
```
...
| 参数名       | 类型   | 描述                         | 必填 |
| ------------ | ------ | ---------------------------- | ---- |
| `stage_name` | string | 当前等待手动执行的阶段名称   | 是   |
...
::: tip
该接口接收 JSON 格式的工作流定义，参数相对复杂，自己组装比较费力。推荐先在页面中创建工作流 -> 在配置页面点击 `YAML` -> 复制 YAML 内容后借助[在线转换工具](https://onlineyamltools.com/convert-yaml-to-json)来快速获得参数。
...
参考[创建工作流参数](/cn/Zadig%20v5.0/api/workflow/#创建工作流)
...
| 参数名         | 说明                                         | 类型   | 必填 |
| -------------- | -------------------------------------------- | ------ | ---- |
| `workflowName` | 工作流标识                                   | string | 是   |
| `taskID`       | 工作流任务 ID                                | int    | 是   |
| `jobName`      | 工作流 JobTask 标识，可从工作流任务详情获取 | string | 是   |
| `lines`        | 日志行数                                     | int    | 是   |
