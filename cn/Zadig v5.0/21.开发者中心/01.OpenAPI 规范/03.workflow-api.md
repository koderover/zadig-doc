        ...
        "repo_info": [                          # 通用脚本执行中的代码库信息
            {
                ...
                "remote_name": "origin",        # 远端名称，默认为 origin
                "checkout_path": "zadig",       # 代码库检出路径
...
通用脚本执行任务的 `repo_info` 支持指定已接入代码源中的任意代码库，无需提前在任务中配置。未传或传入空数组时，使用任务中原有的代码库配置；传入非空数组时，使用请求中的列表替换原有配置。多服务执行时，每个服务分别应用其 `repo_info` 配置。

Git 检出目标的优先级为：指定 Commit > 分支 > 单个 PR。`prs` 用于在指定分支上依次合并多个 PR。

下列字段均可通过 `POST /openapi/workflows/custom/task` 的通用脚本执行任务 `parameters.repo_info` 传入，并由 `OpenAPIFreestyleRepoInputToRepository` 写入执行使用的代码库配置。是否按通用脚本执行任务解析取决于工作流中该 `job_name` 对应任务的实际类型，请求中的 `job_type` 不参与服务端任务类型选择。

<h4 id="freestyle_repo_info">通用脚本执行任务 RepoInfo 参数说明</h4>

| 参数名           | 类型   | 描述                                                   | 默认值   | 是否必须 |
| ---------------- | ------ | ------------------------------------------------------ | -------- | -------- |
| `codehost_name`  | string | 已接入 Zadig 的代码源别名                              | 无       | 是       |
| `repo_namespace` | string | 代码库所属命名空间（组织/个人）                        | 无       | 是       |
| `repo_name`      | string | 代码库名称                                             | 无       | 是       |
| `branch`         | string | 分支名称，按分支检出或使用 `prs` 时传入                | 无       | 条件必填 |
| `pr`             | int    | 单个 PR 编号，未指定 Commit 和分支时生效               | 无       | 条件必填 |
| `prs`            | []int  | PR 编号列表，在 `branch` 指定的分支上依次合并这些 PR    | 无       | 否       |
| `enable_commit`  | bool   | 是否检出指定 Commit                                    | `false`  | 否       |
| `commit_id`      | string | Commit ID，`enable_commit` 为 `true` 时使用             | 无       | 条件必填 |
| `remote_name`    | string | Git 远端名称                                           | `origin` | 否       |
| `checkout_path`  | string | 代码库检出路径                                         | 无       | 否       |
| `submodules`     | bool   | 是否检出 Git 子模块                                    | `false`  | 否       |
