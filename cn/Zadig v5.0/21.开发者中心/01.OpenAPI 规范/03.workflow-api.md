...
自由任务的 `repo_info` 支持指定已接入代码源中的任意代码库，无需提前在任务中配置。未传或传入空数组时，使用任务中原有的代码库配置；传入非空数组时，使用请求中的列表替换原有配置。多服务执行时，每个服务分别应用其 `repo_info` 配置。

Git 检出目标的优先级为：指定 Commit > 分支 > 单个 PR。`prs` 用于在指定分支上依次合并多个 PR。

下列字段均可通过 `POST /openapi/workflows/custom/task` 的自由任务 `parameters.repo_info` 传入，并由 `OpenAPIFreestyleRepoInputToRepository` 写入执行使用的代码库配置。是否按自由任务解析取决于工作流中该 `job_name` 对应任务的实际类型，请求中的 `job_type` 不参与服务端任务类型选择。

<h4 id="freestyle_repo_info">自由任务 RepoInfo 参数说明</h4>
