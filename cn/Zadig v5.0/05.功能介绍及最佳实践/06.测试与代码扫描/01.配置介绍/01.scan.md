...
- `扫描环境`：执行扫码任务时的具体运行环境
    ...
    - 若使用其他工具进行代码扫描，可参考[自定义镜像](/cn/Zadig%20v5.0/settings/custom-image/)按需添加
...
- `Sonar 地址`：使用 `SonarQube` 工具做代码扫描时需配置该项
    - 需要事先在系统中集成 Sonar，参考 [Sonar 集成](/cn/Zadig%20v5.0/settings/sonar/)
...
更多实践参考[Sonar 扫描最佳实践](/cn/Zadig%20v5.0/sonar-scan/practice/)。
