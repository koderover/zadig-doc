...
permalink: /cn/ZadigX dev/template/build/
---
...
依次访问`资产管理`-> `模板库` -> `构建` 进入构建模板管理页面。点击 `+` 按钮，填写模板名称，参考[构建配置](/ZadigX%20dev/project/build/)完成构建模板的配置。
...
使用构建模板创建构建时，按照顺序完成对应代码库的配置即可，参考[使用构建模板](/ZadigX%20dev/project/build/#zadig-构建模板)。
:::


## 使用模板

参考[使用构建模板](/ZadigX%20dev/project/build/#zadig-构建模板)。
