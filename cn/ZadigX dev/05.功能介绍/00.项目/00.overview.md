...
permalink: /cn/ZadigX dev/project/overview/
---
...
- `访问权限`：可选`公开`或`私有`，定义该项目的默认权限。
    - 公开项目：用户默认享有 `read-only` 角色中的权限，可查看项目中的资源。该角色的细节请阅[权限配置](/ZadigX%20dev/project/permission/#权限配置)。
...
项目管理员访问项目 -> 配置 -> 系统集成，为项目集成代码源等资源。资源集成后仅在项目内可用，实现项目之间的资源隔离。

![系统集成](../../../_images/system_integration_in_project.png)

### 代码源

支持集成 GitLab、GitHub、Gerrit、Gitee(社区版)、Gitee(企业版)等代码源，具体集成参数配置可参考文档 [代码源集成](/ZadigX%20dev/settings/codehost/overview/)。
