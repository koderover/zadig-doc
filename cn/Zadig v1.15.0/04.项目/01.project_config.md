...
permalink: /cn/Zadig v1.15.0/project/config/
---
...
- `访问权限`：可选`公开`或`私有`，定义该项目的默认权限。
    - 公开项目：用户默认享有 `read-only` 角色中的权限，可查看项目中的资源。该角色的细节请阅[权限配置](/cn/Zadig%20v1.15.0/project/config/#权限配置)。
...
- 角色：一系列权限的集合，在[角色管理](/cn/Zadig%20v1.15.0/project/config/#角色管理)中可查看角色中的权限细节。
- 策略：工作流/环境资源粒度的权限集合，对私有项目适用。在[策略管理](/cn/Zadig%20v1.15.0/project/config/#策略管理)中可查看该策略的权资源和限细节。
...
陈述各个权限项和功能的对应关系，各功能模块中完整的功能文档分别参见：[工作流](/cn/Zadig%20v1.15.0/project/workflow/)/[环境](/cn/Zadig%20v1.15.0/project/env/k8s)/[服务](/cn/Zadig%20v1.15.0/project/service/k8s/)/[构建](/cn/Zadig%20v1.15.0/project/build/)/[测试](/cn/Zadig%20v1.15.0/project/test/)/[版本管理](/cn/Zadig%20v1.15.0/project/version/)/[代码扫描](/cn/Zadig%20v1.15.0/project/scan/)。
...
- 选择参与成员，若需要添加的用户不在成员列表中，可在[成员管理](/cn/Zadig%20v1.15.0/project/config/#成员管理)中添加，并选择 `read-project-only` 角色。
...
主机项目中，项目管理员点击`配置` -> `主机管理`可在项目中对主机资源进行管理，具体配置可参考 [添加主机](/cn/Zadig%20v1.15.0/settings/vm-management/#添加主机)。
