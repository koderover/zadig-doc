...
permalink: /cn/v1.9.0/install/helm-deploy/
---
...
helm upgrade --install zadig koderover-chart/zadig --namespace ${NAMESPACE} --version=1.9.0 --set endpoint.FQDN=${DOMAIN} \
...
helm upgrade --install zadig koderover-chart/zadig --namespace ${NAMESPACE} --version=1.9.0 --set endpoint.type=IP \
    --set endpoint.IP=${IP} \
...
> 后续可在访问系统 `系统设置` -> `对象存储` 配置，配置过程详见[对象存储](/v1.9.0/settings/object-storage/)
