...
permalink: /cn/v1.9.0/faq/troubleshooting-guide/
---
...
这个是 Zadig 系统的 spock-dind 服务挂载的 pvc 的空间不足导致的，可以使用我们系统设置-系统配置-缓存清理功能一键清理这些数据，详见[缓存清理](/v1.9.0/settings/system-settings/)
...
在 Zadig 1.3.0 及以上版本中该接口已下线，在[工作流里配置触发器](/v1.9.0/project/workflow/#git-webhook)后无需再在 GitLab 系统中手动添加 Webhook 设置。
...
编辑具体的工作流，切换到顶栏的`触发器`查看 Webhook 配置细节，确保代码的变更及 Webhook 事件符合触发条件。配置中各参数说明可参考 [Webhook 配置](/v1.9.0/project/workflow/#git-webhook)。
...
> 更多信息可阅读 [工作流的镜像版本回退](/v1.9.0/workflow/image/rollback/)
