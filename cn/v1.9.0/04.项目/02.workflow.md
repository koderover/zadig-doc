...
permalink: /cn/v1.9.0/project/workflow/
---
...
关于镜像版本回退，更多信息可阅读：[工作流的镜像版本回退](/v1.9.0/workflow/image/rollback/)。
...
> 服务构建配置细节可参阅[构建配置](/v1.9.0/project/build/)
...
通过修改工作流的测试配置，来实现测试步骤，关于测试配置的创建和使用，请参阅[测试管理](/v1.9.0/project/test/)
...
::: tip 前提
镜像的分发支持需要管理员提前正确配置[`镜像仓库`](/v1.9.0/settings/image-registry/)<br>
存储空间的分发支持需要管理员提前正确配置[`对象存储`](/v1.9.0/settings/object-storage/)
:::
...
- 添加服务：要被执行分发的目标服务
- 镜像仓库：构建的镜像产物被分发的目标镜像仓库，可在系统设置的 [镜像仓库](/v1.9.0/settings/image-registry/) 中查阅
- 对象存储：构建的文件产物（一般为压缩包）被分发的目标对象存储，可在系统设置的 [对象存储](/v1.9.0/settings/object-storage) 中查阅
...
- `代码库`：需要监听触发事件的代码仓库。
- `YAML 文件路径`：YAML 文件在代码库中存放的路径，比如图中 YAML 文件在 go-micro 库中的路径为 ci/zadig-trigger.yaml。

触发器 YAML 描述文件语法：

``` yaml
# 定义触发的步骤
# YAML 文件中一级关键字 build、deploy、test 必须保留
stages:
  - build
  - deploy
  - test
...
具体操作可以参阅[工作流的触发](/v1.9.0/workflow/trigger/)部分获取更多的信息。
