...
permalink: /cn/Zadig v5.0/settings/custom-task/
---
...
> 支持的代码源请参考 [代码源信息](/cn/Zadig%20v5.0/settings/codehost/overview/)。
...
outputs:                            # 输出变量，在 Zadig 工作流中可使用
  - name: RET                       # 变量名称
    description: "description"      # 变量描述
...
1. 参考 [代码源集成](/cn/Zadig%20v5.0/settings/codehost/overview/)，将上述代码库在 Zadig 中集成。
2. 参考 [镜像仓库集成](/cn/Zadig%20v5.0/settings/image-registry/)，将构建镜像对应的镜像仓库在 Zadig 中集成。
...
配置完毕后，在工作流中添加任务，选择任务后保存工作流即可，更多进阶功能可参考 [工作流](/cn/Zadig%20v5.0/project/workflow-config/)。
...
参数说明：
- `name`：任务名称，如下图所示。
- `category`：任务类型，包含build（构建）、deploy（部署）、test（验证）、project-manager（项目协同）、config-change（配置变更）、data-change（数据变更）、release（发布与回滚）。其他字段可在双引号内自行输入：`"这是一个自定义的字段"`
- `version`：任务的版本号
...
- `name`： 变量名称
- `description`：描述信息
- `type`：变量类型，支持类型 string（单行文本）、text（多行文本） choice（单选）以及 script（脚本）
- `choice_option`：type 为 choice 类型时，可选的变量值
- `script`：type 为 script 类型时，脚本内容
- `call_function`：type 为 script 类型时，调用函数
- `default`：变量默认值
...
- `Jenkins Job 执行`：传入 Jenkins 构建参数，触发指定的 Jenkins Job 执行，源码位于 [jenkins-runner](https://github.com/koderover/zadig/tree/release-1.14.0/pkg/microservice/aslan/core/workflow/service/workflow/plugins/jenkins-runner)。
