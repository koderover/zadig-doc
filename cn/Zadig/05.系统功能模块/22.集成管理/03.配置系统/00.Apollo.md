...
permalink: /cn/Zadig v5.0/settings/configsystem/apollo/
---
...
![Apollo配置](./_images/Apollo/apollo_config_01.png)

创建成功后复制下方的 Token。

![配置管理](./_images/Apollo/apollo_config_02.png)

### 第二步：在 Zadig 中集成 Apollo
访问 `系统设置`-> `系统集成` -> `配置管理`，添加 Apollo 配置并通过系统连通性校验即可。

![Apollo配置](./_images/Apollo/apollo_config_03.png)
...
在工作流中编排 Apollo 配置变更任务即可实现 Apollo 配置自动变更，参考：[Apollo 配置变更](/cn/Zadig%20v5.0/project/workflow-jobs/#apollo-配置变更)。
