...
permalink: /cn/Zadig v5.0/settings/codehost/gerrit/
---
...
Gerrit 管理员可以通过点击 Gerrit 右上角的 `Settings` 按钮，进入设置页面。找到 HTTP 密码 `HTTP Credentials` 配置区域，点击生成新密码 `GENERATE NEW PASSWORD`，并复制新生成的密码。

![gerrit-settings](./_images/gerrit/gerrit1.png)
![gerrit-generate-password](./_images/gerrit/gerrit2.png)

### 步骤 2：将配置填入 Zadig

切换到 Zadig 系统，管理员依次点击 `系统设置` -> `系统集成` -> `代码源` -> 点击添加按钮。

![gerrit-integration](./_images/gerrit/gerrit3.png)
