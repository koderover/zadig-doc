...
permalink: /cn/Zadig v5.0/settings/account/ldap/
---

LDAP（轻量目录访问协议）是一个网络协议，用于访问和查找用户和用户组信息。
...
![ad](./_images/ldap/user_account_ad.png)
...
| 用户过滤器         | 用户过滤器，关于 LDAP Filter 参考 [https://ldap.com/ldap-filters/](https://ldap.com/ldap-filters/)|
| 用户名属性（用于登录）|用于登录的用户名称属性|
| 用户昵称属性|用户昵称属性，选填|
| 用户邮箱属性|用户邮箱属性|
|**组规则**||
| 组基础 DN | 根专有名称（DN）将会在你在目录服务器上运行查询的时候使用到。 |
| 组过滤器   | 组过滤器，关于 LDAP Filter 参考 [https://ldap.com/ldap-filters/](https://ldap.com/ldap-filters/)|  
| 组名称属性 | 组名称属性 |
...
![default_user_system](./_images/ldap/default_user_system.png)

设置后，将会使用默认账号系统对应的登录页作为系统默认登录页。

![default_login_page](./_images/ldap/default_login_page.png)
