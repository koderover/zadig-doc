...
permalink: /cn/Zadig v5.0/pages/cluster_manage/
---
...
- `连接方式`：支持`代理连接`和`直接连接`两种方式，具体操作参考 [代理连接方式](#代理连接)、[直接连接方式](#直接连接)
- `名称`：集群名称只支持小写字母、数字和中划线
- `集群提供商`：支持`阿里云 ACK`、`腾讯云 TKE`、`腾讯云 TKE Serverless`、`华为云 CCE`、`火山引擎 VKE`、`Amazon EKS`、`微软云 AKS`、`谷歌云 GKE`、`Oracle OKE` 和其他自建的 Kubernetes 集群
- `描述`： 该集群的用途描述信息
- `生产集群`：开启后，构建/测试/代码扫描/通用脚本执行/自定义任务/灰度发布/istio 发布/蓝绿发布/金丝雀发布/K8s YAML 更新/新建环境选择集群等均显示是否为生产
- `KubeConfig`：待接入集群的 KubeConfig 配置文件内容，使用`直接连接`方式时需填写
- `指定项目范围`：指定集群可被哪些项目使用，其中`所有项目`包括后续新建项目
...
![修改集群调度策略](./_images/cluster_manage/cluster_advanced_config_330.png)
...
将会使用已集成的对象存储来缓存资源，可在[对象存储管理](/cn/Zadig%20v5.0/settings/object-storage/)中集成对象存储。

::: tip
缓存的上传/下载受网络情况影响，使用对象存储作为缓存介质时并不一定能提高构建效率，更多最佳实践可参考：[Zadig 构建缓存配置指南](https://www.koderover.com/blog/build-cache/)。
:::

![修改集群缓存资源配置](./_images/cluster_manage/cluster_update_1_330.png)

### 使用集群存储

![修改集群缓存资源配置](./_images/cluster_manage/cluster_update_2_330.png)
...
![修改集群缓存资源配置](./_images/cluster_manage/cluster_share_storage_config_330.png)

### 如何使用
在工作流中使用共享存储资源，实现多个任务之间的存储资源共享，参考 [在工作流中配置共享存储](/cn/Zadig%20v5.0/project/workflow-config/#共享存储)。
...
- `存储驱动`：Dind 组件使用的存储驱动配置

![Dind 资源配置](./_images/cluster_manage/cluster_update_dind_410.png)

## Agent 配置

配置集群的 Agent 组件相关部署信息。
- `启用宿主网络(hostNetwork)`：启用宿主网络，Agent 组件将使用宿主机网络，避免因网络隔离导致的工作流任务无法访问宿主机网络资源的问题
- `节点选择器`：配置节点选择器，指定 Agent 组件部署在哪些节点上
- `容忍度`：配置容忍度，允许 Agent 组件调度到带有污点的节点上
- `亲和性`：配置亲和性规则，控制 Agent 组件的调度策略

![Agent 配置](./_images/cluster_manage/cluster_agent_config_430.png)

## 集群使用

1. 在 Zadig 系统中接入外部集群后，便可以在该集群上创建新环境，并使用工作流对新环境进行更新操作，环境操作参考 [新建环境](/cn/Zadig%20v5.0/project/env/k8s/#新建环境)
2. 支持在指定集群上运行工作流/测试/代码扫描任务，在其高级配置中指定集群资源即可，参考（以构建示例）：[构建配置](/cn/Zadig%20v5.0/project/build/#高级配置)
