...
permalink: /cn/Zadig v5.0/template/build/
---
...
依次访问`资产管理`→ `模板库` → `构建` 进入构建模板管理页面。点击 `+` 按钮，填写模板名称，参考[构建配置](/cn/Zadig%20v5.0/project/build/)完成构建模板的配置。

> 模板中的`代码信息`部分无需配置，在使用构建模板为服务创建构建时再配置。

![构建模板](./_images/build/build_template_1.png)
...
参考[使用构建模板](/cn/Zadig%20v5.0/project/build/template/)。
