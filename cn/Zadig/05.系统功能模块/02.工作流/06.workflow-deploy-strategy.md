...
permalink: /cn/Zadig v5.0/project/release-workflow/
---
...
MSE 全链路灰度场景需基于网关能力建设，需在集群中安装 MSE 组件并根据自身情况选择合适的网关，以 MSE 自带的云原生网关为例：

- 安装 MSE 组件，参考 [安装 ack-onepilot](https://help.aliyun.com/zh/mse/user-guide/enable-microservices-governance-for-microservice-applications-in-an-ack-cluster-1)
- 安装 MSE Ingress 云原生网关，参考 [创建 MSE 云原生网关](https://help.aliyun.com/zh/mse/getting-started/create-a-cloud-native-gateway)、[通过 MSE Ingress 访问容器服务](https://help.aliyun.com/zh/mse/user-guide/use-mse-ingresses-to-access-applications-in-ack-clusters#task-2229308)
...
- `灰度环境`：将要进行灰度发布的环境
- `镜像仓库`：灰度镜像所在的镜像仓库
- `灰度标`：设置灰度标，后续执行 MSE 灰度服务下线时将会自动删除带有这些灰度标的资源
- `状态检测`：若开启，灰度发布任务会轮询灰度服务运行状态，待服务正常运行后任务状态才为成功

在工作流中添加「MSE 灰度服务下线」任务并配置灰度环境。
...
![配置工作流](./_images/workflow-deploy-strategy/run_mse_workflow.png)
...
灰度服务验证无误后，启动工作流，执行 MSE 灰度服务下线任务，选择需要下线的灰度标即可自动下线对应的灰度服务。
...
![新建工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_1_310.png)

2. 配置蓝绿环境部署任务。选择「蓝绿环境部署」任务并配置。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_2_310.png)
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_2_0.png)

3. 配置验证任务，验证蓝环境质量是否符合上线要求。选择「check」任务并配置。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_3_310.png)

4. 配置审批任务。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_4_310.png)

5. 配置蓝绿发布任务。选择「蓝绿发布」任务并配置。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_5_310.png)

参数说明：
- `选择「蓝绿环境部署」任务`：选择前置的蓝绿环境部署任务
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_blue_green_7.png)
...
![新建工作流](./_images/workflow-deploy-strategy/release_workflow_canary_1_310.png)

2. 配置金丝雀环境部署任务。选择「金丝雀环境部署」任务并配置。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_canary_2_310.png)
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_canary_3_310.png)

4. 配置审批任务。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_canary_4_310.png)

5. 配置金丝雀发布任务。选择「金丝雀发布」并配置。

![配置工作流](./_images/workflow-deploy-strategy/release_workflow_canary_5_310.png)
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_canary_11.png)

- 人工审批通过并完成金丝雀发布。此时环境已更新为金丝雀版本，如下图所示。
  
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_canary_13.png)
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_gray_3.png)
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_gray_4.png)

- 工作流按照设置的任务完成执行，执行状态如下图所示。

![执行工作流](./_images/workflow-deploy-strategy/release_workflow_gray_6.png)
...
![执行工作流](./_images/workflow-deploy-strategy/release_workflow_gray_rollback_5.png)
...
![配置工作流](./_images/workflow-deploy-strategy/release_workflow_istio_3.png)
...
![执行工作流](./_images/workflow-deploy-strategy/release_workflow_istio_5.png)

- 工作流按照设置的任务完成执行，执行状态如下图所示。

![执行工作流](./_images/workflow-deploy-strategy/release_workflow_istio_6.png)

## Istio 回滚

如果在 Istio 发布过程中出现不预期的中断或者需要回滚到发布之前的状态，可以使用「Istio 回滚」任务。
...
### 如何使用
#### 配置 Istio 回滚
1. 编辑工作流，添加「Istio 回滚」任务：点击「+ 任务」，选择「Istio 回滚」任务并配置。
...
![执行工作流](./_images/workflow-deploy-strategy/release_workflow_istio_rollback_3.png)

- 工作流按照设置的任务完成执行，执行状态如下图所示。

![执行工作流](./_images/workflow-deploy-strategy/release_workflow_istio_rollback_5.png)
