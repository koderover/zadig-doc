...
飞书群组通知、飞书成员通知、钉钉、Microsoft Teams 和邮件支持通过变量动态指定收件人，企业微信暂不支持。

- 变量格式为 <span v-pre>`{{.变量名}}`</span>，例如 <span v-pre>`{{.workflow.params.NOTIFY_EMAIL}}`</span>。
- 支持引用 `workflow`、`payload` 变量，以及 `job` 任务输入、输出变量。
...
![teams 通知配置](./_images/workflow-im/teams_webhook_notification_3_400.png)
...
![邮件配置步骤](./_images/workflow-im/email_imconfig_400.png)

参数说明：
- `通知成员`：可选择工作流执行人/阶段执行人、用户或者用户组，需确保通知成员已在「账号设置」中 [配置个人邮箱](/cn/Zadig%20v5.0/preferences/#账号设置)。
- `通知事件`：配置通知的规则，工作流状态可多选。
