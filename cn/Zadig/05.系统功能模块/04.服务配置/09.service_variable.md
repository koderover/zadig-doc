...
permalink: /cn/Zadig v5.0/project/service/variable/
---

本文介绍 K8s YAML 项目中的服务变量功能。
...
![变量](./_images/service_variable/var_500.png)

### 系统内置变量
包括 `$Namespace$`、`$Product$`、`$Service$`、`$EnvName$`、`$ClusterName$`，可直接在 YAML 中进行引用，具体说明如下：
- `$Namespace$`：项目创建的环境所在的 K8s 空间名称
- `$Product$`：项目名称
- `$Service$`：服务名称
- `$EnvName$`：创建的环境名称
- `$ClusterName$`：创建的环境所在的 K8s 集群名称
...
![变量](./_images/service_variable/var1.png)

### 全局变量
点击自定义变量右侧的`配置全局变量`进行配置。

![变量](./_images/service_variable/public_var.png)

添加全局变量，默认可选的键来自于所有服务的自定义变量（键名相同的变量视为同一个），也可以输入自定义键名来添加。

![变量](./_images/service_variable/public_var_1.png)
...
![新建环境变量渲染](./_images/service_variable/var_create_env.png)
...
- [数据库隔离](/cn/Zadig%20v5.0/env/multi-env/#数据库隔离)
- [域名区分](/cn/Zadig%20v5.0/env/multi-env/#差异化域名访问)
