...
permalink: /cn/Zadig v5.0/project/service/overview/
---
...
Zadig 中的服务可以理解为一组 Kubernetes 资源，包括 Ingress、Service、Deployment/StatefulSet/DaemonSet/CronJob、InitContainer、ConfigMap 等，也可以是一个完整的 Helm Chart 或者云主机/物理机服务，成功部署后可对外提供服务能力。
...
![服务配置](./_images/overview/Zadig-System-Architecture_220.png)
...
对于 Deployment、StatefulSet、DaemonSet、CronJob、InitContainer 等标准 Kubernetes 资源，Zadig 会自动识别服务组件，以下图为例：

- `服务组件`：Deployment/StatefulSet/DaemonSet/CronJob/InitContainer 资源中，container 的名称
- `当前镜像版本`：服务组件的 image 信息
- `镜像名`：从`当前镜像版本`中解析得出，解析格式为：`仓库地址/命名空间/镜像名:标签名`

![服务配置](./_images/overview/k8s_service_module_demo.png)

若服务使用 CR 资源，可通过自定义方式声明服务组件。新建服务时贴入 CR YAML，添加服务组件后，系统会自动注入镜像变量 `$servicename-image$`，将其替换到需要更新镜像的位置即可。
...
- `当前镜像版本`：系统按照一定的规则对 values.yaml 文件做解析获得镜像信息，解析规则可参考 [解析规则](#服务组件解析规则)
- `服务组件`：和`镜像名`相同
- `镜像名`：从`当前镜像版本`中解析得出，解析格式为：`仓库地址/命名空间/镜像名:标签名`

![服务配置](./_images/overview/helm_chart_service_module_demo.png)

##### 服务组件解析规则

包括内置规则和自定义规则，导入 Helm Chart 配置文件后，系统会按照内置规则解析 values.yaml 文件中的镜像内容。当内置规则不满足需求时，可点击服务组件右侧的`更新匹配规则`来自定义镜像解析规则。

![Helm Chart 服务组件自定义](./_images/overview/helm_chart_service_component_define_1.png)
...
服务组件的信息来源于被托管的服务实例，目前支持 Deployment/StatefulSet/DaemonSet/CronJob/InitContainer 类型的服务实例，以下图为例：

- `服务组件`：Deployment/StatefulSet/DaemonSet/CronJob/InitContainer 实例中，container 的名称
...
- [构建配置](/cn/Zadig%20v5.0/project/build/#zadig-构建)
- [工作流部署任务配置](/cn/Zadig%20v5.0/project/workflow-jobs/#部署任务)

2. 在环境中直接修改服务组件镜像来对其进行更新，参考 [服务实例操作](/cn/Zadig%20v5.0/project/env/service/#服务实例操作)。
3. 通过修改服务配置文件中的镜像信息，并在环境中更新服务来实现服务组件的更新，参考 [更新服务](/cn/Zadig%20v5.0/project/env/k8s/#更新服务)。
