...
permalink: /cn/Zadig v5.0/project/service/k8s/
---
...
![新建服务](./_images/k8s_service/service.png)
...
![新建服务](./_images/k8s_service/service_1.png)

添加服务成功后即可对环境进行更新：点击`环境更新`，选择要更新的环境，将服务加入到环境中。

![更新环境](./_images/k8s_service/service_4.png)
...
![从代码库同步新建服务](./_images/k8s_service/sync_k8s_service_from_repo.png)

同步后，效果如下：

![从代码库同步新建服务效果](./_images/k8s_service/sync_k8s_service_from_repo_effect.png)
...
![从代码库批量同步新建服务](./_images/k8s_service/sync_multi_k8s_service_from_repo.png)

同步后，效果如下：

![从代码库批量同步新建服务效果](./_images/k8s_service/sync_multi_k8s_service_from_repo_effect.png)

### 使用模板新建服务

::: tip 前提
需要先在系统模板库里创建 K8s YAML 模板，参考 [K8s YAML 模板管理](/cn/Zadig%20v5.0/template/k8s_yaml/)。
:::
...
![使用 K8s YAML 模板](./_images/k8s_service/use_k8s_yaml_template_1.png)
...
- 点击`添加服务`后填写服务名称，选择资源类型及对应的配置，目前支持选择 deployment/statefulset/daemonset/cronjob/service/ingress/secret/configmap/pvc 资源进行导入

![从 K8s 导入](./_images/k8s_service/create_k8s_service_from_k8s.png)
...
![从 K8s 导入](./_images/k8s_service/update_k8s_service_config_from_k8s.png)
...
![修改服务](./_images/k8s_service/service_5.png)

服务修改成功后即可对环境进行更新：点击环境更新，选择要更新的环境对环境中的服务进行更新。

![更新环境中的服务](./_images/k8s_service/service_4.png)

### 更新从代码库同步的服务

* 提交服务配置变更到代码仓库。

![配置变更](./_images/k8s_service/service_upgrade_git.png)

* 变更合并后，会通过 Webhook 的能力自动同步最新配置到 Zadig 系统。也可以在界面上手动同步服务配置，如下图所示。

![服务手动配置同步](./_images/k8s_service/service_upgrade_manual_update.png)

* 在环境中，查看服务配置变更，点击更新服务钮执行更新操作。

![服务版本diff](./_images/k8s_service/service_upgrade_diff.png)
![服务更新](./_images/k8s_service/service_upgrade.png)
...
![配置变更](./_images/k8s_service/update_service_created_by_template_1.png)

![配置变更](./_images/k8s_service/update_service_created_by_template_2.png)
...
![配置变更](./_images/k8s_service/update_service_created_by_template.png)

### 更新从 Kubernetes 导入的服务

点击从 Kubernetes 导入的服务即可预览该服务配置的详细内容，修改内容后保存即可。

![更新服务配置](./_images/k8s_service/update_service_config_from_k8s.png)

## 删除服务

点击服务右侧的删除按钮即可将服务删除。

![删除服务](./_images/k8s_service/delete_service.png)

## 服务版本回滚

参考文档： [服务版本回滚](/cn/Zadig%20v5.0/project/service/versions/)。

## 服务组件

![服务组件](./_images/k8s_service/k8s_container.png)

参考文档：[服务组件](/cn/Zadig%20v5.0/project/service/overview/#什么是服务组件)。
...
![K8s 服务编排](./_images/k8s_service/k8s_service_orchestration.png)
![K8s 服务编排](./_images/k8s_service/k8s_service_orchestration_1.png)
...
通过变量配置，实现多个环境共享一份服务配置，具体配置和使用参考[文档](/cn/Zadig%20v5.0/project/service/variable/)。

## 策略配置
在策略中设置部署服务的超时时间、服务配置更新后是否自动同步更新环境以及交付物命名规则。

![服务策略配置](./_images/k8s_service/service_strategy_config.png)
...
- 交付物命名规则设置：自定义该项目中工作流构建产物的命名规则，可通过以下[内置变量](#内置变量)和常量组合的方式设置，注意：
  ...
  <!-- - `JENKINS 生成镜像规则`适用于使用 [Jenkins 构建](/cn/Zadig%20v5.0/project/build/#jenkins-构建) 的服务，不可使用下述[内置变量](#内置变量)中的 `REPO_*` 变量。 -->
