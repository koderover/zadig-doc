...
permalink: /cn/Zadig v5.0/release/plan/
---

本文主要介绍如何使用 Zadig 的发布计划功能，包括发布计划的规划、审批和执行，以及追踪详细的操作记录。

发布计划支持列表模式和日历模式两种视图，日历模式可以更清晰查看发布计划的时间安排。

![发布计划](./_images/release-plan/release_plan_calendar_view.png)

## 规划
发布管理 -> 发布计划，新建发布计划并填写内容后保存草稿即可，支持多人协同合作规划发布计划。
...
- `发布内容`：具体的发布内容，可创建多个，每一个发布项可指定负责人，可指定负责人可操作执行。发布项包含富文本和工作流两种类型
    - `富文本类型`：输入内容
    - `工作流类型`：选择 Zadig 中的项目和工作流，按需配置工作流参数
- `审批配置`：按需配置审批，支持 Zadig、飞书、钉钉三种审批方式
    - `Zadig`：审批申请将会发送到 Zadig 账号对应的邮箱中，具体审批配置可参考 [Zadig 审批](/cn/Zadig%20v5.0/workflow/approval/#zadigx-审批/)
    - `飞书`：审批申请将会发送到飞书中，具体审批配置可参考 [飞书审批](/cn/Zadig%20v5.0/workflow/approval/飞书审批/)
    - `钉钉`：审批申请将会发送到钉钉中，具体审批配置可参考 [钉钉审批](/cn/Zadig%20v5.0/workflow/approval/钉钉审批/)
...
![发布计划](./_images/release-plan/release_plan_3.png)
...
![发布计划](./_images/release-plan/release_plan_4.png)
...
系统中详细记录了发布计划的操作日志，以及信息变更，点击`操作记录`即可查看。

![发布计划](./_images/release-plan/release_plan_7.png)
...
| `id`            | string         | 发布任务 id                                                                                                                                                                  |
| `name`          | string         | 发布任务名称                                                                                                                                                                 |
| `manager`       | string         | 发布任务负责人                                                                                                                                                               |
| `manager_id`    | string         | 发布任务负责人 ID                                                                                                                                                            |
...
| `tapd`                   | [WorkflowTapdJobSpec](#hook-workflow-tapd-job-spec)                        | Tapd 任务 Spec    |
...
<h4 id="hook-workflow-tapd-job-spec">WorkflowTapdJobSpec 参数说明</h4>
...
| `source_status` | string                                  | 来源状态，可选值为：open/done |
| `status`        | string                                  | 目标状态，可选值为：open/done |
| `iterations`    | [][Iteration](#hook-workflow-iteration) | 迭代列表                      |

<h4 id="hook-workflow-iteration">Iteration 参数说明</h4>
...
| `result`          | string | 结果，枚举值：success, failed, executing                        | 是       | 无     |
| `failed_reason`   | string | 失败原因                                                        | 否       | 无     |
| `description`     | string | 描述，当结果为 executing 时使用                                 | 否       | 无     |
