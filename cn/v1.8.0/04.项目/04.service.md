...
permalink: /cn/v1.8.0/project/service/
---
...
- `仓库托管`：服务的相关 K8s YAML 配置文件托管在代码仓库中，可通过在 Zadig 平台配置好 YAML 文件的目录路径后导入该部分配置。支持两种方式来从代码仓库中同步服务的 K8s YAML 配置文件：
  ...
  - 自动同步：通过配置 Webhook（参阅 [Webhook 配置](/v1.8.0/settings/webhook-config/)），Zadig 监听分支上有代码改动，对应的服务配置会被自动同步。
...
::: tip 前提
需要先在系统模板库里创建 K8s YAML 模板，请参考 [K8s YAML 模板管理](/v1.8.0/template/k8s_yaml/)
:::
...
2. 在 K8s YAML 模板中定义变量并赋予系统支持的变量值，见下图中的 `IngressPrefix` 变量和 `CPULimitsInTemplate` 变量。系统支持被赋值的变量可参考[变量配置](/v1.8.0/project/service/#变量配置)。
:::
...
::: tip 前提
需要先集成代码源，参考 [GitLab 集成](/v1.8.0/settings/codehost/gitlab/)/[GitHub 集成](/v1.8.0/settings/codehost/github/)
:::
...
::: tip 前提
需要先在系统模板库里创建 Helm Chart 模板，请参考 [Helm Chart 模板管理](/v1.8.0/template/helm_chart/)
:::
...
主机服务的构建脚本相关配置说明，参考[构建模块](/v1.8.0/project/build/)。
