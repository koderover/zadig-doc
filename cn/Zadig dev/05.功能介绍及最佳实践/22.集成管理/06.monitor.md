...
permalink: /cn/dev/settings/monitor/
---
...
在自定义工作流中配置`观测云监测` 任务即可使用观测云监测服务是否健康，参考文档：[观测云监测](/dev/project/workflow-jobs/#观测云监测)。
...
在自定义工作流中配置 `Grafana 服务监测任务`即可使用 Grafana 监测服务是否健康，参考文档：[Grafana 监测](/dev/project/workflow-jobs/#grafana-监测)。
