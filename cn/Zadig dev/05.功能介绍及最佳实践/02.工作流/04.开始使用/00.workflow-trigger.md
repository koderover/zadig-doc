...
permalink: /cn/dev/project/workflow-trigger/
---
...
- 其他参数：请参考 [Git 触发器基本配置](/dev/project/workflow/#自动创建)中的描述
...
3. 参考 [Git 触发器基本配置](/dev/project/workflow/#自动创建)，填写 Zadig 中的相关配置后保存即可。
...
2. 支持的代码源请参考：[代码源信息](/dev/settings/codehost/overview/#功能兼容列表)。
:::
...
- 其他参数：参考[定时器配置](/dev/project/workflow/#定时器)
...
通过调用 OpenAPI 来触发工作流，具体操作请参阅[执行工作流](/dev/api/workflow/#执行工作流)。