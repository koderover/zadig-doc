...
permalink: /cn/dev/project/workflow/
---
...
#### 运行策略
- `并发运行`：参考文档 [工作流任务并发运行](/dev/workflow/parallel/)。
...
- `服务`：内容的格式为`服务名/服务组件名`，点击服务可跳转查看该服务的配置详情。关于服务组件的概念可阅读[服务组件](/dev/env/overview/#什么是服务组件)。
...
支持将工作流执行情况通知到钉钉、企业微信、飞书，具体如何使用可参考文档：[工作流通知](/dev/workflow/im/)。
