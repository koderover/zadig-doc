...
2. 基于构建模板，为服务配置构建，参考文档：[构建](/dev/project/build/template/)。
...
2. 从环境中删除服务，参考文档：[K8s YAML 环境](/dev/project/env/k8s/#删除服务)。
...
第 2 步：配置生产环境，参考文档：[K8s YAML 生产环境](/dev/project/env/release/)。
