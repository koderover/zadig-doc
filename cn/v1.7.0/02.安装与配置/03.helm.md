...
permalink: /cn/v1.7.0/install/helm-deploy/
---
...
```bash
helm upgrade --install zadig koderover-chart/zadig --namespace <NAMESPACE> --version=1.7.0 --set endpoint.FQDN=<your domain> \
    --set global.extensions.extAuth.extauthzServerRef.namespace=<NAMESPACE>
    --set dex.config.issuer=http://<your domain>/dex
...
```bash
helm upgrade --install zadig koderover-chart/zadig --namespace <NAMESPACE> --version=1.7.0 --set endpoint.type=IP \
    --set endpoint.IP=<IP> \
    --set ingress-nginx.controller.service.nodePorts.http=<30000~32767任一端口> \
    --set global.extensions.extAuth.extauthzServerRef.namespace=<NAMESPACE>
    --set dex.config.issuer=http://<IP>:<PORT>/dex
...
| Key                                      | 说明                                                              | Value 示例   | 默认值   |
|------------------------------------------|-------------------------------------------------------------------|--------------|----------|
| gloo.gatewayProxies.gatewayProxy.service.type | Gateway Proxy 服务的暴露方式，可选项为 LoadBalancer、NodePort、ClusterIP，可选项为 LoadBalancer 或者 NodePort | LoadBalancer | LoadBalancer |
...
> 后续可在访问系统 `系统设置` -> `对象存储` 配置，配置过程详见[对象存储](/v1.7.0/settings/object-storage/)
...
- 不安装内置 MySQL
> 如果你有自己的 MySQL，可以选择不安装内置的 MySQL，版本要求 5.7 及以上，具体配置如下。
