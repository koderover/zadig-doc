...
permalink: /cn/v1.7.0/release-notes/v1.7.0/
---
...
2. <font color=#FF000 >**不支持跨版本升级**</font>，如果当前 Zadig 系统版本 < v1.6.0，请先参考 [v1.6.0 升级方式](/v1.7.0/release-notes/v1.6.0/#版本升级方式) 升级到 v1.6.0，再按照以下方式升级到 v1.7.0。
...
# 配置必要的参数，具体参考[基于现有 Kubernetes 安装](/v1.7.0/install/install-on-k8s/)
./install.sh
```
...
#### 失败回滚流程
1. 卸载 Zadig （方式：[步骤 2](#基于现有-k8s-集群-高可用数据库的升级流程)）
2. 重新安装 Zadig v1.6.0
...
#### 失败回滚流程
1. 卸载 Zadig （方式：[步骤 2](#基于现有-k8s-集群-使用-zadig-内置-mongodb-的升级流程)）
2. 重新安装 Zadig v1.6.0
> 注：在安装完成后先不要进入系统，等待数据恢复
3. 回滚数据至备份版本
```bash
...
#### 失败回滚流程
1. 卸载 Zadig （方式：[步骤 2](#基于-helm-命令-高可用数据库的升级流程)）
2. 重新安装 Zadig v1.6.0
> 注：在安装完成后先不要进入系统，等待数据恢复
3. 回滚数据至备份版本
```bash
...
#### 失败回滚流程
1. 卸载 Zadig （方式：[步骤 2](#基于-helm-命令-使用-zadig-内置-mongodb-的升级流程)）
2. 重新安装 Zadig v1.6.0
> 注：在安装完成后先不要进入系统，等待数据恢复
3. 回滚数据至备份版本
```bash
