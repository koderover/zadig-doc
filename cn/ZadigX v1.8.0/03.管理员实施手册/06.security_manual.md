...
permalink: /cn/ZadigX v1.8.0/security-manual/
---
...
新增代码扫描，选择 `SonarQube` 扫描工具，填写代码扫描配置，参考文档：[代码扫描](/ZadigX%20v1.8.0/project/scan/)。

![安全服务接入手册](../../_images/security_manual_0.png)

**第二步：在工作流中编排代码扫描**

编辑工作流，按需在指定阶段（比如：构建任务之前）添加代码扫描任务，参考文档：[配置工作流](/ZadigX%20v1.8.0/project/common-workflow/)。
...
1. 构建自定义镜像并上传到镜像仓库中，参考文档：[自定义镜像](/ZadigX%20v1.8.0/settings/custom-image/#步骤-1-生成自定义镜像)。
...
新增`测试`，并完成 Fortify SCA 代码扫描的具体配置，相关参数说明如下，更细节的介绍可参考文档：[测试](/ZadigX%20v1.8.0/project/test/)。
...
在工作流中编排 SCA 扫描，参考文档：[编排任务](/ZadigX%20v1.8.0/project/workflow-jobs/#测试任务)。
...
::: tip
可以在服务的 YAML 配置内容中灵活使用 ZadigX 的服务变量能力，按需控制是否需要为服务安装 Agent 进行动态安全检测，参考文档：[服务变量](/ZadigX%20v1.8.0/project/service/k8s/#变量配置)。
:::
...
将注入 IAST Agent 的服务部署到环境中，参考文档：[添加服务](/ZadigX%20v1.8.0/project/env/k8s/#添加服务)。
...
2. 灵活编排 MSE 灰度、蓝绿、金丝雀、分批次灰度、Istio 等发布策略，以确保发布可靠性，可参考：[发布策略](/ZadigX%20v1.8.0/project/release-workflow/)。
