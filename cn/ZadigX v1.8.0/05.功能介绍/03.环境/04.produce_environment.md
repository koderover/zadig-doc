...
permalink: /cn/ZadigX v1.8.0/project/env/release/
---
...
环境创建完成后，点击不同的环境 Tab 即可切换到该环境的详情页面，生产环境右上角会有`生产`标识。更多信息请参考测试环境中的描述：[查看环境](/ZadigX%20v1.8.0/project/env/helm/chart/#查看环境)。

![环境详情](../../../_images/show_helm_chart_prod_env.png)

## 新建环境

参考：[新建环境](/ZadigX%20v1.8.0/project/prod/env/k8s/#新建环境)。

## 更新全局变量

对环境中的全局变量进行更新，具体操作参考测试环境中的描述：[更新全局变量](/ZadigX%20v1.8.0/project/env/helm/chart/#更新全局变量)。

## 管理服务

对环境中的服务进行管理，具体操作参考测试环境中的描述：[管理服务](/ZadigX%20v1.8.0/project/env/helm/chart/#管理服务)。
...
具体操作可参考文档 [添加 ZadigX 服务](/ZadigX%20v1.8.0/project/env/helm/chart/#添加服务)。
...
参考文档：[更新 ZadigX 服务](/ZadigX%20v1.8.0/project/env/helm/chart/#更新服务)。
...
参考文档：[删除服务](/ZadigX%20v1.8.0/project/env/helm/chart/#删除服务)。

## 服务回滚

支持一键将 Chart 回滚到指定版本，具体操作请参考测试环境中的描述：[服务回滚](/ZadigX%20v1.8.0/project/env/helm/chart/#服务回滚)。

## 环境配置管理

对环境中的 Ingress、ConfigMap、Secret、PVC 配置进行管理，具体操作参考测试环境中的描述：[环境配置管理](/ZadigX%20v1.8.0/project/env/helm/chart/#环境配置管理)。

## 设置别名

参考：[设置别名](/ZadigX%20v1.8.0/project/prod/env/k8s/#设置别名)。
