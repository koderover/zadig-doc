...
permalink: /cn/Zadig v1.17.0/project/env/helm/chart/
---
...
- 环境创建成功后，此处新建的配置即会在环境/集群中生效，也可以在环境创建完成后再管理环境配置，参考[环境配置管理](/cn/Zadig%20v1.17.0/project/env/k8s/#环境配置管理)
...
- `环境配置`：为环境配置 Ingress、ConfigMap、Secret、PVC 资源，细节描述请阅读[环境配置管理](/cn/Zadig%20v1.17.0/project/env/k8s/#环境配置管理)
...
- 可手动输入 values 内容
- 从代码库导入 values 文件
- 使用变量组，变量组管理参考[变量组配置](/cn/Zadig%20v1.17.0/project/config/#变量组) <Badge text="企业版"/>
...
可对环境中的 Ingress、ConfigMap、Secret、PVC 配置进行操作，参考[环境配置管理](/cn/Zadig%20v1.17.0/project/env/k8s/#环境配置管理)。

## 自测模式

参考[自测模式](/cn/Zadig%20v1.17.0/project/env/k8s/#自测模式)。
