...
Zadig 构建是基于容器运行时环境提供的高效云原生构建引擎，内置了常见的构建环境及软件包、缓存机制，支持多个服务共享构建脚本及 Jenkins 构建的调用，还可以通过自定义[构建镜像](/v1.12.0/settings/custom-image) 及 [安装软件包](/v1.12.0/settings/app/) 满足更为丰富的构建场景。
